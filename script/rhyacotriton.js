...
(function(){var j="execute",i="north",h="Control+S",g="Control+P",f="Control+R",d="Control+D",c="changeSelection",b="Control+A",a="rhyacotriton.Container";
qx.Class.define(a,{extend:qx.ui.container.Composite,construct:function(){var k=new qx.ui.layout.Dock();
qx.ui.container.Composite.call(this,k);
this._initializeCommands();
this.__kI=new rhyacotriton.Header();
this.add(this.__kI,{edge:i});
this.__kJ=new rhyacotriton.ToolBar(this);
this.add(this.__kJ,{edge:i});
this.__kK=new rhyacotriton.Store;
this.__kL=new rhyacotriton.Table(this.__kK);
this.add(this.__kL);
var l=this.__kL.getSelectionModel();
l.addListener(c,function(e){var m=!(e.getCurrentTarget().isSelectionEmpty());
this.enableRowButtons(m);
},this.__kJ);
},members:{removeSelectedRows:function(){this.__kL.removeSelectedRows();
},startSelectedRows:function(){this.__kL.startSelectedRows();
},stopSelectedRows:function(){this.__kL.stopSelectedRows();
},reload:function(){},showAddTorrent:function(){},getCommand:function(n){return this.__kM[n];
},_initializeCommands:function(){var o={};
o.reload=new qx.ui.core.Command(f);
o.reload.addListener(j,this.reload,this);
o.addTorrent=new qx.ui.core.Command(b);
o.addTorrent.addListener(j,this.showAddTorrent,this);
o.removeSelectedRows=new qx.ui.core.Command(d);
o.removeSelectedRows.addListener(j,this.removeSelectedRows,this);
o.startSelectedRows=new qx.ui.core.Command(h);
o.startSelectedRows.addListener(j,this.startSelectedRows,this);
o.stopSelectedRows=new qx.ui.core.Command(g);
o.stopSelectedRows.addListener(j,this.stopSelectedRows,this);
this.__kM=o;
}}});
})();
...
}},properties:{sort:{check:[l,m,n],init:l,apply:j},separatorX:{check:p,nullable:true,apply:s},separatorY:{check:p,nullable:true,apply:s},connectSeparators:{check:e,init:false,apply:s},spacingX:{check:o,init:0,apply:s},spacingY:{check:o,init:0,apply:s}},members:{__ff:null,__kN:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kO:{north:1,south:2,west:3,east:4,center:5},__kP:{1:q,2:d,3:r,4:g},__kQ:function(){var x=this._getLayoutChildren();
var F,z;
var length=x.length;
var B=[];
var E=[];
...
},renderLayout:function(H,I){if(this._invalidChildrenCache){this.__kQ();
...
}},destruct:function(){this.__kN=this.__ff=null;
}});
})();
...
}},properties:{alignX:{check:[m,k,c],init:m,apply:n},alignY:{check:[j,g,e],init:j,apply:n},spacing:{check:a,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:d,init:false,apply:f}},members:{__kR:null,__kS:null,__kT:null,__ff:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kQ:function(){var w=this._getLayoutChildren();
var length=w.length;
var t=false;
var r=this.__kR&&this.__kR.length!=length&&this.__kS&&this.__kR;
var u;
var s=r?this.__kR:new Array(length);
var v=r?this.__kS:new Array(length);
...
this.__ff=w;
delete this._invalidChildrenCache;
},verifyLayoutProperty:null,renderLayout:function(x,y){if(this._invalidChildrenCache){this.__kQ();
}var E=this.__ff;
var length=E.length;
...
for(i=0;i<length;i+=1){J=this.__kR[i];
K=J!=null?Math.floor((x-B)*J):E[i].getSizeHint().width;
P.push(K);
F+=K;
}if(this.__kT&&F!=x){var H={};
var L,O;

for(i=0;i<length;i+=1){L=this.__kS[i];
...
}},destruct:function(){this.__kR=this.__kS=this.__ff=null;
}});
})();
...
}else if(this.__kU.length>0){do{var M=this.__kU[0];
...
var K=false;
if(this.__kU.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__kW(M);
B+=F;
if(C&&this.__kU.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__kU.length>0);
}},__kW:function(N){N.setVisibility(w);
this.__kU.shift();
this.fireDataEvent(p,N);
},__kX:function(O){if(!O){return;
}this.__kU.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__kV.length-1;i>=0;i--){var P=this.__kV[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__kV[T]!=undefined){throw new Error("Priority already in use!");
}this.__kV[T]=S;
...
}for(var i=0;i<this.__kU.length;i++){this.__kU[i].setVisibility(w);
}this.__kU=[];
...
}},__kY:false,_setAllowMenuOpenHover:function(bc){this.__kY=bc;
},_isAllowMenuOpenHover:function(){return this.__kY;
...
qx.Mixin.define(j,{events:{"execute":g},properties:{command:{check:b,apply:f,event:a,nullable:true}},members:{__la:null,__lb:false,__lc:null,_bindableProperties:[d,k,l,m,h,c],execute:function(){var o=this.getCommand();

if(o){if(this.__lb){this.__lb=false;
}else{this.__lb=true;
o.execute(this);
}}this.fireEvent(n);
},__ld:function(e){if(this.__lb){this.__lb=false;
return;
}this.__lb=true;
this.execute();
},_applyCommand:function(p,q){if(q!=null){q.removeListenerById(this.__lc);
}
if(p!=null){this.__lc=p.addListener(n,this.__ld,this);
}var t=this.__la;

if(t==null){this.__la=t={};
}var u;
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="__le",o="blur",n="Enter",m="__lf",l="__jG",d="Up",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
}this.__le=new qx.event.Timer;
this.__le.addListener(t,this._onOpenInterval,this);
this.__lf=new qx.event.Timer;
this.__lf.addListener(t,this._onCloseInterval,this);
},members:{__lg:null,__lh:null,__le:null,__lf:null,__jG:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
if(L.isVisible()){if(this.__lg){this.cancelOpen(this.__lg);
}}else if(this.__lg!=L){this.__lg=L;
this.__le.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__lh){this.cancelClose(this.__lh);
}}else if(this.__lh!=M){this.__lh=M;
this.__lf.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__lg==N){this.__le.stop();
this.__lg=null;
}},cancelClose:function(O){if(this.__lh==O){this.__lf.stop();
this.__lh=null;
}},_onOpenInterval:function(e){this.__le.stop();
this.__lg.open();
this.__lg=null;
},_onCloseInterval:function(e){this.__lf.stop();
this.__lh.exclude();
this.__lh=null;
...
}},__li:{"Enter":1,"Space":1},__lj:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__lj[R]||(this.__li[R]&&Q.getSelectedButton())){e.stopPropagation();
...
},__ln:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__lo:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__lo,this);
return;
...
if(this.__lm){this.__lm.call(this);
delete this.__lm;
...
if(this.__ll){bo.cancelOpen(this.__ll);
this.__ll=null;
...
}if(this.__ll){br.cancelOpen(this.__ll);
...
if(q){this.setSeparator(q);
}},properties:{alignY:{check:[m,j,e],init:m,apply:n},alignX:{check:[k,g,a],init:k,apply:n},spacing:{check:c,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:b,init:false,apply:f}},members:{__lp:null,__kS:null,__kT:null,__ff:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kQ:function(){var w=this._getLayoutChildren();
var length=w.length;
var s=false;
var r=this.__lp&&this.__lp.length!=length&&this.__kS&&this.__lp;
var u;
...
K.push(B);
Q+=B;
}if(this.__kT&&Q!=y){var H={};
var N,R;

for(i=0;i<length;i+=1){N=this.__kS[i];
...
}},destruct:function(){this.__lp=this.__kS=this.__ff=null;
}});
})();
(function(){var c="Integer",b="_applyLayoutChange",a="qx.ui.menu.Layout";
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__lq:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__lq=[0,0,0,0];
...
},getColumnSizes:function(){return this.__lq||null;
...
}},__lr:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
},events:{"execute":j,"press":j,"release":j},properties:{interval:{check:l,init:100},firstInterval:{check:l,init:500},minTimer:{check:l,init:20},timerDecrease:{check:l,init:2}},members:{__ls:null,__jj:null,press:function(){if(this.isEnabled()){if(!this.hasState(n)){this.__lt();
}this.removeState(m);
this.addState(n);
...
},events:{"interval":b},properties:{interval:{check:e,init:100},firstInterval:{check:e,init:500},minimum:{check:e,init:20},decrease:{check:e,init:2}},members:{__jj:null,__lv:null,start:function(){this.__jj.setInterval(this.getFirstInterval());
this.__jj.start();
},stop:function(){this.__jj.stop();
this.__lv=null;
},_onInterval:function(){this.__jj.stop();

if(this.__lv==null){this.__lv=this.getInterval();
}this.__lv=Math.max(this.getMinimum(),this.__lv-this.getDecrease());
this.__jj.setInterval(this.__lv);
this.__jj.start();
this.fireEvent(d);
}},destruct:function(){this._disposeObjects(c);
}});
...
s.addListener(b,this.__lw,this);
this._add(s);
break;
...
},__lw:function(){var w=this.getChildrenContainer().getChildren();
...
(function(){var p="removeSelectedRows",o="Add",n="Start",m="icon/22/actions/list-remove.png",l="icon/22/actions/list-add.png",k="icon/22/actions/view-refresh.png",j="startSelectedRows",i="icon/22/actions/media-playback-pause.png",h="addTorrent",g="reload",c="stopSelectedRows",f="icon/22/actions/media-playback-start.png",e="Pause",b="Refresh",a="Remove",d="rhyacotriton.ToolBar";
qx.Class.define(d,{extend:qx.ui.toolbar.ToolBar,construct:function(q){qx.ui.toolbar.ToolBar.call(this);
this.__lx=new qx.ui.toolbar.Button(this.tr(b),k);
this.__lx.setCommand(q.getCommand(g));
this.add(this.__lx);
this.__ly=new qx.ui.toolbar.Button(this.tr(o),l);
this.__ly.setCommand(q.getCommand(h));
this.add(this.__ly);
this.__lz=new qx.ui.toolbar.Button(this.tr(a),m);
this.__lz.setCommand(q.getCommand(p));
this.add(this.__lz);
this.__lA=new qx.ui.toolbar.Button(this.tr(n),f);
this.__lA.setCommand(q.getCommand(j));
this.add(this.__lA);
this.__lB=new qx.ui.toolbar.Button(this.tr(e),i);
this.__lB.setCommand(q.getCommand(c));
this.add(this.__lB);
this.enableRowButtons(false);
},members:{enableRowButtons:function(r){this.__lz.setEnabled(r);
this.__lA.setEnabled(r);
this.__lB.setEnabled(r);
}}});
})();
...
(function(){var f="qx.event.type.Data",e="Erlang",d="rhyacotriton.Store",c="PHP",b="dataRemoveFailure",a="dataLoadCompleted";
qx.Class.define(d,{extend:qx.core.Object,construct:function(){},members:{loadData:function(){var g={"data":[{id:1,name:e,size:100000,progress:0.1,online:true},{id:2,name:c,size:130000,progress:1.0,online:false}]};
this.fireDataEvent(a,g);
},removeElement:function(h){var i=h.id;
var j={"id":i};
this.fireDataEvent(b,j,h);
},stop:function(k){},start:function(l){}},events:{"dataUpdated":f,"dataRemoved":f,"dataRemoveFailure":f,"dataLoadCompleted":f}});
})();
(function(){var p="Boolean",o="column-button",n="Function",m="qx.event.type.Data",k="statusbar",h="qx.ui.table.pane.CellEvent",g="function",f="PageUp",e="dataChanged",d="changeLocale",bH="__lM",bG="changeSelection",bF="appear",bE="qx.dynlocale",bD='"',bC="Enter",bB="metaDataChanged",bA="_applyStatusBarVisible",bz="columnVisibilityMenuCreateStart",by="blur",w="qx.ui.table.Table",y="columnVisibilityMenuCreateEnd",u="changeVisible",v="_applyResetSelectionOnHeaderClick",s="_applyMetaColumnCounts",t="focus",q="changeDataRowRenderer",r="changeHeaderCellHeight",F="Escape",G="A",ba="changeSelectionModel",V="Left",bi="__lD",bd="Down",bu="Integer",bo="_applyHeaderCellHeight",O="visibilityChanged",bx="qx.ui.table.ITableModel",bw="orderChanged",bv="_applySelectionModel",M="menu-button",R="menu",T="_applyAdditionalStatusBarText",X="_applyFocusCellOnMouseMove",bb="table",be="_applyColumnVisibilityButtonVisible",bk="changeTableModel",bq="qx.event.type.Event",z="__lK",A="tableWidthChanged",Q="_applyHeaderCellsVisible",bh="Object",bg="__jj",bf="_applyShowCellFocusIndicator",bm="resize",bl="verticalScrollBarChanged",bc="changeScrollY",bj="_applyTableModel",a="End",bp="_applyKeepFirstVisibleRowComplete",B="widthChanged",C="one of one row",W="Home",b="_applyRowHeight",c="F2",L="Up",D="%1 rows",E="qx.ui.table.selection.Model",K="one row",Y="__lC",bs="PageDown",br="%1 of %2 rows",S="keypress",bt="changeRowHeight",N="Number",bn="header",H="__lL",J="_applyContextMenuFromDataCellsOnly",P="qx.ui.table.IRowRenderer",U="Right",I="Space";
...
this.__lC=new qx.ui.container.Composite(new qx.ui.layout.HBox());
this._add(this.__lC,{flex:1});
this.setDataRowRenderer(new qx.ui.table.rowrenderer.Default(this));
this.__lD=this.getNewSelectionManager()(this);
...
}}},members:{__lE:null,__lF:null,__lC:null,__lD:null,__lH:null,__lI:null,__lJ:null,__lK:null,__lL:null,__lM:null,__lN:null,__jj:null,_createChildControlImpl:function(bS,bT){var bU;
...
},_applySelectionModel:function(bW,bX){this.__lD.setSelectionModel(bW);
...
}},_applyAdditionalStatusBarText:function(cr,cs){this.__lH=cr;
this._updateStatusBar();
...
for(var cG in qx.ui.table.Table.__lG){cB[cG]={};
...
for(cG in qx.ui.table.Table.__lG){if(!cB[cG]){break;
}
...
}},getSelectionManager:function(){return this.__lD;
},_getPaneScrollerArr:function(){return this.__lC.getChildren();
...
if(this.__lF>=dq&&this.__lF<(dq+dr)){this.setFocusedCell();
}}
for(var i=0;i<ds.length;i++){ds[i].onTableModelDataChanged(dl,dm,dn,dp);
}var dt=this.getTableModel().getRowCount();

if(dt!=this.__lI){this.__lI=dt;
this._updateScrollBarVisibility();
this._updateStatusBar();
}},_onScrollY:function(du){if(!this.__lJ){this.__lJ=true;
...
if(this.__lF!=dD){dA=this.startEditing();
}break;
...
default:dA=false;
break;
}}else{switch(dE){case I:this.__lD.handleSelectKeyDown(this.__lF,dw);
break;
case c:case bC:this.startEditing();
dA=true;
break;
case W:this.setFocusedCell(this.__lE,0,true);
break;
case a:var dB=this.getTableModel().getRowCount();
this.setFocusedCell(this.__lE,dB-1,true);
break;
...
if(dD!=this.__lF&&this.getRowFocusChangeModifiesSelection()){this.__lD.handleMoveKeyDown(this.__lF,dw);
}
...
},setFocusedCell:function(dR,dS,dT){if(!this.isEditing()&&(dR!=this.__lE||dS!=this.__lF)){if(dR===null){dR=0;
}this.__lE=dR;
this.__lF=dS;
...
},getFocusedColumn:function(){return this.__lE;
},getFocusedRow:function(){return this.__lF;
...
}},moveFocusedCell:function(ea,eb){var ef=this.__lE;
var eg=this.__lF;
...
}},isEditing:function(){if(this.__lE!=null){var x=this.getTableColumnModel().getVisibleX(this.__lE);
var em=this._getMetaColumnAtColumnX(x);
return this.getPaneScroller(em).isEditing();
}return false;
},startEditing:function(){if(this.__lE!=null){var x=this.getTableColumnModel().getVisibleX(this.__lE);
var eo=this._getMetaColumnAtColumnX(x);
var en=this.getPaneScroller(eo).startEditing();
return en;
}return false;
},stopEditing:function(){if(this.__lE!=null){var x=this.getTableColumnModel().getVisibleX(this.__lE);
var ep=this._getMetaColumnAtColumnX(x);
this.getPaneScroller(ep).stopEditing();
}},cancelEditing:function(){if(this.__lE!=null){var x=this.getTableColumnModel().getVisibleX(this.__lE);
...
if(this.__lH){if(eD){eD+=this.__lH;
}else{eD=this.__lH;
}}
...
if(eP&&eN!=this.__lN){this.fireDataEvent(bl,eN);
...
this._updateScrollerWidths();
this._updateScrollBarVisibility();
},addListener:function(fd,fe,self,ff){if(this.self(arguments).__lG[fd]){var fh=[fd];
...
}},removeListener:function(fi,fj,self,fk){if(this.self(arguments).__lG[fi]){for(var i=0,fl=this._getPaneScrollerArr();i<fl.length;i++){fl[i].removeListener.apply(fl[i],arguments);
}}else{qx.ui.core.Widget.prototype.removeListener.call(this,fi,fj,self,fk);
}},removeListenerById:function(fm){var fq=fm.split(bD);
var fp=fq.shift();

if(this.self(arguments).__lG[fp]){var fo=true;
...
this._disposeObjects(bi,Y,bH,bH,H,bg);
this._disposeMap(z);
}});
})();
...
},properties:{highlightFocusRow:{check:h,init:true}},members:{_colors:null,__lP:null,__lO:null,_insetY:1,_renderFont:function(x){if(x){this.__lP=x.getStyles();
this.__lO=qx.bom.element.Style.compile(this.__lP);
this.__lO=this.__lO.replace(/"/g,d);
}else{this.__lO=j;
this.__lP=qx.bom.Font.getDefaultStyles();
}},updateDataRowElement:function(y,z){var B=this.__lP;
var A=z.style;
...
}},destruct:function(){this._colors=this.__lP=this.__lO=null;
}});
})();
...
},members:{__lQ:false,_applyVisible:function(h,i){if(!this.bInListener){this.setValue(h);
}}}});
})();
(function(){var b="qx.ui.table.selection.Model",a="qx.ui.table.selection.Manager";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
},properties:{selectionModel:{check:b}},members:{__lR:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();
...
}}},handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__lR){this._handleSelectEvent(f,g);
...
this.__lV=false;
},events:{"changeSelection":f},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:a}},members:{__lV:null,__lT:null,__lU:null,__lS:null,_applySelectionMode:function(o){this.resetSelection();
...
},_setAnchorSelectionIndex:function(q){this.__lT=q;
},getLeadSelectionIndex:function(){return this.__lU;
},_setLeadSelectionIndex:function(r){this.__lU=r;
...
for(var i=0;i<this.__lS.length;i++){var s=this.__lS[i];
t+=s.maxIndex-s.minIndex+1;
}return t;
},isSelectedIndex:function(u){for(var i=0;i<this.__lS.length;i++){var v=this.__lS[i];

if(u>=v.minIndex&&u<=v.maxIndex){return true;
}}return false;
},getSelectedRanges:function(){var w=[];

for(var i=0;i<this.__lS.length;i++){w.push({minIndex:this.__lS[i].minIndex,maxIndex:this.__lS[i].maxIndex});
}return w;
},iterateSelection:function(x,y){for(var i=0;i<this.__lS.length;i++){for(var j=this.__lS[i].minIndex;j<=this.__lS[i].maxIndex;j++){x.call(y,j);
...
this.__lT=-1;
this.__lU=-1;
},_addSelectionInterval:function(N,O){this.__lT=N;
this.__lU=O;
var P=Math.min(N,O);
var R=Math.max(N,O);
var Q=0;

for(;Q<this.__lS.length;Q++){var S=this.__lS[Q];

if(S.minIndex>P){break;
}}this.__lS.splice(Q,0,{minIndex:P,maxIndex:R});
var T=this.__lS[0];

for(var i=1;i<this.__lS.length;i++){var S=this.__lS[i];
...
for(var i=0;i<this.__lS.length;i++){var V=this.__lS[i];
...
if(!J.__lW){var L=qx.theme.manager.Color.getInstance();
J.__lW=this.self(arguments);
...
}J.__lW.stylesheet=qx.bom.Stylesheet.createElement(K);
...
this.__lX=qx.locale.Manager.getInstance().bind(dn,this,dn);
}else{this.__iz=dt;
this.setLocale(dt);
}this.__lY=this.__iz;

if(ds!=null){this.__ma=ds.toString();

if(this.__ma in qx.util.format.DateFormat.ISO_MASKS){if(this.__ma===d){this.__mb=true;
}this.__ma=qx.util.format.DateFormat.ISO_MASKS[this.__ma];
}}else{this.__ma=qx.locale.Date.getDateFormat(bd,this.__iz)+N+qx.locale.Date.getDateTimeFormat(dq,dl,this.__iz);
...
if(dv._dateInstance==null||dv._dateInstance.__ma!=du){dv._dateTimeInstance=new dv();
...
if(dx._dateInstance==null||dx._dateInstance.__ma!=dw){dx._dateInstance=new dx(dw);
}return dx._dateInstance;
},ASSUME_YEAR_2000_THRESHOLD:30,LOGGING_DATE_TIME__format:da,ISO_MASKS:{isoDate:"yyyy-MM-dd",isoTime:"HH:mm:ss",isoDateTime:"yyyy-MM-dd'T'HH:mm:ss",isoUtcDateTime:"yyyy-MM-dd'T'HH:mm:ss'Z'"},AM_MARKER:"am",PM_MARKER:"pm"},members:{__lX:null,__iz:null,__lY:null,__ma:null,__mc:null,__md:null,__me:null,__mb:null,__mf:function(dy,dz){var dA=cG+(dy<0?((-1)*dy):dy);

while(dA.length<dz){dA=bH+dA;
}return dy<0?cF+dA:dA;
},__mg:function(dB){var dC=new Date(dB.getTime());
var dD=dC.getDate();

while(dC.getMonth()!=0){dC.setDate(-1);
dD+=dC.getDate()+1;
}return dD;
},__mh:function(dE){return new Date(dE.getTime()+(3-((dE.getDay()+6)%7))*86400000);
},__mi:function(dF){var dH=this.__mh(dF);
var dI=dH.getFullYear();
var dG=this.__mh(new Date(dI,0,4));
return Math.floor(1.5+(dH.getTime()-dG.getTime())/86400000/7);
},__mj:function(dJ){var dL=this.__mh(dJ);
var dK=this.__mh(new Date(dJ.getFullYear(),dJ.getMonth(),4));
return Math.floor(1.5+(dL.getTime()-dK.getTime())/86400000/7);
},__mk:function(dM){var dN=this.__mh(dM);
return dN.getFullYear();
},__ml:function(dO){var dP=new Date(dO,2,1);
dP.setDate(-1);
return dP.getDate()+1===29;
},__mm:function(dQ,dR){var dV=0;
var dW=0;
if(!dR){dR=1971;
}var dS=0;
...
},__mn:function(dX,dY,ea){var eb;

switch(dY){case 11:eb=dX-1;

if(dX!=this.__mk(new Date(eb,dY,ea))){eb=dX;
}break;
case 0:eb=dX+1;
...
},_applyLocale:function(ec,ed){this.__iz=ec===null?this.setLocale(this.__lY):ec;
},format:function(ee){if(ee==null){return null;
}
if(isNaN(ee.getTime())){return null;
}
if(this.__mb){ee=new Date(ee.getUTCFullYear(),ee.getUTCMonth(),ee.getUTCDate(),ee.getUTCHours(),ee.getUTCMinutes(),ee.getUTCSeconds(),ee.getUTCMilliseconds());
}var eh=this.__iz;
...
for(var i=0;i<this.__me.length;i++){var en=this.__me[i];
...
switch(eg){case x:if(er==2){ez=this.__mf(em%100,2);
...
case cl:if(er==1||er==2){ez=this.__mf(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case U:if(er==1||er==2){ez=this.__mf(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case cz:ez=this.__mf(this.__mg(ee),er);
break;
case dc:ez=this.__mf(el,er);
break;
case A:ez=this.__mf(this.__mi(ee),er);
break;
case E:ez=this.__mj(ee);
break;
...
if(er>=1&&er<=2){ez=this.__mf(eu,er);
...
case H:if(er==1||er==2){ez=this.__mf(ei+1,er);
...
case t:if(er==1||er==2){ez=this.__mf(ei+1,er);
...
break;
case ca:ez=this.__mf(es,er);
break;
case cu:ez=this.__mf((es==0)?24:es,er);
break;
case cy:ez=this.__mf(es%12,er);
break;
case bT:ez=this.__mf(((es%12)==0)?12:(es%12),er);
break;
case m:ez=this.__mf(ey,er);
break;
case bR:ez=this.__mf(eq,er);
break;
case cb:ez=ew+cG;
...
case bb:if(er>=1&&er<=4){ez=dm+((et>0)?cF:cO)+this.__mf(Math.abs(ef),2)+w+this.__mf(ev,2);
}break;
case K:if(er>=1&&er<=3){ez=((et>0)?cF:cO)+this.__mf(Math.abs(ef),2)+this.__mf(ev,2);
}else{ez=dm+((et>0)?cF:cO)+this.__mf(Math.abs(ef),2)+w+this.__mf(ev,2);
}break;
}ex+=ez;
}}return ex;
},parse:function(eA){this.__mp();
var eJ=this.__mc.regex.exec(eA);

if(eJ==null){throw new Error("Date string '"+eA+"' does not match the date format: "+this.__ma);
...
for(var i=0;i<this.__mc.usedRules.length;i++){var eF=this.__mc.usedRules[i];
var eG=eJ[eD];
...
for(var k=0;k<this.__mc.usedRules.length;k++){if(this.__mc.usedRules[k].pattern==dr){eH=true;
break;
}}
if(!eH){eC=true;
}}
...
for(var k=0;k<this.__mc.usedRules.length;k++){if(this.__mc.usedRules[k].pattern.indexOf(bi)!=-1){eL=true;
break;
}}
if(!eL){eE=true;
}}eD+=(eF.groups==null)?1:eF.groups;
}
if(eC){eB.year=this.__mn(eB.weekYear,eB.month,eB.day);
}
if(eE){var eK=this.__mm(eB.dayOfYear,eB.year);
eB.month=eK.month;
eB.day=eK.day;
}
...
if(this.__mb){eI=new Date(eI.getUTCFullYear(),eI.getUTCMonth(),eI.getUTCDate(),eI.getUTCHours(),eI.getUTCMinutes(),eI.getUTCSeconds(),eI.getUTCMilliseconds());
}
...
},__mo:function(){if(this.__me!=null){return;
}this.__me=[];
var eQ;
var eO=0;
var eS=cG;
...
case bF:if(eR==eQ){eO++;
i++;
}else{this.__me.push({type:bF,character:eQ,size:eO});
eQ=null;
eO=0;
eP=bI;
}break;
...
if(eP!=bI){if(eS.length>0){this.__me.push({type:bG,text:eS});
eS=cG;
}}else{eS+=eR;
i++;
}break;
}}if(eQ!=null){this.__me.push({type:bF,character:eQ,size:eO});
}else if(eS.length>0){this.__me.push({type:bG,text:eS});
}},__mp:function(){if(this.__mc!=null){return ;
}var eW=this.__ma;
this.__mr();
this.__mo();
var fd=[];
...
for(var eU=0;eU<this.__me.length;eU++){var fe=this.__me[eU];
...
for(var ff=0;ff<this.__md.length;ff++){var fb=this.__md[ff];

if(this.__mq(fb,eV,fa)){eX=fb;
break;
...
}this.__mc={regex:eT,"usedRules":fd,pattern:eY};
},__mq:function(fh,fi,fj){if(fi===x&&fh.pattern===dr){fh.regex=fh.regexFunc(fj);
return true;
}else if(fi===be&&fh.pattern===bw){fh.regex=fh.regexFunc(fj);
return true;
}else{return fi==fh.pattern.charAt(0)&&fj==fh.pattern.length;
}},__mr:function(){var fx=qx.util.format.DateFormat;
var fz=qx.lang.String;

if(this.__md!=null){return ;
}var fy=this.__md=[];
...
}},destruct:function(){if(this.__lX!=null){qx.locale.Manager.getInstance().removeBinding(this.__lX);
}this.__me=this.__mc=this.__md=null;
}});
})();
...
qx.Class.define(l,{statics:{__ms:qx.locale.Manager.getInstance(),getAmMarker:function(F){return this.__ms.localize(o,[],F);
},getPmMarker:function(G){return this.__ms.localize(p,[],G);
},getDayNames:function(length,H,I,J){var I=I?I:h;
var L=[B,C,D,a,e,E,d];
var M=[];

for(var i=0;i<L.length;i++){var K=c+I+k+length+k+L[i];
M.push(J?this.__mt(I,I===j?f:j,K,H):this.__ms.localize(K,[],H));
}return M;
},getDayName:function(length,N,O,P,Q){var P=P?P:h;
var S=[B,C,D,a,e,E,d];
var R=c+P+k+length+k+S[N];
return Q?this.__mt(P,P===j?f:j,R,O):this.__ms.localize(R,[],O);
},getMonthNames:function(length,T,U,V){var U=U?U:h;
var X=[];

for(var i=0;i<12;i++){var W=b+U+k+length+k+(i+1);
X.push(V?this.__mt(U,U===j?f:j,W,T):this.__ms.localize(W,[],T));
}return X;
},getMonthName:function(length,Y,ba,bb,bc){var bb=bb?bb:h;
var bd=b+bb+k+length+k+(Y+1);
return bc?this.__mt(bb,bb===j?f:j,bd,ba):this.__ms.localize(bd,[],ba);
},getDateFormat:function(be,bf){var bg=v+be;
return this.__ms.localize(bg,[],bf);
},getDateTimeFormat:function(bh,bi,bj){var bl=m+bh;
var bk=this.__ms.localize(bl,[],bj);

if(bk==bl){bk=bi;
}return bk;
},getTimeFormat:function(bm,bn){var bp=t+bm;
var bo=this.__ms.localize(bp,[],bn);
...
}else{bE=this.__ms.getTerritory()||this.__ms.getLanguage();
}return bE.toUpperCase();
},__mt:function(bF,bG,bH,bI){var bK=this.__ms.localize(bH,[],bI);

if(bK==bH){var bJ=bH.replace(g+bF+g,g+bG+g);
return this.__ms.localize(bJ,[],bI);
}else{return bK;
}}}});
})();
...
qx.Mixin.define(d,{construct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().addListener(j,this.__mu,this);
...
},__mu:qx.core.Environment.select(l,{"true":function(e){var o=this.getInvalidMessage();
...
}},"false":null})},destruct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().removeListener(j,this.__mu,this);
}}});
})();
...
if(bk&&this.__mv){this.__mC().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__mv){this.__mC().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__mv){if(bs){this._showPlaceholder();
}else{this._removePlaceholder();
}}else{var bu=this.getContentElement();
bu.setAttribute(a,bs?this.getPlaceholder():j);
}},__mA:{width:16,height:16},_getContentHint:function(){return {width:this.__mA.width*10,height:this.__mA.height||16};
...
if(this.__mv){this.__mC().setStyles(bx);
}if(bv){this.__mA=qx.bom.Label.getTextSize(d,bx);
}else{delete this.__mA;
}qx.ui.core.queue.Layout.add(this);
...
this.selectAllText();
},_getTextSize:function(){return this.__mA;
},_onHtmlInput:function(e){var bD=e.getData();
var bC=true;
this.__mw=false;
if(this.__mz&&this.__mz===bD){bC=false;
...
}if(bC){this.fireDataEvent(be,bD,this.__mz);
this.__mz=bD;
if(this.getLiveUpdate()){this.__mB(bD);
}}},_onWebFontStatusChange:function(bF){if(bF.getData().valid===true){var bG=this.__jK.getStyles();
this.__mA=qx.bom.Label.getTextSize(d,bG);
qx.ui.core.queue.Layout.add(this);
}},__mB:function(bH){var bI=this.__my;
this.__my=bH;

if(bI!=bH){this.fireNonBubblingEvent(o,qx.event.type.Data,[bH,bI]);
}},setValue:function(bJ){if(bJ===null){if(this.__mw){return bJ;
}bJ=j;
this.__mw=true;
}else{this.__mw=false;
if(this.__mv){this._removePlaceholder();
}}
...
bL.setValue(bJ);
var bK=this.__mw?null:bJ;
this.__my=bM;
this.__mB(bK);
}if(this.__mv){this._showPlaceholder();
}return bJ;
}throw new Error("Invalid value type: "+bJ);
},getValue:function(){var bN=this.getContentElement().getValue();
return this.__mw?null:bN;
},resetValue:function(){this.setValue(null);
},_onChangeContent:function(e){this.__mw=e.getData()===null;
this.__mB(e.getData());
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__mC().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__mC().setStyle(bc,L);
}},__mC:function(){if(this.__mx==null){this.__mx=new qx.html.Label();
var bS=qx.theme.manager.Color.getInstance();
this.__mx.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bS.resolve(I)});
this.getContainerElement().add(this.__mx);
}return this.__mx;
},_onChangeLocale:qx.core.Environment.select(g,{"true":function(e){var content=this.getPlaceholder();

if(content&&content.translate){this.setPlaceholder(content.translate());
}},"false":null}),_applyPlaceholder:function(bT,bU){if(this.__mv){this.__mC().setValue(bT);
...
}}},destruct:function(){this.__mx=this.__jK=null;
...
this.__mD=o;
},members:{__mD:null,__mE:null,__mF:null,_createDomElement:function(){return qx.bom.Input.create(this.__mD);
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__mF=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__mE?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__mE=w;
qx.html.Element.prototype.setSelectable.call(this,this.__mF&&w);
...
},setWrap:function(B,C){if(this.__mD===l){this._setProperty(n,B,C);
}else{throw new Error("Text wrapping is only support by textareas!");
}return this;
},getWrap:function(){if(this.__mD===l){return this._getProperty(n);
}else{throw new Error("Text wrapping is only support by textareas!");
}}}});
})();
...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__mG:false,__mH:null,__my:null,__mz:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===g)){return true;
}
if(C===j&&(D===m||D===g||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&(qx.core.Environment.get(d)<9||(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)<9))){if(!E.__mI){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__mJ:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(d)<9&&qx.core.Environment.get(A)<9){if(O.__mI){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__mK:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__my){this.__my=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__mz){this.__mz=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__mG=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__mG=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__mH&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__mH);
...
if(!this.__mG||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__mH=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
(function(){var k="qx.event.type.Data",j="visibilityChanged",h="orderChanged",g="visibilityChangedPre",f="widthChanged",e="qx.ui.table.columnmodel.Basic",d="__nd",c="__nc",b="__nb",a="headerCellRendererChanged";
qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__mW=[];
this.__mX=[];
},events:{"widthChanged":k,"visibilityChangedPre":k,"visibilityChanged":k,"orderChanged":k,"headerCellRendererChanged":k},statics:{DEFAULT_WIDTH:100,DEFAULT_HEADER_RENDERER:qx.ui.table.headerrenderer.Default,DEFAULT_DATA_RENDERER:qx.ui.table.cellrenderer.Default,DEFAULT_EDITOR_FACTORY:qx.ui.table.celleditor.TextField},members:{__lJ:null,__mY:null,__mX:null,__mW:null,__na:null,__nb:null,__nc:null,__nd:null,init:function(l,m){this.__na=[];
var q=qx.ui.table.columnmodel.Basic.DEFAULT_WIDTH;
var r=this.__nb||(this.__nb=new qx.ui.table.columnmodel.Basic.DEFAULT_HEADER_RENDERER());
var o=this.__nc||(this.__nc=new qx.ui.table.columnmodel.Basic.DEFAULT_DATA_RENDERER());
var n=this.__nd||(this.__nd=new qx.ui.table.columnmodel.Basic.DEFAULT_EDITOR_FACTORY());
this.__mW=[];
this.__mX=[];
var t;
...
for(var u=0;u<l;u++){this.__na[u]={width:q,headerRenderer:r,dataRenderer:o,editorFactory:n};
this.__mW[u]=u;
this.__mX[u]=u;
}this.__mY=null;
this.__lJ=true;
...
},setColumnWidth:function(v,w,y){var A=this.__na[v].width;

if(A!=w){this.__na[v].width=w;
var z={col:v,newWidth:w,oldWidth:A,isMouseAction:y||false};
this.fireDataEvent(f,z);
}},getColumnWidth:function(B){return this.__na[B].width;
},setHeaderCellRenderer:function(C,D){var E=this.__na[C].headerRenderer;

if(E!==this.__nb){E.dispose();
}this.__na[C].headerRenderer=D;
this.fireDataEvent(a,{col:C});
},getHeaderCellRenderer:function(F){return this.__na[F].headerRenderer;
},setDataCellRenderer:function(G,H){this.__na[G].dataRenderer=H;
var I=this.__na[G].dataRenderer;

if(I!==this.__nc){return I;
}return null;
},getDataCellRenderer:function(J){return this.__na[J].dataRenderer;
},setCellEditorFactory:function(K,L){var M=this.__na[K].headerRenderer;

if(M!==this.__nd){M.dispose();
}this.__na[K].editorFactory=L;
},getCellEditorFactory:function(N){return this.__na[N].editorFactory;
},_getColToXPosMap:function(){if(this.__mY==null){this.__mY={};

for(var Q=0;Q<this.__mW.length;Q++){var P=this.__mW[Q];
this.__mY[P]={overX:Q};
}
for(var O=0;O<this.__mX.length;O++){var P=this.__mX[O];
this.__mY[P].visX=O;
}}return this.__mY;
},getVisibleColumnCount:function(){return this.__mX!=null?this.__mX.length:0;
},getVisibleColumnAtX:function(R){return this.__mX[R];
},getVisibleX:function(S){return this._getColToXPosMap()[S].visX;
},getOverallColumnCount:function(){return this.__mW.length;
},getOverallColumnAtX:function(T){return this.__mW[T];
...
for(var x=bb+1;x<this.__mW.length;x++){var bd=this.__mW[x];
var Y=be[bd].visX;

if(Y!=null){bc=Y;
break;
}}if(bc==null){bc=this.__mX.length;
}this.__mX.splice(bc,0,W);
}else{var ba=this.getVisibleX(W);
this.__mX.splice(ba,1);
}this.__mY=null;
if(!this.__lJ){var bf={col:W,visible:X};
this.fireDataEvent(g,bf);
this.fireDataEvent(j,bf);
}}},moveColumn:function(bg,bh){this.__lJ=true;
var bk=this.__mW[bg];
var bi=this.isColumnVisible(bk);

if(bi){this.setColumnVisible(bk,false);
}this.__mW.splice(bg,1);
this.__mW.splice(bh,0,bk);
this.__mY=null;
...
},setColumnsOrder:function(bl){if(bl.length==this.__mW.length){this.__lJ=true;
var bo=new Array(bl.length);

for(var bm=0;bm<this.__mW.length;bm++){var bn=this.isColumnVisible(bm);
bo[bm]=bn;

if(bn){this.setColumnVisible(bm,false);
}}this.__mW=qx.lang.Array.clone(bl);
this.__mY=null;
for(var bm=0;bm<this.__mW.length;bm++){if(bo[bm]){this.setColumnVisible(bm,true);
}}this.__lJ=false;
this.fireDataEvent(h);
}else{throw new Error("setColumnsOrder: Invalid number of column positions given, expected "+this.__mW.length+", got "+bl.length);
}}},destruct:function(){for(var i=0;i<this.__na.length;i++){this.__na[i].headerRenderer.dispose();
this.__na[i].dataRenderer.dispose();
this.__na[i].editorFactory.dispose();
}this.__mW=this.__mX=this.__na=this.__mY=null;
...
this.__lI=0;
this.__ng=[];
},events:{"paneReloadsData":b,"paneUpdated":z},properties:{firstVisibleRow:{check:j,init:0,apply:o},visibleRowCount:{check:j,init:0,apply:w},maxCacheLines:{check:j,init:1000,apply:C},allowShrinkX:{refine:true,init:false}},members:{__lI:null,__nf:null,__ne:null,__nh:null,__lF:null,__lE:null,__ng:null,__ni:0,_applyFirstVisibleRow:function(G,H){this.updateContent(false,G-H);
...
},setFocusedCell:function(K,L,M){if(K!=this.__lE||L!=this.__lF){var N=this.__lF;
this.__lE=K;
this.__lF=L;
...
},onTableModelDataChanged:function(Q,R,S,T){this.__nj();
var V=this.getFirstVisibleRow();
var U=this.getVisibleRowCount();

if(R==-1||R>=V&&Q<V+U){this.updateContent();
}},onTableModelMetaDataChanged:function(){this.updateContent(true);
},_applyMaxCacheLines:function(W,X){if(this.__ni>=W&&W!==-1){this.__nj();
}},__nj:function(){this.__ng=[];
this.__ni=0;
},__nk:function(Y,ba,bb){if(!ba&&!bb&&this.__ng[Y]){return this.__ng[Y];
}else{return null;
}},__nl:function(bc,bd,be,bf){var bg=this.getMaxCacheLines();

if(!be&&!bf&&!this.__ng[bc]&&bg>0){this._applyMaxCacheLines(bg);
this.__ng[bc]=bd;
this.__ni+=1;
}},updateContent:function(bh,bi,bj,bk){if(bh){this.__nj();
...
bz.push({col:bY,xPos:x,editable:bA.isColumnEditable(bY),focusedCol:this.__lE==bY,styleLeft:bC,styleWidth:bF});
bC+=bF;
}var bX=[];
var ca=false;
...
}}if(!this.__nh){this.__nh=document.createElement(E);
}var ck=e;
ck+=this._getRowsHtml(ci+ce,Math.abs(cb));
ck+=h;
this.__nh.innerHTML=ck;
var cf=this.__nh.firstChild.childNodes;
...
}}if(this.__lF!==null){this._updateRowStyles(this.__lF-cb);
this._updateRowStyles(this.__lF);
}this.fireEvent(f);
...
this.__lI=cq;
this.fireEvent(f);
}},destruct:function(){this.__nh=this.__ne=this.__ng=null;
}});
})();
(function(){var e="first",d="last",c="hovered",b="__ne",a="qx.ui.table.pane.Header";
qx.Class.define(a,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);
this._setLayout(new qx.ui.layout.HBox());
this.__ka=new qx.ui.core.Blocker(this);
this.__ne=f;
},members:{__ne:null,__nm:null,__nn:null,__ka:null,getPaneScroller:function(){return this.__ne;
},getTable:function(){return this.__ne.getTable();
},getBlocker:function(){return this.__ka;
...
}},setMouseOverColumn:function(k){if(k!=this.__nn){if(this.__nn!=null){var l=this.getHeaderWidgetAtColumn(this.__nn);
...
if(this.__nm==null){var y=this.getTable();
...
this.__nm=q;
}this.__nm.setLayoutProperties({left:s.left+x});
this.__nm.show();
...
return {width:this.__no?100:G,maxWidth:this.__no?null:G,minWidth:this.__no?null:G,height:this.__no?G:100,maxHeight:this.__no?G:null,minHeight:this.__no?G:null};
...
if(this.__no){content.scrollToX(K);
}else{content.scrollToY(K);
}},_applyOrientation:function(L,M){var N=this.__no=L===k;
...
qx.ui.core.queue.Layout.add(this);
},_updateScrollBar:function(){var P=this.__no;
...
var W=this.__no?X.getScrollX():X.getScrollY();
this.setPosition(W);
...
if(this.getOrientation()==h){if(R.height<S.minHeight+this.__nq){Q=true;
}}else{if(R.width<S.minWidth+this.__nq){Q=true;
}}
if(Q){P.exclude();
}else{P.show();
}}}});
...
},_onMouseDown:function(e){if(this.__nu){return;
}var bk=this.__no;
var bi=this.getChildControl(k);
var bj=bk?f:d;
var bm=bk?e.getDocumentLeft():e.getDocumentTop();
var bn=this.__nr=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bj];
var bl=this.__ns=qx.bom.element.Location.get(bi.getContainerElement().getDomElement())[bj];

if(e.getTarget()===bi){this.__nu=true;

if(!this.__nz){this.__nz=new qx.event.Timer(100);
this.__nz.addListener(W,this._fireValue,this);
}this.__nz.start();
this.__nv=bm+bn-bl;
bi.addState(b);
}else{this.__nw=true;
...
this.capture();
e.stopPropagation();
},_onMouseUp:function(e){if(this.__nu){this.releaseCapture();
delete this.__nu;
this.__nz.stop();
this._fireValue();
delete this.__nv;
...
if(this.__no){bp=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__nr);
...
}else{bp=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__nr);
...
if(bq<0||bq>this.__nt||bp<0||bp>this.__nt){this.getChildControl(k).removeState(g);
}}}else if(this.__nw){this.__jj.stop();
this.releaseCapture();
delete this.__nw;
delete this.__nx;
delete this.__ny;
...
}},_onMouseMove:function(e){if(this.__nu){var bs=this.__no?e.getDocumentLeft():e.getDocumentTop();
var br=bs-this.__nv;
this.slideTo(this._positionToValue(br));
}else if(this.__nw){this.__nD(e);
}e.stopPropagation();
},_onInterval:function(e){var bt=this.getValue()+(this.__nx*this.getPageStep());
if(bt<this.getMinimum()){bt=this.getMinimum();
}else if(bt>this.getMaximum()){bt=this.getMaximum();
}var bu=this.__nx==-1;

if((bu&&bt<=this.__ny)||(!bu&&bt>=this.__ny)){bt=this.__ny;
}this.slideTo(bt);
},_onUpdate:function(e){var bw=this.getInnerSize();
var bx=this.getChildControl(k).getBounds();
var bv=this.__no?E:C;
this._updateKnobSize();
this.__nC=bw[bv]-bx[bv];
this.__nt=bx[bv];
this._updateKnobPosition();
},__no:false,__nC:0,__nD:function(e){var by=this.__no;
...
var bI=this.__nx<0?N:z;
bD=bE+(Math[bI]((bD-bE)/bC)*bC);
}if(this.__ny==null||(this.__nx==-1&&bD<=this.__ny)||(this.__nx==1&&bD>=this.__ny)){this.__ny=bD;
}},_positionToValue:function(bK){var bL=this.__nC;
...
},_valueToPosition:function(bO){var bP=this.__nC;
...
if(this.__no){bT.setStyle(f,bS+a,true);
...
if(bU==null){return;
}if(this.__no){this.getChildControl(k).setWidth(Math.round(bV*bU.width));
...
if(this.__nu){this.__nB=[ce,cf];
}else{this.fireEvent(X,qx.event.type.Data,[ce,cf]);
}}else{this.resetValue();
}},_fireValue:function(){if(!this.__nB){return;
}var cg=this.__nB;
this.__nB=null;
...
(function(){var o="Boolean",n="resize-line",m="mousedown",l="qx.event.type.Data",k="mouseup",j="qx.ui.table.pane.CellEvent",i="scroll",h="focus-indicator",g="excluded",d="scrollbar-y",bq="table-scroller-focus-indicator",bp="visible",bo="mousemove",bn="header",bm="editing",bl="click",bk="modelChanged",bj="scrollbar-x",bi="cellClick",bh="pane",v="__nF",w="mouseout",t="changeHorizontalScrollBarVisible",u="__nK",r="bottom",s="_applyScrollTimeout",p="changeScrollX",q="_applyTablePaneModel",C="Integer",D="dblclick",M="dataEdited",J="mousewheel",U="interval",P="qx.ui.table.pane.Scroller",bd="_applyShowCellFocusIndicator",ba="y",F="resize",bg="vertical",bf="changeScrollY",be="appear",E="table-scroller",H="beforeSort",I="__jj",L="cellDblclick",N="__nE",Q="__nI",W="__kI",bc="horizontal",y="losecapture",z="contextmenu",G="__nL",T="col-resize",S="disappear",R="_applyVerticalScrollBarVisible",Y="__nJ",X="_applyHorizontalScrollBarVisible",O="__nG",V="os.scrollBarOverlayed",a="cellContextmenu",bb="close",A="changeTablePaneModel",B="x",K="qx.ui.table.pane.Model",b="__nH",c="changeVerticalScrollBarVisible";
qx.Class.define(P,{extend:qx.ui.core.Widget,include:qx.ui.core.scroll.MScrollBarFactory,construct:function(br){qx.ui.core.Widget.call(this);
this.__kL=br;
var bs=new qx.ui.layout.Grid();
bs.setColumnFlex(0,1);
bs.setRowFlex(1,1);
this._setLayout(bs);
this.__kI=this._showChildControl(bn);
this.__nE=this._showChildControl(bh);
this.__nF=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({minWidth:0});
this._add(this.__nF,{row:0,column:0,colSpan:2});
this.__nG=new qx.ui.table.pane.Clipper();
this.__nG.add(this.__kI);
this.__nG.addListener(y,this._onChangeCaptureHeader,this);
this.__nG.addListener(bo,this._onMousemoveHeader,this);
this.__nG.addListener(m,this._onMousedownHeader,this);
this.__nG.addListener(k,this._onMouseupHeader,this);
this.__nG.addListener(bl,this._onClickHeader,this);
this.__nF.add(this.__nG,{flex:1});
this.__nH=new qx.ui.table.pane.Clipper();
this.__nH.add(this.__nE);
this.__nH.addListener(J,this._onMousewheel,this);
this.__nH.addListener(bo,this._onMousemovePane,this);
this.__nH.addListener(m,this._onMousedownPane,this);
this.__nH.addListener(k,this._onMouseupPane,this);
this.__nH.addListener(bl,this._onClickPane,this);
this.__nH.addListener(z,this._onContextMenu,this);
this.__nH.addListener(D,this._onDblclickPane,this);
this.__nH.addListener(F,this._onResizePane,this);
if(qx.core.Environment.get(V)){this.__nI=new qx.ui.container.Composite();
this.__nI.setLayout(new qx.ui.layout.Canvas());
this.__nI.add(this.__nH,{edge:0});
this._add(this.__nI,{row:1,column:0});
}else{this._add(this.__nH,{row:1,column:0});
}this.__nJ=this._showChildControl(bj);
this.__nK=this._showChildControl(d);
this.__nL=this.getChildControl(h);
...
this.__jj.addListener(U,this._oninterval,this);
this.initScrollTimeout();
},statics:{MIN_COLUMN_WIDTH:10,RESIZE_REGION_RADIUS:5,CLICK_TOLERANCE:5,HORIZONTAL_SCROLLBAR:1,VERTICAL_SCROLLBAR:2},events:{"changeScrollY":l,"changeScrollX":l,"cellClick":j,"cellDblclick":j,"cellContextmenu":j,"beforeSort":l},properties:{horizontalScrollBarVisible:{check:o,init:false,apply:X,event:t},verticalScrollBarVisible:{check:o,init:false,apply:R,event:c},tablePaneModel:{check:K,apply:q,event:A},liveResize:{check:o,init:false},focusCellOnMouseMove:{check:o,init:false},selectBeforeFocus:{check:o,init:false},showCellFocusIndicator:{check:o,init:true,apply:bd},contextMenuFromDataCellsOnly:{check:o,init:true},resetSelectionOnHeaderClick:{check:o,init:true},scrollTimeout:{check:C,init:100,apply:s},appearance:{refine:true,init:E}},members:{__lI:null,__kL:null,__nM:null,__nN:null,__nO:null,__nP:null,__nQ:null,__nR:null,__nS:null,__nT:null,__nU:null,__nV:null,__nW:null,__nX:null,__nY:false,__oa:null,__ob:null,__oc:null,__lE:null,__lF:null,__od:null,__oe:null,__of:null,__nJ:null,__nK:null,__kI:null,__nG:null,__nE:null,__nH:null,__nI:null,__nL:null,__nF:null,__jj:null,getPaneInsetRight:function(){var bv=this.getTopRightWidget();
...
this.__nH.add(bA);
break;
case bj:bA=this._createScrollBar(bc).set({minWidth:0,alignY:r});
bA.addListener(i,this._onScrollX,this);

if(this.__nI!=null){bA.setMinHeight(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);
this.__nI.add(bA,{bottom:0,right:0,left:0});
}else{this._add(bA,{row:2,column:0});
}break;
case d:bA=this._createScrollBar(bg);
bA.addListener(i,this._onScrollY,this);

if(this.__nI!=null){bA.setMinWidth(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);
this.__nI.add(bA,{right:0,bottom:0,top:0});
}else{this._add(bA,{row:1,column:1});
}break;
}return bA||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,by);
},_applyHorizontalScrollBarVisible:function(bB,bC){this.__nJ.setVisibility(bB?bp:g);
},_applyVerticalScrollBarVisible:function(bD,bE){this.__nK.setVisibility(bD?bp:g);
...
},_applyShowCellFocusIndicator:function(bH,bI){if(bH){this.__nL.setDecorator(bq);
this._updateFocusIndicator();
}else{if(this.__nL){this.__nL.setDecorator(null);
}}},getScrollY:function(){return this.__nK.getPosition();
},setScrollY:function(scrollY,bJ){this.__nK.scrollTo(scrollY);

if(bJ){this._updateContent();
}},getScrollX:function(){return this.__nJ.getPosition();
},setScrollX:function(scrollX){this.__nJ.scrollTo(scrollX);
},getTable:function(){return this.__kL;
},onColVisibilityChanged:function(){this.updateHorScrollBarMaximum();
this._updateFocusIndicator();
},setColumnWidth:function(bK,bL){this.__kI.setColumnWidth(bK,bL);
this.__nE.setColumnWidth(bK,bL);
var bM=this.getTablePaneModel();
var x=bM.getX(bK);

if(x!=-1){this.updateHorScrollBarMaximum();
this._updateFocusIndicator();
}},onColOrderChanged:function(){this.__kI.onColOrderChanged();
this.__nE.onColOrderChanged();
this.updateHorScrollBarMaximum();
},onTableModelDataChanged:function(bN,bO,bP,bQ){this.__nE.onTableModelDataChanged(bN,bO,bP,bQ);
var bR=this.getTable().getTableModel().getRowCount();

if(bR!=this.__lI){this.updateVerScrollBarMaximum();

if(this.getFocusedRow()>=bR){if(bR==0){this.setFocusedCell(null,null);
}else{this.setFocusedCell(this.getFocusedColumn(),bR-1);
}}this.__lI=bR;
}},onSelectionChanged:function(){this.__nE.onSelectionChanged();
},onFocusChanged:function(){this.__nE.onFocusChanged();
},onTableModelMetaDataChanged:function(){this.__kI.onTableModelMetaDataChanged();
this.__nE.onTableModelMetaDataChanged();
},_onPaneModelChanged:function(){this.__kI.onPaneModelChanged();
this.__nE.onPaneModelChanged();
},_onResizePane:function(){this.updateHorScrollBarMaximum();
this.updateVerScrollBarMaximum();
this._updateContent();
this.__kI._updateContent();
this.__kL._updateScrollBarVisibility();
},updateHorScrollBarMaximum:function(){var bV=this.__nH.getInnerSize();
...
bU.setKnobFactor(1);
bU.setPosition(0);
}},updateVerScrollBarMaximum:function(){var cf=this.__nH.getInnerSize();
...
this.__nG.scrollToX(cg);
this.__nH.scrollToX(cg);
...
}var ck=e.getWheelDelta(ba);
if(ck>0&&ck<1){ck=1;
}else if(ck<0&&ck>-1){ck=-1;
}this.__nK.scrollBySteps(ck);
ck=e.getWheelDelta(B);
if(ck>0&&ck<1){ck=1;
}else if(ck<0&&ck>-1){ck=-1;
}this.__nJ.scrollBySteps(ck);
if(this.__ob&&this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(this.__ob,this.__oc);
}var cj=this.__nK.getPosition();
var ci=this.__nK.getMaximum();
if(ck<0&&cj<=0||ck>0&&cj>=ci){return;
}e.stop();
},__og:function(cl){var cq=this.getTable();
var cr=this.__kI.getHeaderWidgetAtColumn(this.__nU);
var cm=cr.getSizeHint().minWidth;
var co=Math.max(cm,this.__nW+cl-this.__nV);

if(this.getLiveResize()){var cn=cq.getTableColumnModel();
cn.setColumnWidth(this.__nU,co,true);
}else{this.__kI.setColumnWidth(this.__nU,co,true);
var cp=this.getTablePaneModel();
this._showResizeLine(cp.getColumnLeft(this.__nU)+co);
}this.__nV+=co-this.__nW;
this.__nW=co;
},__oh:function(cs){var ct=qx.ui.table.pane.Scroller.CLICK_TOLERANCE;

if(this.__kI.isShowingColumnMoveFeedback()||cs>this.__nT+ct||cs<this.__nT-ct){this.__nQ+=cs-this.__nT;
this.__kI.showColumnMoveFeedback(this.__nP,this.__nQ);
var cu=this.__kL.getTablePaneScrollerAtPageX(cs);

if(this.__nS&&this.__nS!=cu){this.__nS.hideColumnMoveFeedback();
}
if(cu!=null){this.__nR=cu.showColumnMoveFeedback(cs);
}else{this.__nR=null;
}this.__nS=cu;
this.__nT=cs;
...
if(this.__nU!=null){this.__og(cz);
cC=true;
e.stopPropagation();
}else if(this.__nP!=null){this.__oh(cz);
e.stopPropagation();
...
this.setCursor(cx);
this.__kI.setMouseOverColumn(cv);
...
}}this.__kI.setMouseOverColumn(null);
...
this.__nU=cL;
this.__nV=cM;
this.__nW=cN.getColumnWidth(this.__nU);
this.__nG.capture();
},_startMoveHeader:function(cO,cP){this.__nP=cO;
this.__nT=cP;
this.__nQ=this.getTablePaneModel().getColumnLeft(cO);
this.__nG.capture();
...
if(cV!==null){this.__nX={row:cV,col:cU};
this.__nY=false;
...
if(!cR){cT.getSelectionManager().handleMouseDown(cV,e);
}}},_onMouseupFocusIndicator:function(e){if(this.__nX&&!this.__nY&&!this.isEditing()&&this.__nL.getRow()==this.__nX.row&&this.__nL.getColumn()==this.__nX.col){this.fireEvent(bi,qx.ui.table.pane.CellEvent,[this,e,this.__nX.row,this.__nX.col],true);
this.__nY=true;
}else if(!this.isEditing()){this._onMousedownPane(e);
}},_onChangeCaptureHeader:function(e){if(this.__nU!=null){this._stopResizeHeader();
}
if(this.__nP!=null){this._stopMoveHeader();
}},_stopResizeHeader:function(){var cW=this.getTable().getTableColumnModel();
if(!this.getLiveResize()){this._hideResizeLine();
cW.setColumnWidth(this.__nU,this.__nW,true);
}this.__nU=null;
this.__nG.releaseCapture();
this.getApplicationRoot().setGlobalCursor(null);
this.setCursor(null);
if(this.isEditing()){var cX=this.__od.getBounds().height;
this.__od.setUserBounds(0,0,this.__nW,cX);
}},_stopMoveHeader:function(){var dd=this.getTable().getTableColumnModel();
var de=this.getTablePaneModel();
this.__kI.hideColumnMoveFeedback();

if(this.__nS){this.__nS.hideColumnMoveFeedback();
}
if(this.__nR!=null){var dg=de.getFirstColumnX()+de.getX(this.__nP);
var dc=this.__nR;
...
if(this.__nU!=null){this._stopResizeHeader();
this.__oa=true;
e.stop();
}else if(this.__nP!=null){this._stopMoveHeader();
e.stop();
}},_onClickHeader:function(e){if(this.__oa){this.__oa=false;
return;
...
if(this.fireDataEvent(H,dr,null,true)){if(dp.isEditing()){dp.stopEditing();
...
if(this.__nL.isHidden()||(this.__nX&&!this.__nY&&!this.isEditing()&&du==this.__nX.row&&dv==this.__nX.col)){this.fireEvent(bi,qx.ui.table.pane.CellEvent,[this,e,du,dv],true);
this.__nY=true;
...
if(!this.getShowCellFocusIndicator()||dz===null||(this.__nX&&dz==this.__nX.row&&dA==this.__nX.col)){this.fireEvent(a,qx.ui.table.pane.CellEvent,[this,e,dz,dA],true);
...
if(dD!=-1&&dD!=null){this.fireEvent(L,qx.ui.table.pane.CellEvent,[this,e,dD],true);
}},_onMouseout:function(e){var dG=this.getTable();

if(!dG.getEnabled()){return;
}if(this.__nU==null){this.setCursor(null);
this.getApplicationRoot().setGlobalCursor(null);
}this.__kI.setMouseOverColumn(null);
if(this.getFocusCellOnMouseMove()){this.__kL.setFocusedCell();
}},_showResizeLine:function(x){var dI=this._showChildControl(n);
var dH=dI.getWidth();
var dJ=this.__nH.getBounds();
...
var dN=this.__nE.getContainerLocation().left;
...
dO=dL+1;
dM=dW-dN;
}var dQ=this.__nH.getContainerLocation().left;
var dV=this.__nH.getBounds().width;
var scrollX=dQ-dN;
...
this.__kL.setFocusedCell(ea,eb);
}},setFocusedCell:function(ec,ed){if(!this.isEditing()){this.__nE.setFocusedCell(ec,ed,this.__nN);
this.__lE=ec;
this.__lF=ed;
this._updateFocusIndicator();
}},getFocusedColumn:function(){return this.__lE;
...
if(eg!=-1){var em=this.__nH.getInnerSize();
...
}},isEditing:function(){return this.__od!=null;
},startEditing:function(){var ew=this.getTable();
var eu=ew.getTableModel();
var ey=this.__lE;

if(!this.isEditing()&&(ey!=null)&&eu.isColumnEditable(ey)){var ez=this.__lF;
var es=this.getTablePaneModel().getX(ey);
var et=eu.getValue(ey,ez);
this.scrollCellVisible(es,ez);
this.__oe=ew.getTableColumnModel().getCellEditorFactory(ey);
var ev={col:ey,row:ez,xPos:es,value:et,table:ew};
this.__od=this.__oe.createCellEditor(ev);
if(this.__od===null){return false;
}else if(this.__od instanceof qx.ui.window.Window){this.__od.setModal(true);
this.__od.setShowClose(false);
this.__od.addListener(bb,this._onCellEditorModalWindowClose,this);
var f=ew.getModalCellEditorPreOpenFunction();

if(f!=null){f(this.__od,ev);
}this.__od.open();
}else{var ex=this.__nL.getInnerSize();
this.__od.setUserBounds(0,0,ex.width,ex.height);
this.__nL.addListener(m,function(e){this.__nX={row:this.__lF,col:this.__lE};
e.stopPropagation();
},this);
this.__nL.add(this.__od);
this.__nL.addState(bm);
this.__nL.setKeepActive(false);
this.__nL.setDecorator(bq);
this.__od.focus();
this.__od.activate();
}return true;
}return false;
},stopEditing:function(){if(!this.getShowCellFocusIndicator()){this.__nL.setDecorator(null);
}this.flushEditor();
this.cancelEditing();
},flushEditor:function(){if(this.isEditing()){var eB=this.__oe.getCellEditorValue(this.__od);
var eA=this.getTable().getTableModel().getValue(this.__lE,this.__lF);
this.getTable().getTableModel().setValue(this.__lE,this.__lF,eB);
this.__kL.focus();
this.__kL.fireDataEvent(M,{row:this.__lF,col:this.__lE,oldValue:eA,value:eB});
}},cancelEditing:function(){if(this.isEditing()&&!this.__od.pendingDispose){if(this._cellEditorIsModalWindow){this.__od.destroy();
this.__od=null;
this.__oe=null;
this.__od.pendingDispose=true;
}else{this.__nL.removeState(bm);
this.__nL.setKeepActive(true);
this.__od.destroy();
this.__od=null;
this.__oe=null;
...
var eI=this.__nE.getContentLocation().left;
...
var eQ=this.__kI.getContainerLocation().left;
...
},_getRowForPagePos:function(eR,eS){var eT=this.__nE.getContentLocation();

if(eR<eT.left||eR>eT.right){return null;
}
if(eS>=eT.top&&eS<=eT.bottom){var eU=this.getTable().getRowHeight();
var scrollY=this.__nK.getPosition();
...
}var eW=this.__kI.getContainerLocation();

if(eS>=eW.top&&eS<=eW.bottom&&eR<=eW.right){return -1;
}return null;
},setTopRightWidget:function(fb){var fc=this.__of;

if(fc!=null){this.__nF.remove(fc);
}
if(fb!=null){this.__nF.add(fb);
}this.__of=fb;
},getTopRightWidget:function(){return this.__of;
},getHeader:function(){return this.__kI;
},getTablePane:function(){return this.__nE;
},getVerticalScrollBarWidth:function(){var fd=this.__nK;
return fd.isVisible()?(fd.getSizeHint().width||0):0;
},getNeededScrollBars:function(fe,ff){var fo=this.__nK;
...
},getPaneClipper:function(){return this.__nH;
...
},_oninterval:qx.event.GlobalError.observeMethod(function(){if(this.__nN&&!this.__nE._layoutPending){this.__nN=false;
this._updateContent();
}}),_updateContent:function(){var fC=this.__nH.getInnerSize();

if(!fC){return;
}var fF=fC.height;
var scrollX=this.__nJ.getPosition();
var scrollY=this.__nK.getPosition();
var fz=this.getTable().getRowHeight();
var fA=Math.floor(scrollY/fz);
var fE=this.__nE.getFirstVisibleRow();
this.__nE.setFirstVisibleRow(fA);
...
if(!fD){fB++;
fy=scrollY%fz;
}this.__nE.setVisibleRowCount(fB);

if(fA!=fE){this._updateFocusIndicator();
}this.__nH.scrollToX(scrollX);
if(!fD){this.__nH.scrollToY(fy);
}},_updateFocusIndicator:function(){var fG=this.getTable();

if(!fG.getEnabled()){return;
}this.__nL.moveToCell(this.__lE,this.__lF);
}},destruct:function(){this._stopInterval();
var fH=this.getTablePaneModel();

if(fH){fH.dispose();
}this.__nX=this.__of=this.__kL=null;
this._disposeObjects(Y,u,O,b,G,W,N,v,I,Q);
}});
})();
...
},properties:{visibility:{refine:true,init:b},row:{check:g,nullable:true},column:{check:g,nullable:true}},members:{__oi:null,_onKeyPress:function(e){var i=e.getKeyIdentifier();

if(i!==f&&i!==c){e.stopPropagation();
}},moveToCell:function(j,k){if(!this.__oi.getShowCellFocusIndicator()&&!this.__oi.getTable().getTableModel().isColumnEditable(j)){this.exclude();
return;
}else{this.show();
}
if(j==null){this.hide();
this.setRow(null);
this.setColumn(null);
}else{var l=this.__oi.getTablePaneModel().getX(j);

if(l==-1){this.hide();
this.setRow(null);
this.setColumn(null);
}else{var q=this.__oi.getTable();
var o=q.getTableColumnModel();
var p=this.__oi.getTablePaneModel();
var n=this.__oi.getTablePane().getFirstVisibleRow();
...
(function(){var l="move",k="Boolean",j="mouseup",i="mousedown",h="losecapture",g="qx.ui.core.MMovable",f="__ok",d="mousemove",c="maximized",b="__ol",a="move-frame";
qx.Mixin.define(g,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__ok:null,__ol:null,__om:null,__on:null,__oo:null,__op:null,__oq:null,__or:false,__os:null,__ot:0,_activateMoveHandle:function(m){if(this.__ok){throw new Error("The move handle could not be redefined!");
}this.__ok=m;
...
m.addListener(h,this.__ox,this);
},__ou:function(){var n=this.__ol;

if(!n){n=this.__ol=new qx.ui.core.Widget();
n.setAppearance(a);
n.exclude();
qx.core.Init.getApplication().getRoot().add(n);
}return n;
},__ov:function(){var location=this.getContainerLocation();
var p=this.getBounds();
var o=this.__ou();
o.setUserBounds(location.left,location.top,p.width,p.height);
o.show();
o.setZIndex(this.getZIndex()+1);
},__ow:function(e){var r=this.__om;
var u=Math.max(r.left,Math.min(r.right,e.getDocumentLeft()));
var t=Math.max(r.top,Math.min(r.bottom,e.getDocumentTop()));
var q=this.__on+u;
var s=this.__oo+t;
return {viewportLeft:q,viewportTop:s,parentLeft:q-this.__op,parentTop:s-this.__oq};
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__os=parent.getBlockerColor();
this.__ot=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__or=true;
}}this.__om={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__op=w.left;
this.__oq=w.top;
this.__on=v.left-e.getDocumentLeft();
this.__oo=v.top-e.getDocumentTop();
this.addState(l);
this.__ok.capture();
if(this.getUseMoveFrame()){this.__ov();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__ow(e);

if(this.getUseMoveFrame()){this.__ou().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__or){parent.unblockContent();
parent.setBlockerColor(this.__os);
parent.setBlockerOpacity(this.__ot);
this.__os=null;
this.__ot=0;
this.__or=false;
}}this.__ok.releaseCapture();
var z=this.__ow(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__ou().exclude();
}e.stopPropagation();
},__ox:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__ou().exclude();
...
qx.Mixin.define(k,{properties:{contentPaddingTop:{check:p,init:0,apply:o,themeable:true},contentPaddingRight:{check:p,init:0,apply:o,themeable:true},contentPaddingBottom:{check:p,init:0,apply:o,themeable:true},contentPaddingLeft:{check:p,init:0,apply:o,themeable:true},contentPadding:{group:[e,a,d,c],mode:b,themeable:true}},members:{__oy:{contentPaddingTop:i,contentPaddingRight:h,contentPaddingBottom:m,contentPaddingLeft:f},__oz:{contentPaddingTop:l,contentPaddingRight:n,contentPaddingBottom:g,contentPaddingLeft:j},_applyContentPadding:function(q,r,name){var s=this._getContentPaddingTarget();

if(q==null){var t=this.__oz[name];
s[t]();
}else{var u=this.__oy[name];
s[u](q);
}}}});
})();
...
},events:{"modelChanged":e},statics:{EVENT_TYPE_MODEL_CHANGED:"modelChanged"},properties:{firstColumnX:{check:c,init:0,apply:d},maxColumnCount:{check:f,init:-1,apply:a}},members:{__oU:null,__oV:null,_applyFirstColumnX:function(j,k){this.__oU=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},_applyMaxColumnCount:function(l,m){this.__oU=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},setTableColumnModel:function(n){if(this.__oV){this.__oV.removeListener(g,this._onColVisibilityChanged,this);
this.__oV.removeListener(h,this._onColVisibilityChanged,this);
}this.__oV=n;
this.__oV.addListener(g,this._onColVisibilityChanged,this);
this.__oV.addListener(h,this._onHeaderCellRendererChanged,this);
this.__oU=null;
},_onColVisibilityChanged:function(o){this.__oU=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},_onHeaderCellRendererChanged:function(p){this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},getColumnCount:function(){if(this.__oU==null){var q=this.getFirstColumnX();
var s=this.getMaxColumnCount();
var r=this.__oV.getVisibleColumnCount();

if(s==-1||(q+s)>r){this.__oU=r-q;
}else{this.__oU=s;
}}return this.__oU;
},getColumnAtX:function(t){var u=this.getFirstColumnX();
return this.__oV.getVisibleColumnAtX(u+t);
},getX:function(v){var w=this.getFirstColumnX();
var y=this.getMaxColumnCount();
var x=this.__oV.getVisibleX(v)-w;
...
if(A==z){return C;
}C+=this.__oV.getColumnWidth(A);
}return -1;
},getTotalWidth:function(){var D=0;
var E=this.getColumnCount();

for(var x=0;x<E;x++){var F=this.getColumnAtX(x);
D+=this.__oV.getColumnWidth(F);
}return D;
}},destruct:function(){if(this.__oV){this.__oV.removeListener(g,this._onColVisibilityChanged,this);
this.__oV.removeListener(h,this._onColVisibilityChanged,this);
}this.__oV=null;
}});
})();
...
this.__oX=[];
this.__oY={};
},members:{__oW:null,__oX:null,__oY:null,__lJ:null,init:function(f){},getRowCount:function(){throw new Error("getRowCount is abstract");
...
},getColumnCount:function(){return this.__oW.length;
},getColumnIndexById:function(y){return this.__oY[y];
},getColumnId:function(z){return this.__oW[z];
},getColumnName:function(A){return this.__oX[A];
},setColumnIds:function(B){this.__oW=B;
this.__oY={};

for(var i=0;i<B.length;i++){this.__oY[B[i]]=i;
}this.__oX=new Array(B.length);
if(!this.__lJ){this.fireEvent(e);
}},setColumnNamesByIndex:function(C){if(this.__oW.length!=C.length){throw new Error("this.__columnIdArr and columnNameArr have different length: "+this.__oW.length+" != "+C.length);
}this.__oX=C;
this.fireEvent(e);
},setColumnNamesById:function(D){this.__oX=new Array(this.__oW.length);

for(var i=0;i<this.__oW.length;++i){this.__oX[i]=D[this.__oW[i]];
}},setColumns:function(E,F){var G=this.__oW.length==0||F;

if(F==null){if(this.__oW.length==0){F=E;
}else{F=this.__oW;
}}
...
}},destruct:function(){this.__oW=this.__oX=this.__oY=null;
}});
})();
...
}},members:{__pa:null,__pd:null,__pe:null,__pc:null,__pb:null,__pf:null,getRowData:function(B){var C=this.__pa[B];

if(C==null||C.originalData==null){return C;
}else{return C.originalData;
}},getRowDataAsMap:function(D){var F=this.__pa[D];
...
},setEditable:function(K){this.__pd=[];

for(var L=0;L<this.getColumnCount();L++){this.__pd[L]=K;
}this.fireEvent(d);
},setColumnEditable:function(M,N){if(N!=this.isColumnEditable(M)){if(this.__pd==null){this.__pd=[];
}this.__pd[M]=N;
this.fireEvent(d);
}},isColumnEditable:function(O){return this.__pd?(this.__pd[O]==true):false;
},setColumnSortable:function(P,Q){if(Q!=this.isColumnSortable(P)){if(this.__pe==null){this.__pe=[];
}this.__pe[P]=Q;
this.fireEvent(d);
}},isColumnSortable:function(R){return (this.__pe?(this.__pe[R]!==false):true);
...
},getSortMethods:function(bd){return this.__pc[bd];
},clearSorting:function(){if(this.__pb!=-1){this.__pb=-1;
this.__pf=true;
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__pb;
},_setSortColumnIndex:function(be){this.__pb=be;
},isSortAscending:function(){return this.__pf;
},_setSortAscending:function(bf){this.__pf=bf;
},getRowCount:function(){return this.__pa.length;
},getValue:function(bg,bh){if(bh<0||bh>=this.__pa.length){throw new Error("this.__rowArr out of bounds: "+bh+" (0.."+this.__pa.length+")");
}return this.__pa[bh][bg];
},setValue:function(bi,bj,bk){if(this.__pa[bj][bi]!=bk){this.__pa[bj][bi]=bk;
if(this.hasListener(e)){var bl={firstRow:bj,lastRow:bj,firstColumn:bi,lastColumn:bi};
this.fireDataEvent(e,bl);
}
if(bi==this.__pb){this.clearSorting();
}}},setData:function(bm,bn){this.__pa=bm;
...
}},getData:function(){return this.__pa;
},setDataAsMapArray:function(bp,bq,br){this.setData(this._mapArray2RowArr(bp,bq),br);
},addRows:function(bs,bt,bu){if(bt==null){bt=this.__pa.length;
}bs.splice(0,0,bt,0);
Array.prototype.splice.apply(this.__pa,bs);
var bv={firstRow:bt,lastRow:this.__pa.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
Array.prototype.splice.apply(this.__pa,bA);
var bD={firstRow:bB,lastRow:this.__pa.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
},removeRows:function(bI,bJ,bK){this.__pa.splice(bI,bJ);
var bL={firstRow:bI,lastRow:this.__pa.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1,removeStart:bI,removeCount:bJ};
this.fireDataEvent(e,bL);
...
}},destruct:function(){this.__pa=this.__pd=this.__pc=this.__pe=null;
}});
})();
(function(){var t="id",s="Position ",r="online",q='undefined',p="Name",o="rhyacotriton.Table",n="Size",m="name",l="dataLoadCompleted",k="progress",c="dataUpdated",h="Progress",f="Purge from the table entry by real id ",b="size",a="On-line",e="dataRemoved",d="Id",g="dataRemoveFailure";
qx.Class.define(o,{extend:qx.ui.table.Table,construct:function(u){this.__kK=u;
this.__pg=new smart.model.Default;
this.__pg.setColumns([d,p,n,h,a],[t,m,b,k,r]);
qx.ui.table.Table.call(this,this.__pg);
this.__oV=this.getTableColumnModel();
this.__ph=this.__pg.getColumnIndexById(r);
this.__oV.setColumnVisible(this.__ph,false);
this.__oV.setDataCellRenderer(this.__ph,new qx.ui.table.cellrenderer.Boolean());
this.__pi=this.getSelectionModel();
this.__pi.setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
this.__pj=this.__pg.getColumnIndexById(t);
this.__pg.indexedSelection(this.__pj,this.__pi);
this.__pg.addIndex(this.__pj);
this.__kK.addListener(e,this.__pl,this);
this.__kK.addListener(g,this.__pm,this);
this.__kK.addListener(l,this.__pk,this);
this.__kK.addListener(c,this.__pn,this);
this.loadData();
},members:{__kK:null,__pj:null,__ph:null,__pg:null,__oV:null,__pi:null,logSelectedRows:function(){var v=this.__pg;
this.__pi.iterateSelection(function(w){console.log(s+w);
console.dir(v.getRowData(w));
});
},getSelectedIds:function(){var y=[];
var x=this;
this.__pi.iterateSelection(function(z){y.push(x.__pg.getValue(x.__pj,z));
});
return y;
},removeSelectedRows:function(){this.info("RemoveSelectedRows");
this.logSelectedRows();
var A=this;
this.__pi.iterateSelection(function(B){console.log(s+B);
var C=A.__pg.getRowDataAsMap(B);
A.__kK.removeElement(C);
A.__pg.removeRows(B,1);
});
},startSelectedRows:function(){this.info("startSelectedRows");
this.__kK.start(this.getSelectedIds());
},stopSelectedRows:function(){this.info("stopSelectedRows");
this.__kK.stop(this.getSelectedIds());
},loadData:function(){this.__kK.loadData();
},__pk:function(event){var D=event.getData();
...
if(typeof (J.id)==q)this.error("Cannot get Rows[i].id");

for(var j in J){if(j!=t)this.__pg.setValueById(j,J.id,J[j]);
}}},__pn:function(event){var K=event.getData();
this.particallyUpdateRows(K.data);
}}});
})();
...
qx.Mixin.define(b,{members:{testsFailed:false,__po:function(name,f){var cb=false;
...
return !cb;
},unitTest:function(){try{this.__pp();
}catch(e){this.__debug(e);
}},__pp:function(){var i,cj=false;
...
this.__po(bU,function(){for(name in cn){var ct=cn[name];
...
cr.addRows(ci);
this.__po(bc,function(){return (cr.getRowCount()==cp);
});
this.__po(bT,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__po(k,function(){for(i=0;i<N;i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
cr.setData(ci);
this.__po(bv,function(){return (cr.getRowCount()==N);
});
this.__po(Q,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__po(bM,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
this.__po(O,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(bS,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];
...
cr.addRows(ci);
cp+=10;
this.__po(a,function(){return (cr.getRowCount()==cp);
});
this.__po(n,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i)!=i-10)return false;
return true;
});
this.__po(d,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i-10)return false;
return true;
});
this.__po(L,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(r,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];
...
cr.addRows(ci);
cp+=10;
this.__po(q,function(){return (cr.getRowCount()==cp);
});
this.__po(bX,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i)!=i-10)return false;
return true;
});
this.__po(bJ,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i-10)return false;
return true;
});
this.__po(z,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(g,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];
...
cp+=N;
this.__po(bf,function(){return (cr.getRowCount()==cp);
});
this.__po(ba,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(bu,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(N>>2,N>>1);
cp-=(N>>1);
this.__po(br,function(){return (cr.getRowCount()==cp);
});
this.__po(bN,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(by,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(0,5);
cp-=5;
this.__po(X,function(){return (cr.getRowCount()==cp);
});
this.__po(bs,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(bV,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(cr.getRowCount()-5);
cp-=5;
this.__po(T,function(){return (cr.getRowCount()==cp);
});
this.__po(bl,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(y,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(0,1);
...
cp-=3;
this.__po(S,function(){return (cr.getRowCount()==cp);
});
this.__po(F,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
...
cp+=3;
this.__po(bb,function(){return (cr.getRowCount()==cp);
});
this.__po(bC,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(bC,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(c,function(){return (cr.locate(cn[bF],cl)==0);
});
this.__po(bq,function(){return (cr.locate(cn[bF],cm)==cr.getRowCount()-1);
});
cr.setValue(cn[bB],5,null);
this.__po(bL,function(){return (cr.getValue(cn[bB],5)==null);
});
this.__po(K,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(H,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
...
this.__po(p,function(){return (cr.getValue(cn[bD],i)==ce);
});
this.__po(t,function(){for(var i=0;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bD],i,1)==ce)return false;
return true;
});
this.__po(s,function(){return cr.locate(cn[bF],I,1)==undefined;
});
this.__po(bw,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(l,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(B,function(){return (cr.getRowCount()==cf);
});
this.__po(bW,function(){return (cr.getRowCount(1)==(cq-1));
});
var cf=cr.getRowCount();
var cq=cr.getRowCount(1);
var cd=bd;
...
this.__po(bI,function(){return (cr.getValue(cn[bD],i)==cd);
});
this.__po(Y,function(){for(var i=0;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bD],i,1)==cd)return true;
return false;
});
this.__po(bi,function(){return cr.locate(cn[bF],I,1)!=undefined;
});
this.__po(M,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(bg,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(G,function(){return (cr.getRowCount()==cf);
});
this.__po(bz,function(){return (cr.getRowCount(1)==(cq+1));
});
cf=cr.getRowCount();
cq=cr.getRowCount(1);
ck=0.1;
cr.setValue(cn[bF],cq>>1,ck,1);
this.__po(V,function(){return (cr.getValue(cn[bF],cr.locate(cn[bF],ck))==ck);
});
this.__po(o,function(){return (cr.getValue(cn[bF],cr.locate(cn[bF],ck,1),1)==ck);
});
this.__po(bt,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(W,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__po(bH,function(){return (cr.getRowCount()==cf);
});
this.__po(u,function(){return (cr.getRowCount(1)==cq);
});
cf=cr.getRowCount();
cq=cr.getRowCount(1);
cr.sortByColumn(cn[bD],true);
this.__po(ca,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bD],i-1)>cr.getValue(cn[bD],i))return false;
return true;
});
this.__po(bp,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bD],i-1)>cr.getValue(cn[bD],i))return false;
return true;
});
this.__po(bP,function(){return (cr.getRowCount()==cf);
});
this.__po(bo,function(){return (cr.getRowCount(1)==cq);
});
...
this.__po(bO,function(){for(i=0;i<cr.getRowCount(2);i++){var x=cr.getValue(cn[bF],i,2);

if(x>=20&&x<=80)return false;
}return true;
});
this.__po(E,function(){for(i=0;i<cr.getRowCount(3);i++){var x=cr.getValue(cn[bF],i,3);

if(x<20||x>80)return false;
}return true;
});
this.__po(bY,function(){var cu={};
...
this.__po(D,function(){return cr.getRowCount(2)+cr.getRowCount(3)==cr.getRowCount(0);
});
var cg=[];

for(i=0;i<cr.getRowCount(2);i++)cg.push(cr.getRowReference(i,2));
cr.removeReferencedRows(cg);
this.__po(bK,function(){return cr.getRowCount(2)==0;
});
this.__po(bm,function(){for(i=0;i<cr.getRowCount();i++){var x=cr.getValue(cn[bF],i);

if(x<20||x>80)return false;
}return true;
});
cr.clearAllRows();
this.__po(m,function(){for(var v=0;v<cr.getViewCount();v++)if(cr.getRowCount(v)!=0)return false;
return true;
});
cr.sortByColumn(cn[bF],true);
N=1000;
...
}this.__po(P,function(){cc.sort(function(cv,cw){return cv[cn[bF]]-cw[cn[bF]];
});

for(var i=0;i<cc.length;i++)if(cr.getValue(cn[bF],i)!=cc[i][cn[bF]])return false;
return true;
});
this.__po(h,function(){return cr.getRowCount()==co;
});
N=(1<<10);
cr.clearAllRows();
co={};
...
cr.removeRows(I,1);
}this.__po(bj,function(){for(i in co)if(co[i]!=undefined)if(cr.locate(cn[bF],i)==undefined)return false;

for(i=0;i<cr.getRowCount();i++)if(co[cr.getValue(cn[bF],i)]!=1)return false;
return true;
});
this.__po(bk,function(){for(i=0;i<cr.getRowCount(2);i++)if(co[cr.getValue(cn[bF],i,2)]!=1)return false;
return true;
});
cr.clearAllRows();
var N=100,cp=100;
...
this.__po(U,function(){return (cr.getRowCount()==cp);
});
this.__po(bA,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__po(bG,function(){for(i=0;i<N;i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
...
},properties:{view:{init:0,check:G,apply:E,event:d}},members:{__pu:null,__ps:null,__pt:null,__kL:null,init:function(H){this.__kL=H;
...
if(I>=this.__pr.length){throw new Error("_applyView: view out of bounds: "+I+" (0.."+(this.__pr.length-1)+")");
}if(M)this.__py(J);
this.__pa=this.getRowArray(I);
if(K)this.__pS(I);
},_changeView:function(e){this.__pz();
...
for(var bb in this.__pu){this.__pu[bb].push({});
}var bd=this.__pr.length;
this.__pr.push(bc);
if(this.getView()==bd){this.__py();
}this.__pN(bd);
if(this.getView()==bd){this.__pz();
}return bd;
},updateView:function(be){if(this.getView()==be){this.__py();
}this.__pN(be);
if(this.getView()==be){this.__pz();
}},resetFilters:function(bf){this.setFilters(bf);
},getViewCount:function(){return this.__pr.length;
},addIndex:function(bg){var A=[];

for(var v=0;v<this.__pr.length;v++){A.push({});
}this.__pu[bg]=A;
this._updateAssociationMaps(undefined,bg);
},locate:function(bh,bi,bj){if(bj===undefined){bj=this.getView();
}return this.__pu[bh][bj][o+bi];
},indexedSelection:function(bk,bl){this.__pw=bk;
this.__pi=bl;
this.__pv=[];
this.__px=false;
},suspendIndexedSelection:function(bm){this.__px=bm;
},__py:function(bn){if(bn===undefined){bn=this.getView();
}
if(!this.__pi||this.__pw<0||this.__pw>=this.getColumnCount()){return;
}this.__pv=[];
this.__pi.iterateSelection(function(bo){this.__pv.push(this.getValue(this.__pw,bo,bn));
},this);
},__pz:function(bp){if(this.__pv==null||!this.__pi||this.__pw<0||this.__pw>=this.getColumnCount()){return;
}var br=this.__pi;
br.setBatchMode(true);
if(!this.__px){this.__pA();
var bq=this.__pv;
for(var i=0;i<bq.length;i++){var bs=this.locate(this.__pw,bq[i],bp);
if(bs!==undefined){br.addSelectionInterval(bs,bs);
}}}br.setBatchMode(false);
},__pA:function(){var bt=this.__pi;

if(bt){bt.resetSelection();
}},getRowArray:function(bu,bv){if(bu===undefined||bu===null){bu=this.getView();
}
if(typeof bv==p){return (bv?this.__pt[bu]:this.__ps[bu]);
}else{var bw=this.__pt[bu]||this.__ps[bu];
return bw;
}},setAlternateRowArray:function(bx,by){if(bx===undefined||bx===null){bx=this.getView();
}this.__pt[bx]=(by||null);

if(this.__pt[bx]){this.__pt[bx].__pB=D;
}return this.__pt[bx];
},__pC:function(bz,A,bA){if(bA===undefined){bA=false;
}var bB=false;
if(bz==this.getView()){bB=true;
}this.__ps[bz]=A;

if(bB){this._applyView(bz,bz,true,true,bA);
}},__pD:function(bC){if(bC===undefined){bC=this.getView();
}
if(bC<this.__pr.length){return this.__pr[bC].associations;
}else{return undefined;
}},__pE:function(bD){if(bD===undefined){bD=this.getView();
}
if(bD<this.__pr.length){return this.__pr[bD].fFilter;
}else{return undefined;
...
if(bI[bE]===undefined){this.debug("No entry in "+bI.__pB+" for row "+bE);
return null;
...
},__pF:function(bU,bV,bW){if(bW===undefined){bW=true;
}var i;
var A=this.getRowArray(bU);

if(bV.length>=(A.length>>2)){var bX={};

for(i=0;i<bV.length;i++){var ca=this.__pR(bU,bV[i]);
...
}if(!this.isSorted()&&!cr.advanced.fSort){this.__pG(cm,cn,cq);
return;
...
if(!A.length||ct(cn[0],A[A.length-1])>=0){this.__pG(cm,cn,cq);
}else if(ct(cn[cn.length-1],A[0])<=0){this.__pH(cm,cn,cq);
}else{if(cn.length<(A.length>>1)){var cx=cn.length;
var cu=0,cz=A.length-1;
var cy=[];
var i;
for(i=0;i<cx;i++){var cB=this.__pT(A,cn[i],ct,cu,cz);
...
}},__pJ:function(cH,cI,V,cJ){if(cJ===undefined){cJ=this.getView();
...
}else{if(this.__pu[cH]!==undefined){this.__pQ(cH,R[cH],V);
}R[cH]=V;
}cK=(this.isSorted()&&(cH===this.getSortColumnIndex()));
}else{for(var cL=0;cL<V.length&&cL<cM;cL++){if(this.__pu[cL]!==undefined){this.__pQ(cL,R[cL],V[cL]);
}R[cL]=V[cL];
}if(this.isSorted()&&V.length>=this.getSortColumnIndex()){cK=true;
}}this.__pK(R,cK);
this.__pz();
},__pK:function(R,cN,cO,cP){if(cO===undefined){cO=false;
}
if(cP===undefined){cP=true;
}for(var v=(cO?1:0);v<this.__pr.length;v++){var cS=this.__pR(v,R);
var cQ,cR;

if(v==0){cQ=false;
cR=false;
}else{cQ=(this.__pR(v,R)===undefined);
cR=this.__pM(v,R);
}if(cN){if(!cQ){this.__pF(v,[R]);
}if(!cR){this.__pI(v,[R],false);
}}else{if(cQ!=cR){if(cR){this.__pF(v,[R]);
}else{this.__pI(v,[R],false);
}}}if(cP){this.__pS();
}}},setValue:function(cT,cU,cV,cW){this.__pJ(cT,cU,cV,cW);
},setValueById:function(cX,cY,da,db){return this.setValue(this.getColumnIndexById(cX),cY,da,db);
},setRow:function(dc,dd,de){this.__pJ(-1,dc,dd,de);
...
A.sort(dp);
for(var v=0;v<this.__pr.length;v++){this.setAlternateRowArray(v,null);
this.__pI(v,A,true,true);
}this.__pz();
...
for(var v=0;v<this.__pr.length;v++){var dy=this.__pr[v];
...
if(dx){this.__pS();
}},clearAllRows:function(){if(this.getRowCount()>0){this.__pA();

for(var v=0;v<this.__pr.length;v++){this.__ps[v]=[];
this.__ps[v].__pB=y;
this.__pt[v]=null;
}this._updateAssociationMaps();
this.__pS();
}},forceRedraw:function(){this.__pS();
},getRowId:function(dz){return dz.__cq;
},getRowById:function(dA,dB){return this.getRowArray(dA,false)[this.__pr[dA].associations[dB]];
},__pL:function(dC,R,dD){if(dD===undefined){dD=true;
}var dE=this.__pE(dC);
...
},__pM:function(dI,R){return !this.__pL(dI,R,true);
},__pN:function(dJ,dK,dL){if(dJ==0){return;
}
if(dK===undefined){dK=true;
}
if(dL===undefined){dL=true;
}var U=this.getRowArray(0);
this.__pC(dJ,this.__pL(dJ,U,false));
if(dL){this._updateAssociationMaps(dJ);
}if(dK&&this.getView()==dJ){this.__pS(dJ);
}},__pO:function(dM,dN){for(var v=1;v<this.__pr.length;v++){this.__pN(v,dM,dN);
}},__pP:1,assignRowIDs:function(A){for(var i=0;i<A.length;i++){A[i].__cq=this.__pP++;
}},_updateAssociationMaps:function(dO,dP){for(var v=0;v<this.__pr.length;v++){if(dO!==undefined&&dO!=v){continue;
}var A=this.getRowArray(v);
if(dP===undefined){this.__pr[v].associations={};
}for(var dQ in this.__pu){if(dP===undefined||dP==dQ){this.__pu[dQ][v]={};
}}for(var j=0;j<A.length;j++){var R=A[j];

if(dP===undefined){this.__pr[v].associations[R.__cq]=j;
}for(dQ in this.__pu){if(dP===undefined||dP==dQ){this.__pu[dQ][v][o+R[dQ]]=j;
}}}}},_updateAssociationMapsAfterPush:function(dR,dS,dT){var dV=this.__pD(dR);
var dW=dT;
for(var i=0;i<dS.length;i++,dW++){var R=dS[i];
dV[R.__cq]=dW;
for(var dU in this.__pu){this.__pu[dU][dR][o+R[dU]]=dW;
}}},__pQ:function(dX,dY,ea){dY=o+dY;
ea=o+ea;

for(var ed=0;ed<this.__pr.length;ed++){for(var eb in this.__pu){var ec=this.__pu[eb][ed][dY];
this.__pu[eb][ed][dY]=undefined;
this.__pu[eb][ed][ea]=ec;
}}},__pR:function(ee,R){try{if(R.__cq===undefined){return undefined;
}}catch(e){this.__pU(e);
return undefined;
}return this.__pD(ee)[R.__cq];
...
}for(var v=0;v<this.__pr.length;v++){this.getRowArray(v).reverse();
}this._setSortAscending(el);
}else{this._setSortColumnIndex(ek);
this._setSortAscending(el);
var en=this.getComparator();
en.columnIndex=ek;
for(var v=0;v<this.__pr.length;v++){this.getRowArray(v).sort(en);
}}this._updateAssociationMaps();
this.__pz();
this.fireEvent(C);
...
}},__pS:function(eq){if(this.hasListener(k)){if(eq===undefined){eq=this.getView();
}var er={firstRow:0,lastRow:this.getRowCount(eq)-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(k,er);
}},__pT:function(A,e,es,et,eu){if(et===undefined){et=0;
}
...
},__pU:function(ex){if(typeof (qx.core.Variant)!==m){if(qx.core[l].isSet(n,c)){if(this.__pq){this.debug(ex);
}}}
if(typeof (qx.core.Environment)!==m){if(qx.core[h].get(n)){if(this.__pq){this.debug(ex);
}}}},__pV:function(ey,ez,eA){if(ez===undefined){ez=o;
}
if(eA===undefined){eA=true;
}this.__pU(ez+w+(ey?(ey+B):q));

if(ey==null||ey===undefined){this.__pU(t);
return;
}
for(var eB in ey){if(eA&&!ey.hasOwnProperty(eB)){continue;
}this.__pU(x+eB+z+ey[eB]);
...
},init:i}},members:{__pW:16,__pX:16,__pY:null,_insetY:2,_identifyImage:function(A){throw new Error("_identifyImage is abstract");
},_getImageInfos:function(B){var C=this._identifyImage(B);
if(C==null||typeof C==e){C={url:C,tooltip:null};
}if(!C.imageWidth||!C.imageHeight){var D=this.__qa(C.url);
C.imageWidth=D.width;
C.imageHeight=D.height;
}C.width=C.imageWidth;
C.height=C.imageHeight;
return C;
},__qa:function(E){var H=qx.util.ResourceManager.getInstance();
...
},createDataCellHtml:function(J,K){this.__pY=this._getImageInfos(J);
...
if(this.__pY.url){content=qx.bom.element.Decoration.create(this.__pY.url,this.getRepeat(),{width:this.__pY.width+j,height:this.__pY.height+j,display:qx.core.Environment.get(h),verticalAlign:q,position:d});
}return content;
},_getCellAttributes:function(N){var O=this.__pY.tooltip;
...
this.__qb=qx.util.AliasManager.getInstance();
this.initIconTrue();
this.initIconFalse();
},properties:{iconTrue:{check:g,init:e,apply:f},iconFalse:{check:g,init:b,apply:a}},members:{__qc:null,__qd:false,__qb:null,_applyIconTrue:function(h){this.__qc=this.__qb.resolve(h);
},_applyIconFalse:function(i){this.__qd=this.__qb.resolve(i);
...
switch(k.value){case true:l.url=this.__qc;
break;
case false:l.url=this.__qd;
break;
default:l.url=null;
break;
}return l;
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__qe:g,__qf:g,execute:function(U){this.fireDataEvent(L,U);
},__qg:function(event){if(this.getEnabled()&&this.__qh(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__fR:function(event){if(this.getEnabled()&&this.__qh(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__qg,this);
qx.event.Registration.addListener(document.documentElement,d,this.__fR,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__qg,this);
...
this.error(bc);
throw new Error(bc);
}this.__qe={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__qf=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__qj(a[i]);

switch(bd){case f:case u:case K:case C:this.__qe[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__qf){var bc=r;
this.error(bc);
throw bc;
}this.__qf=bd;
}}}return true;
},__qh:function(e){var be=this.__qf;
...
},__qi:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__qj:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__qi[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__qf;
var bj=[];

for(var bi in this.__qe){if(this.__qe[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__qe=this.__qf=null;
}});
})();
...
}},statics:{IS_SUPPORTED:qx.bom.element.Style.isPropertySupported("borderImage")},properties:{borderImage:{check:t,nullable:true,apply:j},sliceTop:{check:h,init:0,apply:j},sliceRight:{check:h,init:0,apply:j},sliceBottom:{check:h,init:0,apply:j},sliceLeft:{check:h,init:0,apply:j},slice:{group:[n,v,a,w],mode:c},repeatX:{check:[i,e,d],init:i,apply:j},repeatY:{check:[i,e,d],init:i,apply:j},repeat:{group:[u,m],mode:c}},members:{__qm:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
...
this.__qm=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__qm;
...
}},members:{__qm:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__qm;
},getMarkup:function(e){if(this.__qm){return this.__qm;
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__qm:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__qm;
},_applyStyle:function(){},getMarkup:function(){if(this.__qm){return this.__qm;
...
qx.Mixin.define(o,{include:[qx.ui.decoration.MSingleBorder,qx.ui.decoration.MBackgroundImage],construct:function(){this._getDefaultInsetsForBorder=this.__qr;
this._resizeBorder=this.__qq;
this._styleBorder=this.__qo;
this._generateMarkup=this.__qp;
},properties:{innerWidthTop:{check:f,init:0},innerWidthRight:{check:f,init:0},innerWidthBottom:{check:f,init:0},innerWidthLeft:{check:f,init:0},innerWidth:{group:[w,z,n,k],mode:a},innerColorTop:{nullable:true,check:g},innerColorRight:{nullable:true,check:g},innerColorBottom:{nullable:true,check:g},innerColorLeft:{nullable:true,check:g},innerColor:{group:[u,v,x,r],mode:a}},members:{__qn:null,__qo:function(D){{var F=qx.theme.manager.Color.getInstance();
...
}},__qp:function(K){var O=this._generateBackgroundMarkup(K);
...
K[y]=0;
K[p]=0;
return this.__qn=this._generateBackgroundMarkup(K,O);
},__qq:function(S,T,U){var V=this.getInsets();
T-=V.left+V.right;
U-=V.top+V.bottom;
...
},__qr:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
