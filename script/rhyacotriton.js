...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:rhyacotriton.4d65d9fe2506.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var r="icon/22/actions/view-refresh.png",q="icon/22/actions/list-remove.png",p="removeSelectedRows",o="Add",n="Start",m="Reconnect",l="reconnect",k="startSelectedRows",j="icon/22/actions/media-playback-pause.png",i="addTorrent",c="icon/22/actions/list-add.png",h="reload",f="stopSelectedRows",b="icon/22/actions/media-playback-start.png",a="Pause",e="Refresh",d="Remove",g="rhyacotriton.ToolBar";
qx.Class.define(g,{extend:qx.ui.toolbar.ToolBar,construct:function(s){qx.ui.toolbar.ToolBar.call(this);
this.__ly=new qx.ui.toolbar.Button(this.tr(m),r);
this.__ly.setCommand(s.getCommand(l));
this.add(this.__ly);
this.__lz=new qx.ui.toolbar.Button(this.tr(e),r);
this.__lz.setCommand(s.getCommand(h));
this.add(this.__lz);
this.__lA=new qx.ui.toolbar.Button(this.tr(o),c);
this.__lA.setCommand(s.getCommand(i));
this.add(this.__lA);
this.__lB=new qx.ui.toolbar.Button(this.tr(d),q);
this.__lB.setCommand(s.getCommand(p));
this.add(this.__lB);
this.__lC=new qx.ui.toolbar.Button(this.tr(n),b);
this.__lC.setCommand(s.getCommand(k));
this.add(this.__lC);
this.__lD=new qx.ui.toolbar.Button(this.tr(a),j);
this.__lD.setCommand(s.getCommand(f));
this.add(this.__lD);
this.enableRowButtons(false);
},members:{__lE:true,__lF:function(t){[this.__lB,this.__lC,this.__lD].map(function(x){x.setEnabled(t);
});
},enableRowButtons:function(u){this.__lE=u;
this.__lF(u);
},setEnabled:function(v){[this.__lA,this.__lz].map(function(x){x.setEnabled(v);
});

if(v==this.__lE)this.__lF(v);
}}});
})();
...
}},events:{"stateChanged":b,"dataUpdated":c,"dataRemoved":c,"dataRemoveFailure":c,"dataLoadCompleted":c}});
})();
(function(){var o="get_entire_torrent_list",n='/',m="stateChanged",l='WebSocket: ',k="rhyacotriton.store.Remote",j='WebSocket: opened',i='http:',h='ws:',g="dataRemoveFailure",f='WebSocket: closed',b='https:',d='wss:',c="stream",a='WebSocket: heartbeat';
qx.Class.define(k,{extend:rhyacotriton.store.Abstract,construct:function(){rhyacotriton.store.Abstract.call(this);
var p=document.location.href;
p=p.replace(i,h).replace(b,d);
...
y.onclose=function(){console.log(f);
x.setActive(false);
x.fireEvent(m);
};
...
(function(){var l="id",k="total",h='undefined',g="progress",f="online",e="left",d="Position ",c="dataUpdated",b="Left",a="leechers",B="%",A="Total",z="Seeders",y="Id",x="dataLoadCompleted",w="Leechers",v="name",u="Is the table empty?",t="Progress",s="Purge from the table entry by real id ",q="seeders",r="Name",o="rhyacotriton.Table",p="dataRemoved",m="dataRemoveFailure",n="On-line";
qx.Class.define(o,{extend:qx.ui.table.Table,construct:function(C){this.__kL=C;
this.__pp=new smart.model.Default;
this.__pp.setColumns([y,r,A,t,b,n,w,z],[l,v,k,g,e,f,a,q]);
qx.ui.table.Table.call(this,this.__pp);
this.__pf=this.getTableColumnModel();
this.__pq=this.__pp.getColumnIndexById(f);
this.__pr=this.__pp.getColumnIndexById(e);
this.__ps=this.__pp.getColumnIndexById(k);
this.__pt=this.__pp.getColumnIndexById(g);
this.__pf.setColumnVisible(this.__pq,false);
this.__pf.setColumnVisible(this.__pr,false);
var D=this;
var E=function(F){return D.__pw(F);
};
this.__pf.setDataCellRenderer(this.__pq,new qx.ui.table.cellrenderer.Boolean());
this.__pf.setDataCellRenderer(this.__pt,new rhyacotriton.cellrenderer.Progress(E));
this.__pf.setDataCellRenderer(this.__ps,new rhyacotriton.cellrenderer.Size());
this.__pf.setDataCellRenderer(this.__pr,new rhyacotriton.cellrenderer.Size());
...
this.__kL.addListener(p,this.__py,this);
this.__kL.addListener(m,this.__pz,this);
this.__kL.addListener(x,this.__px,this);
this.__kL.addListener(c,this.__pA,this);
},members:{__kL:null,__pv:null,__pq:null,__pr:null,__ps:null,__pt:null,__pp:null,__pf:null,__pu:null,__pw:function(G){var I=G[this.__ps];
var J=G[this.__pr];
var H=I-J;
return ((H/I)*100).toFixed(2)+B;
},logSelectedRows:function(){var K=this.__pp;
this.__pu.iterateSelection(function(L){console.log(d+L);
console.dir(K.getRowData(L));
});
},getSelectedIds:function(){var N=[];
var M=this;
this.__pu.iterateSelection(function(O){N.push(M.__pp.getValue(M.__pv,O));
});
return N;
},removeSelectedRows:function(){this.info("RemoveSelectedRows");
this.logSelectedRows();
var P=this;
this.__pu.iterateSelection(function(Q){console.log(d+Q);
var R=P.__pp.getRowDataAsMap(Q);
P.__kL.removeElement(R);
P.__pp.removeRows(Q,1);
});
...
},__px:function(event){var S=event.getData();

try{this.__pp.removeRows(0,undefined,undefined,false);
}catch(T){console.log(u);
}this.__pp.setDataAsMapArray(S.rows);
this.updateContent();
},__py:function(event){var U=event.getData().id;
console.log(s+U);
var V=this.__pp.locate(this.__pv,U);
if(V!=h)this.__pp.removeRows(V,1);
},__pz:function(event){var W=event.getOldData();
this.__pp.setRowsAsMapArray([W]);
},particallyUpdateRows:function(X){for(var i=0,Y=X.length;i<Y;i++){var bc=X[i];

if(typeof (bc.id)==h)this.error("Cannot get Rows[i].id");
var bb=this.__pp.locate(this.__pv,bc.id);

for(var j in bc){if(j==l)continue;
var ba=this.__pp.getColumnIndexById(j);
this.__pp.setValue(ba,bb,bc[j]);
}}},__pA:function(event){var bd=event.getData();
this.particallyUpdateRows(bd.rows);
this.updateContent();
}}});
})();
