...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:rhyacotriton.a576366e5185.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var l="execute",k="north",j="Control+S",i="Control+P",h="stateChanged",g="Control+Shift+R",f="changeSelection",d="Control+A",c="Control+R",b="rhyacotriton.Container",a="Control+D";
qx.Class.define(b,{extend:qx.ui.container.Composite,construct:function(){var m=this;
this.__kI=new qx.ui.layout.Dock();
qx.ui.container.Composite.call(this,this.__kI);
this._initializeCommands();
this.__kJ=new rhyacotriton.Header();
this.add(this.__kJ,{edge:k});
this.__kK=new rhyacotriton.ToolBar(this);
this.add(this.__kK,{edge:k});
this.__kL=new rhyacotriton.store.Remote;
this.__kL.addListener(h,function(e){var o=e.getCurrentTarget().isActive();
m.setEnabled(o);
},this.__kL);
this.__kM=new rhyacotriton.Table(this.__kL);
this.add(this.__kM);
var n=this.__kM.getSelectionModel();
n.addListener(f,function(e){var p=!(e.getCurrentTarget().isSelectionEmpty());
this.enableRowButtons(p);
},this.__kK);
this.setEnabled(false);
},members:{__kI:null,__kJ:null,__kK:null,__kL:null,__kM:null,__kN:null,removeSelectedRows:function(){this.__kM.removeSelectedRows();
},startSelectedRows:function(){this.__kM.startSelectedRows();
},stopSelectedRows:function(){this.__kM.stopSelectedRows();
},reconnect:function(){this.__kL.reconnect();
},reload:function(){this.__kL.reload();
},showAddTorrent:function(){},getCommand:function(q){return this.__kN[q];
},_initializeCommands:function(){var r={};
r.reconnect=new qx.ui.core.Command(g);
r.reconnect.addListener(l,this.reconnect,this);
r.reload=new qx.ui.core.Command(c);
r.reload.addListener(l,this.reload,this);
r.addTorrent=new qx.ui.core.Command(d);
r.addTorrent.addListener(l,this.showAddTorrent,this);
r.removeSelectedRows=new qx.ui.core.Command(a);
r.removeSelectedRows.addListener(l,this.removeSelectedRows,this);
r.startSelectedRows=new qx.ui.core.Command(j);
r.startSelectedRows.addListener(l,this.startSelectedRows,this);
r.stopSelectedRows=new qx.ui.core.Command(i);
r.stopSelectedRows.addListener(l,this.stopSelectedRows,this);
this.__kN=r;
},setEnabled:function(s){this.__kK.setEnabled(s);
this.__kM.setEnabled(s);
}}});
})();
...
}},properties:{sort:{check:[l,m,n],init:l,apply:j},separatorX:{check:p,nullable:true,apply:s},separatorY:{check:p,nullable:true,apply:s},connectSeparators:{check:e,init:false,apply:s},spacingX:{check:o,init:0,apply:s},spacingY:{check:o,init:0,apply:s}},members:{__ff:null,__kO:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kP:{north:1,south:2,west:3,east:4,center:5},__kQ:{1:q,2:d,3:r,4:g},__kR:function(){var x=this._getLayoutChildren();
var F,z;
var length=x.length;
var B=[];
var E=[];
...
},renderLayout:function(H,I){if(this._invalidChildrenCache){this.__kR();
...
}},destruct:function(){this.__kO=this.__ff=null;
}});
})();
...
}},properties:{alignX:{check:[m,k,c],init:m,apply:n},alignY:{check:[j,g,e],init:j,apply:n},spacing:{check:a,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:d,init:false,apply:f}},members:{__kS:null,__kT:null,__kU:null,__ff:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kR:function(){var w=this._getLayoutChildren();
var length=w.length;
var t=false;
var r=this.__kS&&this.__kS.length!=length&&this.__kT&&this.__kS;
var u;
var s=r?this.__kS:new Array(length);
var v=r?this.__kT:new Array(length);
...
this.__ff=w;
delete this._invalidChildrenCache;
},verifyLayoutProperty:null,renderLayout:function(x,y){if(this._invalidChildrenCache){this.__kR();
}var E=this.__ff;
var length=E.length;
...
P.push(K);
F+=K;
}if(this.__kU&&F!=x){var H={};
var L,O;

for(i=0;i<length;i+=1){L=this.__kT[i];
...
}},destruct:function(){this.__kS=this.__kT=this.__ff=null;
}});
})();
...
}else if(this.__kV.length>0){do{var M=this.__kV[0];
...
var K=false;
if(this.__kV.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__kX(M);
B+=F;
if(C&&this.__kV.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__kV.length>0);
}},__kX:function(N){N.setVisibility(w);
this.__kV.shift();
this.fireDataEvent(p,N);
},__kY:function(O){if(!O){return;
}this.__kV.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__kW.length-1;i>=0;i--){var P=this.__kW[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__kW[T]!=undefined){throw new Error("Priority already in use!");
}this.__kW[T]=S;
...
}for(var i=0;i<this.__kV.length;i++){this.__kV[i].setVisibility(w);
}this.__kV=[];
...
}},__la:false,_setAllowMenuOpenHover:function(bc){this.__la=bc;
},_isAllowMenuOpenHover:function(){return this.__la;
...
qx.Mixin.define(j,{events:{"execute":g},properties:{command:{check:b,apply:f,event:a,nullable:true}},members:{__lb:null,__lc:false,__ld:null,_bindableProperties:[d,k,l,m,h,c],execute:function(){var o=this.getCommand();

if(o){if(this.__lc){this.__lc=false;
}else{this.__lc=true;
o.execute(this);
}}this.fireEvent(n);
},__le:function(e){if(this.__lc){this.__lc=false;
return;
}this.__lc=true;
this.execute();
},_applyCommand:function(p,q){if(q!=null){q.removeListenerById(this.__ld);
}
if(p!=null){this.__ld=p.addListener(n,this.__le,this);
}var t=this.__lb;

if(t==null){this.__lb=t={};
}var u;
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="__lg",o="blur",n="Enter",m="__lf",l="__jG",d="Up",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
this.__lf.addListener(t,this._onOpenInterval,this);
this.__lg=new qx.event.Timer;
this.__lg.addListener(t,this._onCloseInterval,this);
},members:{__lh:null,__li:null,__lf:null,__lg:null,__jG:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
if(L.isVisible()){if(this.__lh){this.cancelOpen(this.__lh);
}}else if(this.__lh!=L){this.__lh=L;
this.__lf.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__li){this.cancelClose(this.__li);
}}else if(this.__li!=M){this.__li=M;
this.__lg.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__lh==N){this.__lf.stop();
this.__lh=null;
}},cancelClose:function(O){if(this.__li==O){this.__lg.stop();
this.__li=null;
...
}},__lj:{"Enter":1,"Space":1},__lk:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__lk[R]||(this.__lj[R]&&Q.getSelectedButton())){e.stopPropagation();
...
},removeAt:function(E){return this.__ll(e,E);
}}});
})();
...
},__lo:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__lp:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__lp,this);
return;
...
if(this.__ln){this.__ln.call(this);
delete this.__ln;
...
if(this.__lm){bo.cancelOpen(this.__lm);
this.__lm=null;
...
}if(this.__lm){br.cancelOpen(this.__lm);
...
if(q){this.setSeparator(q);
}},properties:{alignY:{check:[m,j,e],init:m,apply:n},alignX:{check:[k,g,a],init:k,apply:n},spacing:{check:c,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:b,init:false,apply:f}},members:{__lq:null,__kT:null,__kU:null,__ff:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kR:function(){var w=this._getLayoutChildren();
var length=w.length;
var s=false;
var r=this.__lq&&this.__lq.length!=length&&this.__kT&&this.__lq;
var u;
...
K.push(B);
Q+=B;
}if(this.__kU&&Q!=y){var H={};
var N,R;

for(i=0;i<length;i+=1){N=this.__kT[i];
...
}},destruct:function(){this.__lq=this.__kT=this.__ff=null;
}});
})();
(function(){var c="Integer",b="_applyLayoutChange",a="qx.ui.menu.Layout";
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__lr:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__lr=[0,0,0,0];
...
},getColumnSizes:function(){return this.__lr||null;
...
}},__ls:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
},events:{"execute":j,"press":j,"release":j},properties:{interval:{check:l,init:100},firstInterval:{check:l,init:500},minTimer:{check:l,init:20},timerDecrease:{check:l,init:2}},members:{__lt:null,__jj:null,press:function(){if(this.isEnabled()){if(!this.hasState(n)){this.__lu();
}this.removeState(m);
this.addState(n);
}},release:function(q){if(!this.isEnabled()){return;
}if(this.hasState(n)){if(!this.__lt){this.execute();
}}this.removeState(n);
this.removeState(m);
this.__lv();
...
if(this.hasState(n)&&!this.__lt){this.execute();
}}this.__lv();
e.stopPropagation();
},_onKeyUp:function(e){switch(e.getKeyIdentifier()){case i:case h:if(this.hasState(n)){if(!this.__lt){this.execute();
}this.removeState(n);
this.removeState(m);
e.stopPropagation();
this.__lv();
...
e.stopPropagation();
this.__lu();
}},_onInterval:function(e){this.__lt=true;
this.fireEvent(b);
},__lu:function(){this.fireEvent(g);
this.__lt=false;
...
this.removeState(m);
this.addState(n);
},__lv:function(){this.fireEvent(d);
this.__jj.stop();
this.removeState(m);
this.removeState(n);
...
},events:{"interval":b},properties:{interval:{check:e,init:100},firstInterval:{check:e,init:500},minimum:{check:e,init:20},decrease:{check:e,init:2}},members:{__jj:null,__lw:null,start:function(){this.__jj.setInterval(this.getFirstInterval());
this.__jj.start();
...
this.__jj.setInterval(this.__lw);
this.__jj.start();
this.fireEvent(d);
}},destruct:function(){this._disposeObjects(c);
}});
...
s.addListener(b,this.__lx,this);
this._add(s);
break;
...
},__lx:function(){var w=this.getChildrenContainer().getChildren();
...
(function(){var s="icon/22/actions/list-remove.png",r="removeSelectedRows",q="Add",p="Start",o="Reconnect",n="reconnect",m="icon/22/actions/view-refresh.png",l="startSelectedRows",k="icon/22/actions/media-playback-pause.png",j="addTorrent",d="icon/22/actions/list-add.png",i="reload",g="stopSelectedRows",c="icon/22/actions/media-playback-start.png",b="Pause",f="Refresh",e="Remove",h="icon/22/devices/network-wired.png",a="rhyacotriton.ToolBar";
qx.Class.define(a,{extend:qx.ui.toolbar.ToolBar,construct:function(t){qx.ui.toolbar.ToolBar.call(this);
this.__ly=new qx.ui.toolbar.Button(this.tr(o),h);
this.__ly.setCommand(t.getCommand(n));
this.add(this.__ly);
this.__lz=new qx.ui.toolbar.Button(this.tr(f),m);
this.__lz.setCommand(t.getCommand(i));
this.add(this.__lz);
this.__lA=new qx.ui.toolbar.Button(this.tr(q),d);
this.__lA.setCommand(t.getCommand(j));
this.add(this.__lA);
this.__lB=new qx.ui.toolbar.Button(this.tr(e),s);
this.__lB.setCommand(t.getCommand(r));
this.add(this.__lB);
this.__lC=new qx.ui.toolbar.Button(this.tr(p),c);
this.__lC.setCommand(t.getCommand(l));
this.add(this.__lC);
this.__lD=new qx.ui.toolbar.Button(this.tr(b),k);
this.__lD.setCommand(t.getCommand(g));
this.add(this.__lD);
this.enableRowButtons(false);
},members:{__lE:true,__lF:function(u){[this.__lB,this.__lC,this.__lD].map(function(x){x.setEnabled(u);
});
},enableRowButtons:function(v){this.__lE=v;
this.__lF(v);
},setEnabled:function(w){[this.__lA,this.__lz].map(function(x){x.setEnabled(w);
});

if(w==this.__lE)this.__lF(w);
}}});
})();
...
(function(){var c="qx.event.type.Data",b="qx.event.type.Event",a="rhyacotriton.store.Abstract";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){},members:{__lG:true,reconnect:function(){},reload:function(){},removeElement:function(d){},stop:function(e){},start:function(f){},isActive:function(){return this.__lG;
...
(function(){var o="get_entire_torrent_list",n='/',m="stateChanged",l='WebSocket: ',k="rhyacotriton.store.Remote",j='WebSocket: opened',i='http:',h='ws:',g='WebSocket: closed',f='https:',b="remove",d='wss:',c="stream",a='WebSocket: heartbeat';
qx.Class.define(k,{extend:rhyacotriton.store.Abstract,construct:function(){rhyacotriton.store.Abstract.call(this);
var p=document.location.href;
p=p.replace(i,h).replace(f,d);
p=p.substring(0,p.lastIndexOf(n))+n;
this.__lH=p+c;
this.__lJ();
},members:{__lI:null,__lH:null,sendJSON:function(q){this.__lI.send(qx.lang.Json.stringify(q));
},sendText:function(r){this.__lI.send(r);
},reconnect:function(){this.__lK();
this.__lJ();
},reload:function(){this.sendText(o);
},removeElement:function(s){var t=s.id;
var u={"id":t};
this.sendJSON({"event":b,"id":t});
},stop:function(v){},start:function(w){},__lJ:function(){var y=$.bullet(this.__lH);
var x=this;
this.__lI=y;
y.onopen=function(){console.log(j);
x.sendText(o);
x.setActive(true);
x.fireEvent(m);
};
y.onclose=function(){console.log(g);
x.setActive(false);
x.fireEvent(m);
};
y.onmessage=function(e){console.log(l+e.data);
var z=qx.lang.Json.parse(e.data);
console.dir(z);
x.fireDataEvent(z.event,z.data);
};
y.onheartbeat=function(){console.log(a);
};
},__lK:function(){this.__lI.close();
delete this.__lI;
}}});
})();
(function(){var p="Boolean",o="column-button",n="Function",m="qx.event.type.Data",k="statusbar",h="qx.ui.table.pane.CellEvent",g="function",f="PageUp",e="dataChanged",d="changeLocale",bH="changeSelection",bG="__lV",bF="appear",bE="qx.dynlocale",bD='"',bC="Enter",bB="metaDataChanged",bA="_applyStatusBarVisible",bz="columnVisibilityMenuCreateStart",by="blur",w="qx.ui.table.Table",y="columnVisibilityMenuCreateEnd",u="changeVisible",v="_applyResetSelectionOnHeaderClick",s="_applyMetaColumnCounts",t="focus",q="changeDataRowRenderer",r="changeHeaderCellHeight",F="Escape",G="A",ba="changeSelectionModel",V="Left",bi="Down",bd="Integer",bu="_applyHeaderCellHeight",bo="visibilityChanged",O="qx.ui.table.ITableModel",bx="orderChanged",bw="_applySelectionModel",bv="__lM",M="menu-button",R="menu",T="_applyAdditionalStatusBarText",X="_applyFocusCellOnMouseMove",bb="table",be="_applyColumnVisibilityButtonVisible",bk="changeTableModel",bq="qx.event.type.Event",z="tableWidthChanged",A="_applyHeaderCellsVisible",Q="Object",bh="__jj",bg="_applyShowCellFocusIndicator",bf="resize",bm="verticalScrollBarChanged",bl="changeScrollY",bc="_applyTableModel",bj="End",a="_applyKeepFirstVisibleRowComplete",bp="widthChanged",B="one of one row",C="Home",W="_applyRowHeight",b="F2",c="Up",L="%1 rows",D="qx.ui.table.selection.Model",E="one row",K="PageDown",Y="%1 of %2 rows",bs="keypress",br="changeRowHeight",S="Number",bt="__lU",N="header",bn="__lL",H="__lT",J="_applyContextMenuFromDataCellsOnly",P="qx.ui.table.IRowRenderer",U="Right",I="Space";
...
this._add(this.__lL,{flex:1});
this.setDataRowRenderer(new qx.ui.table.rowrenderer.Default(this));
this.__lM=this.getNewSelectionManager()(this);
...
}}},members:{__lN:null,__lO:null,__lL:null,__lM:null,__lQ:null,__lR:null,__lS:null,__lT:null,__lU:null,__lV:null,__lW:null,__jj:null,_createChildControlImpl:function(bS,bT){var bU;
...
},_applySelectionModel:function(bW,bX){this.__lM.setSelectionModel(bW);
...
}},getEmptyTableModel:function(){if(!this.__lV){this.__lV=new qx.ui.table.model.Simple();
this.__lV.setColumns([]);
this.__lV.setData([]);
}return this.__lV;
...
}},getTableColumnModel:function(){if(!this.__lU){var cn=this.__lU=this.getNewTableColumnModel()(this);
...
}},_applyAdditionalStatusBarText:function(cr,cs){this.__lQ=cr;
this._updateStatusBar();
...
for(var cG in qx.ui.table.Table.__lP){cB[cG]={};
...
for(cG in qx.ui.table.Table.__lP){if(!cB[cG]){break;
}
...
}},getSelectionManager:function(){return this.__lM;
},_getPaneScrollerArr:function(){return this.__lL.getChildren();
...
if(this.__lO>=dq&&this.__lO<(dq+dr)){this.setFocusedCell();
}}
for(var i=0;i<ds.length;i++){ds[i].onTableModelDataChanged(dl,dm,dn,dp);
}var dt=this.getTableModel().getRowCount();

if(dt!=this.__lR){this.__lR=dt;
this._updateScrollBarVisibility();
this._updateStatusBar();
}},_onScrollY:function(du){if(!this.__lS){this.__lS=true;
...
if(this.__lO!=dD){dA=this.startEditing();
}break;
...
default:dA=false;
break;
}}else{switch(dE){case I:this.__lM.handleSelectKeyDown(this.__lO,dw);
break;
case b:case bC:this.startEditing();
dA=true;
break;
case C:this.setFocusedCell(this.__lN,0,true);
break;
case bj:var dB=this.getTableModel().getRowCount();
this.setFocusedCell(this.__lN,dB-1,true);
break;
...
if(dD!=this.__lO&&this.getRowFocusChangeModifiesSelection()){this.__lM.handleMoveKeyDown(this.__lO,dw);
}
...
if(this.__lT!=null&&dJ.col!=null&&dJ.visible!=null){this.__lT[dJ.col].setVisible(dJ.visible);
}this._updateScrollerWidths();
this._updateScrollBarVisibility();
...
},setFocusedCell:function(dR,dS,dT){if(!this.isEditing()&&(dR!=this.__lN||dS!=this.__lO)){if(dR===null){dR=0;
}this.__lN=dR;
this.__lO=dS;
...
},getFocusedColumn:function(){return this.__lN;
},getFocusedRow:function(){return this.__lO;
...
}},moveFocusedCell:function(ea,eb){var ef=this.__lN;
var eg=this.__lO;
...
}},isEditing:function(){if(this.__lN!=null){var x=this.getTableColumnModel().getVisibleX(this.__lN);
var em=this._getMetaColumnAtColumnX(x);
return this.getPaneScroller(em).isEditing();
}return false;
},startEditing:function(){if(this.__lN!=null){var x=this.getTableColumnModel().getVisibleX(this.__lN);
var eo=this._getMetaColumnAtColumnX(x);
var en=this.getPaneScroller(eo).startEditing();
return en;
}return false;
},stopEditing:function(){if(this.__lN!=null){var x=this.getTableColumnModel().getVisibleX(this.__lN);
var ep=this._getMetaColumnAtColumnX(x);
this.getPaneScroller(ep).stopEditing();
}},cancelEditing:function(){if(this.__lN!=null){var x=this.getTableColumnModel().getVisibleX(this.__lN);
...
if(this.__lQ){if(eD){eD+=this.__lQ;
}else{eD=this.__lQ;
}}
...
if(eP){if(this.__lW==null){this.__lW=eJ[i].getVerticalScrollBarVisible();
this.__jj=qx.event.Timer.once(function(){this.__lW=null;
this.__jj=null;
},this,0);
}}eJ[i].setVerticalScrollBarVisible(eP&&eN);
if(eP&&eN!=this.__lW){this.fireDataEvent(bm,eN);
...
this._updateScrollerWidths();
this._updateScrollBarVisibility();
},addListener:function(fd,fe,self,ff){if(this.self(arguments).__lP[fd]){var fh=[fd];
...
}},removeListener:function(fi,fj,self,fk){if(this.self(arguments).__lP[fi]){for(var i=0,fl=this._getPaneScrollerArr();i<fl.length;i++){fl[i].removeListener.apply(fl[i],arguments);
}}else{qx.ui.core.Widget.prototype.removeListener.call(this,fi,fj,self,fk);
}},removeListenerById:function(fm){var fq=fm.split(bD);
var fp=fq.shift();

if(this.self(arguments).__lP[fp]){var fo=true;
...
this._disposeObjects(bv,bn,bG,bG,bt,bh);
this._disposeMap(H);
}});
})();
...
},properties:{highlightFocusRow:{check:h,init:true}},members:{_colors:null,__lY:null,__lX:null,_insetY:1,_renderFont:function(x){if(x){this.__lY=x.getStyles();
this.__lX=qx.bom.element.Style.compile(this.__lY);
this.__lX=this.__lX.replace(/"/g,d);
}else{this.__lX=j;
this.__lY=qx.bom.Font.getDefaultStyles();
}},updateDataRowElement:function(y,z){var B=this.__lY;
var A=z.style;
...
}},destruct:function(){this._colors=this.__lY=this.__lX=null;
}});
})();
...
},members:{__lT:null,__ka:null,factory:function(g,h){switch(g){case a:var j=new qx.ui.menu.Menu();
this.setMenu(j);
return j;
...
},members:{__ma:false,_applyVisible:function(h,i){if(!this.bInListener){this.setValue(h);
}}}});
})();
(function(){var b="qx.ui.table.selection.Model",a="qx.ui.table.selection.Manager";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
},properties:{selectionModel:{check:b}},members:{__mb:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();

if(!e.isSelectedIndex(c)){this._handleSelectEvent(c,d);
this.__mb=true;
}else{this.__mb=false;
}}else if(d.isRightPressed()&&d.getModifiers()==0){var e=this.getSelectionModel();

if(!e.isSelectedIndex(c)){e.setSelectionInterval(c,c);
}}},handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__mb){this._handleSelectEvent(f,g);
...
this.__mf=false;
},events:{"changeSelection":f},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:a}},members:{__mf:null,__md:null,__me:null,__mc:null,_applySelectionMode:function(o){this.resetSelection();
},setBatchMode:function(p){if(p){this.hasBatchModeRefCount+=1;
}else{if(this.hasBatchModeRefCount==0){throw new Error("Try to turn off batch mode althoug it was not turned on.");
}this.hasBatchModeRefCount-=1;

if(this.__mf){this.__mf=false;
this._fireChangeSelection();
}}return this.hasBatchMode();
},hasBatchMode:function(){return this.hasBatchModeRefCount>0;
},getAnchorSelectionIndex:function(){return this.__md;
},_setAnchorSelectionIndex:function(q){this.__md=q;
},getLeadSelectionIndex:function(){return this.__me;
},_setLeadSelectionIndex:function(r){this.__me=r;
},_getSelectedRangeArr:function(){return this.__mc;
},resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();
this._fireChangeSelection();
}},isSelectionEmpty:function(){return this.__mc.length==0;
},getSelectedCount:function(){var t=0;

for(var i=0;i<this.__mc.length;i++){var s=this.__mc[i];
t+=s.maxIndex-s.minIndex+1;
}return t;
},isSelectedIndex:function(u){for(var i=0;i<this.__mc.length;i++){var v=this.__mc[i];

if(u>=v.minIndex&&u<=v.maxIndex){return true;
}}return false;
},getSelectedRanges:function(){var w=[];

for(var i=0;i<this.__mc.length;i++){w.push({minIndex:this.__mc[i].minIndex,maxIndex:this.__mc[i].maxIndex});
}return w;
},iterateSelection:function(x,y){for(var i=0;i<this.__mc.length;i++){for(var j=this.__mc[i].minIndex;j<=this.__mc[i].maxIndex;j++){x.call(y,j);
...
break;
}},removeSelectionInterval:function(F,G){this.__md=F;
this.__me=G;
var H=Math.min(F,G);
var J=Math.max(F,G);
for(var i=0;i<this.__mc.length;i++){var L=this.__mc[i];

if(L.minIndex>J){break;
}else if(L.maxIndex>=H){var M=(L.minIndex>=H)&&(L.minIndex<=J);
var K=(L.maxIndex>=H)&&(L.maxIndex<=J);

if(M&&K){this.__mc.splice(i,1);
i--;
}else if(M){L.minIndex=J+1;
}else if(K){L.maxIndex=H-1;
}else{var I={minIndex:J+1,maxIndex:L.maxIndex};
this.__mc.splice(i+1,0,I);
L.maxIndex=H-1;
break;
}}}this._fireChangeSelection();
},_resetSelection:function(){this.__mc=[];
this.__md=-1;
this.__me=-1;
},_addSelectionInterval:function(N,O){this.__md=N;
this.__me=O;
var P=Math.min(N,O);
var R=Math.max(N,O);
var Q=0;

for(;Q<this.__mc.length;Q++){var S=this.__mc[Q];

if(S.minIndex>P){break;
}}this.__mc.splice(Q,0,{minIndex:P,maxIndex:R});
var T=this.__mc[0];

for(var i=1;i<this.__mc.length;i++){var S=this.__mc[i];

if(T.maxIndex+1>=S.minIndex){T.maxIndex=Math.max(T.maxIndex,S.maxIndex);
this.__mc.splice(i,1);
i--;
}else{T=S;
}}},_dumpRanges:function(){var U=d;

for(var i=0;i<this.__mc.length;i++){var V=this.__mc[i];
U+=k+V.minIndex+m+V.maxIndex+n;
}this.debug(U);
},_fireChangeSelection:function(){if(this.hasBatchMode()){this.__mf=true;
...
if(!J.__mg){var L=qx.theme.manager.Color.getInstance();
J.__mg=this.self(arguments);
...
}J.__mg.stylesheet=qx.bom.Stylesheet.createElement(K);
...
this.__mh=qx.locale.Manager.getInstance().bind(dn,this,dn);
}else{this.__iz=dt;
this.setLocale(dt);
}this.__mi=this.__iz;

if(ds!=null){this.__mj=ds.toString();

if(this.__mj in qx.util.format.DateFormat.ISO_MASKS){if(this.__mj===d){this.__mk=true;
}this.__mj=qx.util.format.DateFormat.ISO_MASKS[this.__mj];
}}else{this.__mj=qx.locale.Date.getDateFormat(bd,this.__iz)+N+qx.locale.Date.getDateTimeFormat(dq,dl,this.__iz);
...
if(dv._dateInstance==null||dv._dateInstance.__mj!=du){dv._dateTimeInstance=new dv();
...
},__mw:function(dX,dY,ea){var eb;

switch(dY){case 11:eb=dX-1;

if(dX!=this.__mt(new Date(eb,dY,ea))){eb=dX;
}break;
case 0:eb=dX+1;
...
},_applyLocale:function(ec,ed){this.__iz=ec===null?this.setLocale(this.__mi):ec;
},format:function(ee){if(ee==null){return null;
}
if(isNaN(ee.getTime())){return null;
}
if(this.__mk){ee=new Date(ee.getUTCFullYear(),ee.getUTCMonth(),ee.getUTCDate(),ee.getUTCHours(),ee.getUTCMinutes(),ee.getUTCSeconds(),ee.getUTCMilliseconds());
}var eh=this.__iz;
...
for(var i=0;i<this.__mn.length;i++){var en=this.__mn[i];
...
switch(eg){case x:if(er==2){ez=this.__mo(em%100,2);
...
case cl:if(er==1||er==2){ez=this.__mo(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case U:if(er==1||er==2){ez=this.__mo(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case cz:ez=this.__mo(this.__mp(ee),er);
break;
case dc:ez=this.__mo(el,er);
break;
case A:ez=this.__mo(this.__mr(ee),er);
break;
case E:ez=this.__ms(ee);
break;
...
if(er>=1&&er<=2){ez=this.__mo(eu,er);
...
case H:if(er==1||er==2){ez=this.__mo(ei+1,er);
...
case t:if(er==1||er==2){ez=this.__mo(ei+1,er);
...
break;
case ca:ez=this.__mo(es,er);
break;
case cu:ez=this.__mo((es==0)?24:es,er);
break;
case cy:ez=this.__mo(es%12,er);
break;
case bT:ez=this.__mo(((es%12)==0)?12:(es%12),er);
break;
case m:ez=this.__mo(ey,er);
break;
case bR:ez=this.__mo(eq,er);
break;
case cb:ez=ew+cG;
...
case bb:if(er>=1&&er<=4){ez=dm+((et>0)?cF:cO)+this.__mo(Math.abs(ef),2)+w+this.__mo(ev,2);
}break;
case K:if(er>=1&&er<=3){ez=((et>0)?cF:cO)+this.__mo(Math.abs(ef),2)+this.__mo(ev,2);
}else{ez=dm+((et>0)?cF:cO)+this.__mo(Math.abs(ef),2)+w+this.__mo(ev,2);
}break;
}ex+=ez;
}}return ex;
},parse:function(eA){this.__my();
var eJ=this.__ml.regex.exec(eA);

if(eJ==null){throw new Error("Date string '"+eA+"' does not match the date format: "+this.__mj);
...
for(var i=0;i<this.__ml.usedRules.length;i++){var eF=this.__ml.usedRules[i];
var eG=eJ[eD];
...
for(var k=0;k<this.__ml.usedRules.length;k++){if(this.__ml.usedRules[k].pattern==dr){eH=true;
break;
}}
if(!eH){eC=true;
}}
...
for(var k=0;k<this.__ml.usedRules.length;k++){if(this.__ml.usedRules[k].pattern.indexOf(bi)!=-1){eL=true;
break;
}}
if(!eL){eE=true;
}}eD+=(eF.groups==null)?1:eF.groups;
}
if(eC){eB.year=this.__mw(eB.weekYear,eB.month,eB.day);
}
if(eE){var eK=this.__mv(eB.dayOfYear,eB.year);
eB.month=eK.month;
eB.day=eK.day;
}
...
if(this.__mk){eI=new Date(eI.getUTCFullYear(),eI.getUTCMonth(),eI.getUTCDate(),eI.getUTCHours(),eI.getUTCMinutes(),eI.getUTCSeconds(),eI.getUTCMilliseconds());
}
...
},__mx:function(){if(this.__mn!=null){return;
}this.__mn=[];
var eQ;
var eO=0;
var eS=cG;
...
case bF:if(eR==eQ){eO++;
i++;
}else{this.__mn.push({type:bF,character:eQ,size:eO});
eQ=null;
eO=0;
eP=bI;
}break;
...
if(eP!=bI){if(eS.length>0){this.__mn.push({type:bG,text:eS});
eS=cG;
}}else{eS+=eR;
i++;
}break;
}}if(eQ!=null){this.__mn.push({type:bF,character:eQ,size:eO});
}else if(eS.length>0){this.__mn.push({type:bG,text:eS});
}},__my:function(){if(this.__ml!=null){return ;
}var eW=this.__mj;
this.__mA();
this.__mx();
var fd=[];
...
for(var eU=0;eU<this.__mn.length;eU++){var fe=this.__mn[eU];
...
for(var ff=0;ff<this.__mm.length;ff++){var fb=this.__mm[ff];

if(this.__mz(fb,eV,fa)){eX=fb;
break;
...
}this.__ml={regex:eT,"usedRules":fd,pattern:eY};
},__mz:function(fh,fi,fj){if(fi===x&&fh.pattern===dr){fh.regex=fh.regexFunc(fj);
return true;
}else if(fi===be&&fh.pattern===bw){fh.regex=fh.regexFunc(fj);
return true;
}else{return fi==fh.pattern.charAt(0)&&fj==fh.pattern.length;
}},__mA:function(){var fx=qx.util.format.DateFormat;
var fz=qx.lang.String;

if(this.__mm!=null){return ;
}var fy=this.__mm=[];
...
}},destruct:function(){if(this.__mh!=null){qx.locale.Manager.getInstance().removeBinding(this.__mh);
}this.__mn=this.__ml=this.__mm=null;
}});
})();
...
qx.Class.define(l,{statics:{__mB:qx.locale.Manager.getInstance(),getAmMarker:function(F){return this.__mB.localize(o,[],F);
},getPmMarker:function(G){return this.__mB.localize(p,[],G);
},getDayNames:function(length,H,I,J){var I=I?I:h;
var L=[B,C,D,a,e,E,d];
var M=[];

for(var i=0;i<L.length;i++){var K=c+I+k+length+k+L[i];
M.push(J?this.__mC(I,I===j?f:j,K,H):this.__mB.localize(K,[],H));
}return M;
},getDayName:function(length,N,O,P,Q){var P=P?P:h;
var S=[B,C,D,a,e,E,d];
var R=c+P+k+length+k+S[N];
return Q?this.__mC(P,P===j?f:j,R,O):this.__mB.localize(R,[],O);
},getMonthNames:function(length,T,U,V){var U=U?U:h;
var X=[];

for(var i=0;i<12;i++){var W=b+U+k+length+k+(i+1);
X.push(V?this.__mC(U,U===j?f:j,W,T):this.__mB.localize(W,[],T));
}return X;
},getMonthName:function(length,Y,ba,bb,bc){var bb=bb?bb:h;
var bd=b+bb+k+length+k+(Y+1);
return bc?this.__mC(bb,bb===j?f:j,bd,ba):this.__mB.localize(bd,[],ba);
},getDateFormat:function(be,bf){var bg=v+be;
return this.__mB.localize(bg,[],bf);
},getDateTimeFormat:function(bh,bi,bj){var bl=m+bh;
var bk=this.__mB.localize(bl,[],bj);

if(bk==bl){bk=bi;
}return bk;
},getTimeFormat:function(bm,bn){var bp=t+bm;
var bo=this.__mB.localize(bp,[],bn);
...
}else{bE=this.__mB.getTerritory()||this.__mB.getLanguage();
}return bE.toUpperCase();
},__mC:function(bF,bG,bH,bI){var bK=this.__mB.localize(bH,[],bI);

if(bK==bH){var bJ=bH.replace(g+bF+g,g+bG+g);
return this.__mB.localize(bJ,[],bI);
}else{return bK;
}}}});
})();
...
qx.Mixin.define(d,{construct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().addListener(j,this.__mD,this);
...
},__mD:qx.core.Environment.select(l,{"true":function(e){var o=this.getInvalidMessage();
...
}},"false":null})},destruct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().removeListener(j,this.__mD,this);
}}});
})();
...
this.__mE=!qx.core.Environment.get(M)||(qx.core.Environment.get(bb)==D&&parseFloat(qx.core.Environment.get(l))>=2);
...
if(bk&&this.__mE){this.__mL().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__mE){this.__mL().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__mE){if(bs){this._showPlaceholder();
...
if(this.__mE){this.__mL().setStyles(bx);
}if(bv){this.__mJ=qx.bom.Label.getTextSize(d,bx);
...
var bC=true;
this.__mF=false;
if(this.__mI&&this.__mI===bD){bC=false;
...
}if(bC){this.fireDataEvent(be,bD,this.__mI);
this.__mI=bD;
if(this.getLiveUpdate()){this.__mK(bD);
}}},_onWebFontStatusChange:function(bF){if(bF.getData().valid===true){var bG=this.__jK.getStyles();
this.__mJ=qx.bom.Label.getTextSize(d,bG);
qx.ui.core.queue.Layout.add(this);
}},__mK:function(bH){var bI=this.__mH;
this.__mH=bH;

if(bI!=bH){this.fireNonBubblingEvent(o,qx.event.type.Data,[bH,bI]);
}},setValue:function(bJ){if(bJ===null){if(this.__mF){return bJ;
}bJ=j;
this.__mF=true;
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__mL().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__mL().setStyle(bc,L);
}},__mL:function(){if(this.__mG==null){this.__mG=new qx.html.Label();
var bS=qx.theme.manager.Color.getInstance();
this.__mG.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bS.resolve(I)});
...
}},"false":null}),_applyPlaceholder:function(bT,bU){if(this.__mE){this.__mL().setValue(bT);
...
}}},destruct:function(){this.__mG=this.__jK=null;
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__mO=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__mN?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__mN=w;
qx.html.Element.prototype.setSelectable.call(this,this.__mO&&w);
...
},setWrap:function(B,C){if(this.__mM===l){this._setProperty(n,B,C);
}else{throw new Error("Text wrapping is only support by textareas!");
}return this;
},getWrap:function(){if(this.__mM===l){return this._getProperty(n);
}else{throw new Error("Text wrapping is only support by textareas!");
}}}});
})();
...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__mP:false,__mQ:null,__mH:null,__mI:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===g)){return true;
}
if(C===j&&(D===m||D===g||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&(qx.core.Environment.get(d)<9||(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)<9))){if(!E.__mR){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__mS:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(d)<9&&qx.core.Environment.get(A)<9){if(O.__mR){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__mT:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__mH){this.__mH=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__mI){this.__mI=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__mP=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__mP=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__mQ&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__mQ);
...
if(!this.__mP||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__mQ=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
}},properties:{spacingX:{check:h,init:0,apply:p},spacingY:{check:h,init:0,apply:p}},members:{__mW:null,__mU:null,__mV:null,__mX:null,__mY:null,__na:null,__nb:null,__nc:null,__nd:null,verifyLayoutProperty:null,__ne:function(){var B=[];
var A=[];
var C=[];
var w=-1;
var v=-1;
...
this.__nd=null;
delete this._invalidChildrenCache;
},_setRowData:function(G,H,I){var J=this.__mU[G];

if(!J){this.__mU[G]={};
this.__mU[G][H]=I;
}else{J[H]=I;
}},_setColumnData:function(K,L,M){var N=this.__mV[K];
...
this._applyLayoutChange();
return this;
},getColumnAlign:function(S){var T=this.__mV[S]||{};
return {vAlign:T.vAlign||q,hAlign:T.hAlign||r};
},setRowAlign:function(U,V,W){this._setRowData(U,o,V);
this._setRowData(U,m,W);
this._applyLayoutChange();
return this;
},getRowAlign:function(X){var Y=this.__mU[X]||{};
return {vAlign:Y.vAlign||q,hAlign:Y.hAlign||r};
},getCellWidget:function(ba,bb){if(this._invalidChildrenCache){this.__ne();
}var ba=this.__mW[ba]||{};
return ba[bb]||null;
},getRowCount:function(){if(this._invalidChildrenCache){this.__ne();
}return this.__na+1;
},getColumnCount:function(){if(this._invalidChildrenCache){this.__ne();
}return this.__nb+1;
},getCellAlign:function(bc,bd){var bj=q;
var bh=r;
var bi=this.__mU[bc];
var bf=this.__mV[bd];
var be=this.__mW[bc][bd];
...
for(var i=0,l=this.__mY.length;i<l;i++){var cl=this.__mY[i];
var ch=this.__nf(cl);
...
for(var i=0,l=this.__mX.length;i<l;i++){var ct=this.__mX[i];
var cv=this.__nf(ct);
...
for(var j=0;j<cy.colSpan;j++){cB=cA[cu+j]?cA[cu+j].offset:0;
cs[cu+j].minWidth+=cB;
}}}},_getRowHeights:function(){if(this.__nc!=null){return this.__nc;
}var cP=[];
var cI=this.__na;
var cH=this.__nb;
...
for(var cO=0;cO<=cH;cO++){var cG=this.__mW[cQ][cO];
...
if(this.__mY.length>0){this._fixHeightsRowSpan(cP);
}this.__nc=cP;
return cP;
},_getColWidths:function(){if(this.__nd!=null){return this.__nd;
}var cV=[];
var cS=this.__nb;
var cU=this.__na;
...
for(var dc=0;dc<=cU;dc++){var cR=this.__mW[dc][db];
...
if(this.__mX.length>0){this._fixWidthsColSpan(cV);
}this.__nd=cV;
return cV;
...
},renderLayout:function(ds,dt){if(this._invalidChildrenCache){this.__ne();
...
for(var dC=0;dC<=du;dC++){var dJ=this.__mW[dC][dP];
if(!dJ){top+=dV[dC]+dB;
continue;
...
this.__nd=null;
this.__nc=null;
},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__ne();
...
return eb;
}},destruct:function(){this.__mW=this.__mU=this.__mV=this.__mX=this.__mY=this.__nd=this.__nc=null;
}});
})();
(function(){var k="qx.event.type.Data",j="visibilityChanged",h="orderChanged",g="visibilityChangedPre",f="widthChanged",e="qx.ui.table.columnmodel.Basic",d="__nk",c="__nm",b="headerCellRendererChanged",a="__nl";
qx.Class.define(e,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__ng=[];
this.__nh=[];
},events:{"widthChanged":k,"visibilityChangedPre":k,"visibilityChanged":k,"orderChanged":k,"headerCellRendererChanged":k},statics:{DEFAULT_WIDTH:100,DEFAULT_HEADER_RENDERER:qx.ui.table.headerrenderer.Default,DEFAULT_DATA_RENDERER:qx.ui.table.cellrenderer.Default,DEFAULT_EDITOR_FACTORY:qx.ui.table.celleditor.TextField},members:{__lS:null,__ni:null,__nh:null,__ng:null,__nj:null,__nk:null,__nl:null,__nm:null,init:function(l,m){this.__nj=[];
var q=qx.ui.table.columnmodel.Basic.DEFAULT_WIDTH;
var r=this.__nk||(this.__nk=new qx.ui.table.columnmodel.Basic.DEFAULT_HEADER_RENDERER());
var o=this.__nl||(this.__nl=new qx.ui.table.columnmodel.Basic.DEFAULT_DATA_RENDERER());
var n=this.__nm||(this.__nm=new qx.ui.table.columnmodel.Basic.DEFAULT_EDITOR_FACTORY());
this.__ng=[];
this.__nh=[];
var t;
...
for(var u=0;u<l;u++){this.__nj[u]={width:q,headerRenderer:r,dataRenderer:o,editorFactory:n};
this.__ng[u]=u;
this.__nh[u]=u;
}this.__ni=null;
this.__lS=true;
...
}},getVisibleColumns:function(){return this.__nh!=null?this.__nh:[];
},setColumnWidth:function(v,w,y){var A=this.__nj[v].width;

if(A!=w){this.__nj[v].width=w;
var z={col:v,newWidth:w,oldWidth:A,isMouseAction:y||false};
this.fireDataEvent(f,z);
}},getColumnWidth:function(B){return this.__nj[B].width;
},setHeaderCellRenderer:function(C,D){var E=this.__nj[C].headerRenderer;

if(E!==this.__nk){E.dispose();
}this.__nj[C].headerRenderer=D;
this.fireDataEvent(b,{col:C});
},getHeaderCellRenderer:function(F){return this.__nj[F].headerRenderer;
},setDataCellRenderer:function(G,H){this.__nj[G].dataRenderer=H;
var I=this.__nj[G].dataRenderer;

if(I!==this.__nl){return I;
}return null;
},getDataCellRenderer:function(J){return this.__nj[J].dataRenderer;
},setCellEditorFactory:function(K,L){var M=this.__nj[K].headerRenderer;

if(M!==this.__nm){M.dispose();
}this.__nj[K].editorFactory=L;
},getCellEditorFactory:function(N){return this.__nj[N].editorFactory;
},_getColToXPosMap:function(){if(this.__ni==null){this.__ni={};

for(var Q=0;Q<this.__ng.length;Q++){var P=this.__ng[Q];
this.__ni[P]={overX:Q};
}
for(var O=0;O<this.__nh.length;O++){var P=this.__nh[O];
this.__ni[P].visX=O;
}}return this.__ni;
},getVisibleColumnCount:function(){return this.__nh!=null?this.__nh.length:0;
},getVisibleColumnAtX:function(R){return this.__nh[R];
},getVisibleX:function(S){return this._getColToXPosMap()[S].visX;
},getOverallColumnCount:function(){return this.__ng.length;
},getOverallColumnAtX:function(T){return this.__ng[T];
...
for(var x=bb+1;x<this.__ng.length;x++){var bd=this.__ng[x];
var Y=be[bd].visX;

if(Y!=null){bc=Y;
break;
}}if(bc==null){bc=this.__nh.length;
}this.__nh.splice(bc,0,W);
}else{var ba=this.getVisibleX(W);
this.__nh.splice(ba,1);
}this.__ni=null;
if(!this.__lS){var bf={col:W,visible:X};
this.fireDataEvent(g,bf);
this.fireDataEvent(j,bf);
}}},moveColumn:function(bg,bh){this.__lS=true;
var bk=this.__ng[bg];
var bi=this.isColumnVisible(bk);

if(bi){this.setColumnVisible(bk,false);
}this.__ng.splice(bg,1);
this.__ng.splice(bh,0,bk);
this.__ni=null;
...
},setColumnsOrder:function(bl){if(bl.length==this.__ng.length){this.__lS=true;
var bo=new Array(bl.length);

for(var bm=0;bm<this.__ng.length;bm++){var bn=this.isColumnVisible(bm);
bo[bm]=bn;

if(bn){this.setColumnVisible(bm,false);
}}this.__ng=qx.lang.Array.clone(bl);
this.__ni=null;
for(var bm=0;bm<this.__ng.length;bm++){if(bo[bm]){this.setColumnVisible(bm,true);
}}this.__lS=false;
this.fireDataEvent(h);
}else{throw new Error("setColumnsOrder: Invalid number of column positions given, expected "+this.__ng.length+", got "+bl.length);
}}},destruct:function(){for(var i=0;i<this.__nj.length;i++){this.__nj[i].headerRenderer.dispose();
this.__nj[i].dataRenderer.dispose();
this.__nj[i].editorFactory.dispose();
}this.__ng=this.__nh=this.__nj=this.__ni=null;
...
this.__lR=0;
this.__np=[];
},events:{"paneReloadsData":b,"paneUpdated":z},properties:{firstVisibleRow:{check:j,init:0,apply:o},visibleRowCount:{check:j,init:0,apply:w},maxCacheLines:{check:j,init:1000,apply:C},allowShrinkX:{refine:true,init:false}},members:{__lR:null,__no:null,__nn:null,__nq:null,__lO:null,__lN:null,__np:null,__nr:0,_applyFirstVisibleRow:function(G,H){this.updateContent(false,G-H);
},_applyVisibleRowCount:function(I,J){this.updateContent(true);
},_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};
},getPaneScroller:function(){return this.__nn;
},getTable:function(){return this.__nn.getTable();
},setFocusedCell:function(K,L,M){if(K!=this.__lN||L!=this.__lO){var N=this.__lO;
this.__lN=K;
this.__lO=L;
...
},onTableModelDataChanged:function(Q,R,S,T){this.__ns();
var V=this.getFirstVisibleRow();
var U=this.getVisibleRowCount();

if(R==-1||R>=V&&Q<V+U){this.updateContent();
}},onTableModelMetaDataChanged:function(){this.updateContent(true);
},_applyMaxCacheLines:function(W,X){if(this.__nr>=W&&W!==-1){this.__ns();
}},__ns:function(){this.__np=[];
this.__nr=0;
},__nt:function(Y,ba,bb){if(!ba&&!bb&&this.__np[Y]){return this.__np[Y];
}else{return null;
}},__nu:function(bc,bd,be,bf){var bg=this.getMaxCacheLines();

if(!be&&!bf&&!this.__np[bc]&&bg>0){this._applyMaxCacheLines(bg);
this.__np[bc]=bd;
this.__nr+=1;
}},updateContent:function(bh,bi,bj,bk){if(bh){this.__ns();
...
bz.push({col:bY,xPos:x,editable:bA.isColumnEditable(bY),focusedCol:this.__lN==bY,styleLeft:bC,styleWidth:bF});
bC+=bF;
}var bX=[];
var ca=false;
...
}}if(!this.__nq){this.__nq=document.createElement(E);
}var ck=e;
ck+=this._getRowsHtml(ci+ce,Math.abs(cb));
ck+=h;
this.__nq.innerHTML=ck;
var cf=this.__nq.firstChild.childNodes;
...
}}if(this.__lO!==null){this._updateRowStyles(this.__lO-cb);
this._updateRowStyles(this.__lO);
}this.fireEvent(f);
...
this.__lR=cq;
this.fireEvent(f);
}},destruct:function(){this.__nq=this.__nn=this.__np=null;
}});
})();
(function(){var e="first",d="last",c="hovered",b="__nn",a="qx.ui.table.pane.Header";
qx.Class.define(a,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);
this._setLayout(new qx.ui.layout.HBox());
this.__ka=new qx.ui.core.Blocker(this);
this.__nn=f;
},members:{__nn:null,__nv:null,__nw:null,__ka:null,getPaneScroller:function(){return this.__nn;
},getTable:function(){return this.__nn.getTable();
},getBlocker:function(){return this.__ka;
...
}},setMouseOverColumn:function(k){if(k!=this.__nw){if(this.__nw!=null){var l=this.getHeaderWidgetAtColumn(this.__nw);
...
if(this.__nv==null){var y=this.getTable();
...
this.__nv=q;
}this.__nv.setLayoutProperties({left:s.left+x});
this.__nv.show();
},hideColumnMoveFeedback:function(){if(this.__nv!=null){this.__nv.destroy();
this.__nv=null;
}},isShowingColumnMoveFeedback:function(){return this.__nv!=null;
...
}},properties:{appearance:{refine:true,init:o},orientation:{check:[k,h],init:k,apply:v},maximum:{check:s,apply:w,init:100},position:{check:q,init:0,apply:n,event:i},singleStep:{check:y,init:20},knobFactor:{check:A,nullable:true}},members:{__nx:null,__ny:null,_getScrollPaneElement:function(){if(!this.__ny){this.__ny=new qx.html.Element();
}return this.__ny;
...
return {width:this.__nx?100:G,maxWidth:this.__nx?null:G,minWidth:this.__nx?null:G,height:this.__nx?G:100,maxHeight:this.__nx?G:null,minHeight:this.__nx?G:null};
...
if(this.__nx){content.scrollToX(K);
}else{content.scrollToY(K);
}},_applyOrientation:function(L,M){var N=this.__nx=L===k;
...
qx.ui.core.queue.Layout.add(this);
},_updateScrollBar:function(){var P=this.__nx;
...
var W=this.__nx?X.getScrollX():X.getScrollY();
this.setPosition(W);
...
if(this.getOrientation()==h){if(R.height<S.minHeight+this.__nz){Q=true;
}}else{if(R.width<S.minWidth+this.__nz){Q=true;
}}
if(Q){P.exclude();
}else{P.show();
}}}});
...
if(this.__nx){bp=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__nA);
...
}}}else if(this.__nF){this.__jj.stop();
this.releaseCapture();
delete this.__nF;
delete this.__nG;
delete this.__nH;
...
}},_onMouseMove:function(e){if(this.__nD){var bs=this.__nx?e.getDocumentLeft():e.getDocumentTop();
var br=bs-this.__nE;
...
var bv=this.__nx?E:C;
this._updateKnobSize();
this.__nL=bw[bv]-bx[bv];
this.__nC=bx[bv];
this._updateKnobPosition();
},__nx:false,__nL:0,__nM:function(e){var by=this.__nx;
...
if(this.__nx){bT.setStyle(f,bS+a,true);
...
if(bU==null){return;
}if(this.__nx){this.getChildControl(k).setWidth(Math.round(bV*bU.width));
...
this.__nx=bY===j;
if(this.__nx){this.removeState(i);
cb.removeState(i);
this.addState(j);
cb.addState(j);
...
}else{if(this.__nx){this.getChildControl(k).resetWidth();
...
(function(){var o="Boolean",n="resize-line",m="mousedown",l="qx.event.type.Data",k="mouseup",j="qx.ui.table.pane.CellEvent",i="scroll",h="focus-indicator",g="excluded",d="scrollbar-y",bq="table-scroller-focus-indicator",bp="visible",bo="mousemove",bn="header",bm="editing",bl="click",bk="modelChanged",bj="scrollbar-x",bi="cellClick",bh="pane",v="mouseout",w="changeHorizontalScrollBarVisible",t="bottom",u="_applyScrollTimeout",r="changeScrollX",s="_applyTablePaneModel",p="Integer",q="dblclick",C="__nT",D="dataEdited",M="__nQ",J="__nR",U="__kJ",P="mousewheel",bd="interval",ba="__nU",F="qx.ui.table.pane.Scroller",bg="__nO",bf="__nP",be="_applyShowCellFocusIndicator",E="y",H="resize",I="vertical",L="changeScrollY",N="appear",Q="table-scroller",W="beforeSort",bc="__jj",y="cellDblclick",z="horizontal",G="losecapture",T="contextmenu",S="__nS",R="__nN",Y="col-resize",X="disappear",O="_applyVerticalScrollBarVisible",V="_applyHorizontalScrollBarVisible",a="os.scrollBarOverlayed",bb="cellContextmenu",A="close",B="changeTablePaneModel",K="x",b="qx.ui.table.pane.Model",c="changeVerticalScrollBarVisible";
...
this.__nN=this._showChildControl(bh);
this.__nO=new qx.ui.container.Composite(new qx.ui.layout.HBox()).set({minWidth:0});
this._add(this.__nO,{row:0,column:0,colSpan:2});
this.__nP=new qx.ui.table.pane.Clipper();
this.__nP.add(this.__kJ);
this.__nP.addListener(G,this._onChangeCaptureHeader,this);
this.__nP.addListener(bo,this._onMousemoveHeader,this);
this.__nP.addListener(m,this._onMousedownHeader,this);
this.__nP.addListener(k,this._onMouseupHeader,this);
this.__nP.addListener(bl,this._onClickHeader,this);
this.__nO.add(this.__nP,{flex:1});
this.__nQ=new qx.ui.table.pane.Clipper();
this.__nQ.add(this.__nN);
this.__nQ.addListener(P,this._onMousewheel,this);
this.__nQ.addListener(bo,this._onMousemovePane,this);
this.__nQ.addListener(m,this._onMousedownPane,this);
this.__nQ.addListener(k,this._onMouseupPane,this);
this.__nQ.addListener(bl,this._onClickPane,this);
this.__nQ.addListener(T,this._onContextMenu,this);
this.__nQ.addListener(q,this._onDblclickPane,this);
this.__nQ.addListener(H,this._onResizePane,this);
if(qx.core.Environment.get(a)){this.__nR=new qx.ui.container.Composite();
this.__nR.setLayout(new qx.ui.layout.Canvas());
this.__nR.add(this.__nQ,{edge:0});
this._add(this.__nR,{row:1,column:0});
}else{this._add(this.__nQ,{row:1,column:0});
}this.__nS=this._showChildControl(bj);
this.__nT=this._showChildControl(d);
this.__nU=this.getChildControl(h);
...
this.__jj.addListener(bd,this._oninterval,this);
this.initScrollTimeout();
},statics:{MIN_COLUMN_WIDTH:10,RESIZE_REGION_RADIUS:5,CLICK_TOLERANCE:5,HORIZONTAL_SCROLLBAR:1,VERTICAL_SCROLLBAR:2},events:{"changeScrollY":l,"changeScrollX":l,"cellClick":j,"cellDblclick":j,"cellContextmenu":j,"beforeSort":l},properties:{horizontalScrollBarVisible:{check:o,init:false,apply:V,event:w},verticalScrollBarVisible:{check:o,init:false,apply:O,event:c},tablePaneModel:{check:b,apply:s,event:B},liveResize:{check:o,init:false},focusCellOnMouseMove:{check:o,init:false},selectBeforeFocus:{check:o,init:false},showCellFocusIndicator:{check:o,init:true,apply:be},contextMenuFromDataCellsOnly:{check:o,init:true},resetSelectionOnHeaderClick:{check:o,init:true},scrollTimeout:{check:p,init:100,apply:u},appearance:{refine:true,init:Q}},members:{__lR:null,__kM:null,__nV:null,__nW:null,__nX:null,__nY:null,__oa:null,__ob:null,__oc:null,__od:null,__oe:null,__of:null,__og:null,__oh:null,__oi:false,__oj:null,__ok:null,__ol:null,__lN:null,__lO:null,__om:null,__on:null,__oo:null,__nS:null,__nT:null,__kJ:null,__nP:null,__nN:null,__nQ:null,__nR:null,__nU:null,__nO:null,__jj:null,getPaneInsetRight:function(){var bv=this.getTopRightWidget();
...
this.__nQ.add(bA);
break;
case bj:bA=this._createScrollBar(z).set({minWidth:0,alignY:t});
bA.addListener(i,this._onScrollX,this);

if(this.__nR!=null){bA.setMinHeight(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);
this.__nR.add(bA,{bottom:0,right:0,left:0});
}else{this._add(bA,{row:2,column:0});
}break;
case d:bA=this._createScrollBar(I);
bA.addListener(i,this._onScrollY,this);

if(this.__nR!=null){bA.setMinWidth(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);
this.__nR.add(bA,{right:0,bottom:0,top:0});
}else{this._add(bA,{row:1,column:1});
}break;
}return bA||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,by);
},_applyHorizontalScrollBarVisible:function(bB,bC){this.__nS.setVisibility(bB?bp:g);
},_applyVerticalScrollBarVisible:function(bD,bE){this.__nT.setVisibility(bD?bp:g);
...
},_applyShowCellFocusIndicator:function(bH,bI){if(bH){this.__nU.setDecorator(bq);
this._updateFocusIndicator();
}else{if(this.__nU){this.__nU.setDecorator(null);
}}},getScrollY:function(){return this.__nT.getPosition();
},setScrollY:function(scrollY,bJ){this.__nT.scrollTo(scrollY);

if(bJ){this._updateContent();
}},getScrollX:function(){return this.__nS.getPosition();
},setScrollX:function(scrollX){this.__nS.scrollTo(scrollX);
},getTable:function(){return this.__kM;
},onColVisibilityChanged:function(){this.updateHorScrollBarMaximum();
this._updateFocusIndicator();
},setColumnWidth:function(bK,bL){this.__kJ.setColumnWidth(bK,bL);
this.__nN.setColumnWidth(bK,bL);
...
this.__nN.onColOrderChanged();
this.updateHorScrollBarMaximum();
},onTableModelDataChanged:function(bN,bO,bP,bQ){this.__nN.onTableModelDataChanged(bN,bO,bP,bQ);
var bR=this.getTable().getTableModel().getRowCount();

if(bR!=this.__lR){this.updateVerScrollBarMaximum();

if(this.getFocusedRow()>=bR){if(bR==0){this.setFocusedCell(null,null);
}else{this.setFocusedCell(this.getFocusedColumn(),bR-1);
}}this.__lR=bR;
}},onSelectionChanged:function(){this.__nN.onSelectionChanged();
},onFocusChanged:function(){this.__nN.onFocusChanged();
},onTableModelMetaDataChanged:function(){this.__kJ.onTableModelMetaDataChanged();
this.__nN.onTableModelMetaDataChanged();
},_onPaneModelChanged:function(){this.__kJ.onPaneModelChanged();
this.__nN.onPaneModelChanged();
},_onResizePane:function(){this.updateHorScrollBarMaximum();
this.updateVerScrollBarMaximum();
this._updateContent();
this.__kJ._updateContent();
this.__kM._updateScrollBarVisibility();
},updateHorScrollBarMaximum:function(){var bV=this.__nQ.getInnerSize();
...
bU.setKnobFactor(1);
bU.setPosition(0);
}},updateVerScrollBarMaximum:function(){var cf=this.__nQ.getInnerSize();
...
this.fireDataEvent(r,cg,e.getOldData());
this.__nP.scrollToX(cg);
this.__nQ.scrollToX(cg);
...
}this.__nT.scrollBySteps(ck);
ck=e.getWheelDelta(K);
if(ck>0&&ck<1){ck=1;
}else if(ck<0&&ck>-1){ck=-1;
}this.__nS.scrollBySteps(ck);
if(this.__ok&&this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(this.__ok,this.__ol);
}var cj=this.__nT.getPosition();
var ci=this.__nT.getMaximum();
if(ck<0&&cj<=0||ck>0&&cj>=ci){return;
}e.stop();
},__op:function(cl){var cq=this.getTable();
var cr=this.__kJ.getHeaderWidgetAtColumn(this.__oe);
var cm=cr.getSizeHint().minWidth;
var co=Math.max(cm,this.__og+cl-this.__of);

if(this.getLiveResize()){var cn=cq.getTableColumnModel();
cn.setColumnWidth(this.__oe,co,true);
}else{this.__kJ.setColumnWidth(this.__oe,co,true);
var cp=this.getTablePaneModel();
this._showResizeLine(cp.getColumnLeft(this.__oe)+co);
}this.__of+=co-this.__og;
this.__og=co;
},__oq:function(cs){var ct=qx.ui.table.pane.Scroller.CLICK_TOLERANCE;

if(this.__kJ.isShowingColumnMoveFeedback()||cs>this.__od+ct||cs<this.__od-ct){this.__oa+=cs-this.__od;
this.__kJ.showColumnMoveFeedback(this.__nY,this.__oa);
var cu=this.__kM.getTablePaneScrollerAtPageX(cs);

if(this.__oc&&this.__oc!=cu){this.__oc.hideColumnMoveFeedback();
}
if(cu!=null){this.__ob=cu.showColumnMoveFeedback(cs);
}else{this.__ob=null;
}this.__oc=cu;
this.__od=cs;
...
if(this.__oe!=null){this.__op(cz);
cC=true;
e.stopPropagation();
}else if(this.__nY!=null){this.__oq(cz);
e.stopPropagation();
...
this.__oe=cL;
this.__of=cM;
this.__og=cN.getColumnWidth(this.__oe);
this.__nP.capture();
},_startMoveHeader:function(cO,cP){this.__nY=cO;
this.__od=cP;
this.__oa=this.getTablePaneModel().getColumnLeft(cO);
this.__nP.capture();
...
if(cV!==null){this.__oh={row:cV,col:cU};
this.__oi=false;
...
if(!cR){cT.getSelectionManager().handleMouseDown(cV,e);
}}},_onMouseupFocusIndicator:function(e){if(this.__oh&&!this.__oi&&!this.isEditing()&&this.__nU.getRow()==this.__oh.row&&this.__nU.getColumn()==this.__oh.col){this.fireEvent(bi,qx.ui.table.pane.CellEvent,[this,e,this.__oh.row,this.__oh.col],true);
this.__oi=true;
}else if(!this.isEditing()){this._onMousedownPane(e);
}},_onChangeCaptureHeader:function(e){if(this.__oe!=null){this._stopResizeHeader();
}
if(this.__nY!=null){this._stopMoveHeader();
}},_stopResizeHeader:function(){var cW=this.getTable().getTableColumnModel();
if(!this.getLiveResize()){this._hideResizeLine();
cW.setColumnWidth(this.__oe,this.__og,true);
}this.__oe=null;
this.__nP.releaseCapture();
this.getApplicationRoot().setGlobalCursor(null);
this.setCursor(null);
if(this.isEditing()){var cX=this.__om.getBounds().height;
this.__om.setUserBounds(0,0,this.__og,cX);
...
if(this.__oc){this.__oc.hideColumnMoveFeedback();
}
if(this.__ob!=null){var dg=de.getFirstColumnX()+de.getX(this.__nY);
var dc=this.__ob;
...
if(this.__oe!=null){this._stopResizeHeader();
this.__oj=true;
e.stop();
}else if(this.__nY!=null){this._stopMoveHeader();
e.stop();
}},_onClickHeader:function(e){if(this.__oj){this.__oj=false;
return;
...
if(this.__nU.isHidden()||(this.__oh&&!this.__oi&&!this.isEditing()&&du==this.__oh.row&&dv==this.__oh.col)){this.fireEvent(bi,qx.ui.table.pane.CellEvent,[this,e,du,dv],true);
this.__oi=true;
...
if(!this.getShowCellFocusIndicator()||dz===null||(this.__oh&&dz==this.__oh.row&&dA==this.__oh.col)){this.fireEvent(bb,qx.ui.table.pane.CellEvent,[this,e,dz,dA],true);
...
}if(this.__oe==null){this.setCursor(null);
this.getApplicationRoot().setGlobalCursor(null);
}this.__kJ.setMouseOverColumn(null);
if(this.getFocusCellOnMouseMove()){this.__kM.setFocusedCell();
}},_showResizeLine:function(x){var dI=this._showChildControl(n);
var dH=dI.getWidth();
var dJ=this.__nQ.getBounds();
...
var dN=this.__nN.getContainerLocation().left;
...
dO=dL+1;
dM=dW-dN;
}var dQ=this.__nQ.getContainerLocation().left;
var dV=this.__nQ.getBounds().width;
var scrollX=dQ-dN;
...
this.__kM.setFocusedCell(ea,eb);
}},setFocusedCell:function(ec,ed){if(!this.isEditing()){this.__nN.setFocusedCell(ec,ed,this.__nW);
this.__lN=ec;
this.__lO=ed;
this._updateFocusIndicator();
}},getFocusedColumn:function(){return this.__lN;
...
if(eg!=-1){var em=this.__nQ.getInnerSize();
...
}},isEditing:function(){return this.__om!=null;
},startEditing:function(){var ew=this.getTable();
var eu=ew.getTableModel();
var ey=this.__lN;

if(!this.isEditing()&&(ey!=null)&&eu.isColumnEditable(ey)){var ez=this.__lO;
var es=this.getTablePaneModel().getX(ey);
var et=eu.getValue(ey,ez);
this.scrollCellVisible(es,ez);
this.__on=ew.getTableColumnModel().getCellEditorFactory(ey);
var ev={col:ey,row:ez,xPos:es,value:et,table:ew};
this.__om=this.__on.createCellEditor(ev);
if(this.__om===null){return false;
}else if(this.__om instanceof qx.ui.window.Window){this.__om.setModal(true);
this.__om.setShowClose(false);
this.__om.addListener(A,this._onCellEditorModalWindowClose,this);
var f=ew.getModalCellEditorPreOpenFunction();

if(f!=null){f(this.__om,ev);
}this.__om.open();
}else{var ex=this.__nU.getInnerSize();
this.__om.setUserBounds(0,0,ex.width,ex.height);
this.__nU.addListener(m,function(e){this.__oh={row:this.__lO,col:this.__lN};
e.stopPropagation();
},this);
this.__nU.add(this.__om);
this.__nU.addState(bm);
this.__nU.setKeepActive(false);
this.__nU.setDecorator(bq);
this.__om.focus();
this.__om.activate();
}return true;
}return false;
},stopEditing:function(){if(!this.getShowCellFocusIndicator()){this.__nU.setDecorator(null);
}this.flushEditor();
this.cancelEditing();
},flushEditor:function(){if(this.isEditing()){var eB=this.__on.getCellEditorValue(this.__om);
var eA=this.getTable().getTableModel().getValue(this.__lN,this.__lO);
this.getTable().getTableModel().setValue(this.__lN,this.__lO,eB);
this.__kM.focus();
this.__kM.fireDataEvent(D,{row:this.__lO,col:this.__lN,oldValue:eA,value:eB});
}},cancelEditing:function(){if(this.isEditing()&&!this.__om.pendingDispose){if(this._cellEditorIsModalWindow){this.__om.destroy();
this.__om=null;
this.__on=null;
this.__om.pendingDispose=true;
}else{this.__nU.removeState(bm);
this.__nU.setKeepActive(true);
this.__om.destroy();
this.__om=null;
this.__on=null;
...
var eI=this.__nN.getContentLocation().left;
...
},_getRowForPagePos:function(eR,eS){var eT=this.__nN.getContentLocation();

if(eR<eT.left||eR>eT.right){return null;
}
if(eS>=eT.top&&eS<=eT.bottom){var eU=this.getTable().getRowHeight();
var scrollY=this.__nT.getPosition();
...
},setTopRightWidget:function(fb){var fc=this.__oo;

if(fc!=null){this.__nO.remove(fc);
}
if(fb!=null){this.__nO.add(fb);
}this.__oo=fb;
},getTopRightWidget:function(){return this.__oo;
},getHeader:function(){return this.__kJ;
},getTablePane:function(){return this.__nN;
},getVerticalScrollBarWidth:function(){var fd=this.__nT;
return fd.isVisible()?(fd.getSizeHint().width||0):0;
},getNeededScrollBars:function(fe,ff){var fo=this.__nT;
...
},getPaneClipper:function(){return this.__nQ;
...
},_oninterval:qx.event.GlobalError.observeMethod(function(){if(this.__nW&&!this.__nN._layoutPending){this.__nW=false;
this._updateContent();
}}),_updateContent:function(){var fC=this.__nQ.getInnerSize();

if(!fC){return;
}var fF=fC.height;
var scrollX=this.__nS.getPosition();
var scrollY=this.__nT.getPosition();
var fz=this.getTable().getRowHeight();
var fA=Math.floor(scrollY/fz);
var fE=this.__nN.getFirstVisibleRow();
this.__nN.setFirstVisibleRow(fA);
...
if(!fD){fB++;
fy=scrollY%fz;
}this.__nN.setVisibleRowCount(fB);

if(fA!=fE){this._updateFocusIndicator();
}this.__nQ.scrollToX(scrollX);
if(!fD){this.__nQ.scrollToY(fy);
}},_updateFocusIndicator:function(){var fG=this.getTable();

if(!fG.getEnabled()){return;
}this.__nU.moveToCell(this.__lN,this.__lO);
}},destruct:function(){this._stopInterval();
var fH=this.getTablePaneModel();

if(fH){fH.dispose();
}this.__oh=this.__oo=this.__kM=null;
this._disposeObjects(S,C,bf,M,ba,U,R,bg,bc,J);
}});
})();
...
},properties:{visibility:{refine:true,init:b},row:{check:g,nullable:true},column:{check:g,nullable:true}},members:{__or:null,_onKeyPress:function(e){var i=e.getKeyIdentifier();

if(i!==f&&i!==c){e.stopPropagation();
}},moveToCell:function(j,k){if(!this.__or.getShowCellFocusIndicator()&&!this.__or.getTable().getTableModel().isColumnEditable(j)){this.exclude();
return;
}else{this.show();
}
if(j==null){this.hide();
this.setRow(null);
this.setColumn(null);
}else{var l=this.__or.getTablePaneModel().getX(j);

if(l==-1){this.hide();
this.setRow(null);
this.setColumn(null);
}else{var q=this.__or.getTable();
var o=q.getTableColumnModel();
var p=this.__or.getTablePaneModel();
var n=this.__or.getTablePane().getFirstVisibleRow();
...
(function(){var b="__os",a="qx.ui.window.Manager";
qx.Class.define(a,{extend:qx.core.Object,implement:qx.ui.window.IWindowManager,members:{__os:null,setDesktop:function(c){this.__os=c;
this.updateStack();
},getDesktop:function(){return this.__os;
},changeActiveWindow:function(d,e){if(d){this.bringToFront(d);
d.setActive(true);
}
if(e){e.resetActive();
}},_minZIndex:1e5,updateStack:function(){qx.ui.core.queue.Widget.add(this);
},syncWidget:function(){this.__os.forceUnblockContent();
var f=this.__os.getWindows();
...
}}this.__os.setActiveWindow(k);
},bringToFront:function(n){var o=this.__os.getWindows();
var p=qx.lang.Array.remove(o,n);

if(p){o.push(n);
this.updateStack();
}},sendToBack:function(q){var r=this.__os.getWindows();
...
(function(){var l="move",k="Boolean",j="__ou",i="mouseup",h="mousedown",g="losecapture",f="qx.ui.core.MMovable",d="__ot",c="mousemove",b="maximized",a="move-frame";
qx.Mixin.define(f,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__ot:null,__ou:null,__ov:null,__ow:null,__ox:null,__oy:null,__oz:null,__oA:false,__oB:null,__oC:0,_activateMoveHandle:function(m){if(this.__ot){throw new Error("The move handle could not be redefined!");
}this.__ot=m;
...
m.addListener(g,this.__oG,this);
},__oD:function(){var n=this.__ou;

if(!n){n=this.__ou=new qx.ui.core.Widget();
n.setAppearance(a);
n.exclude();
qx.core.Init.getApplication().getRoot().add(n);
}return n;
},__oE:function(){var location=this.getContainerLocation();
var p=this.getBounds();
var o=this.__oD();
o.setUserBounds(location.left,location.top,p.width,p.height);
o.show();
o.setZIndex(this.getZIndex()+1);
},__oF:function(e){var r=this.__ov;
var u=Math.max(r.left,Math.min(r.right,e.getDocumentLeft()));
var t=Math.max(r.top,Math.min(r.bottom,e.getDocumentTop()));
var q=this.__ow+u;
var s=this.__ox+t;
return {viewportLeft:q,viewportTop:s,parentLeft:q-this.__oy,parentTop:s-this.__oz};
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__oB=parent.getBlockerColor();
this.__oC=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__oA=true;
}}this.__ov={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__oy=w.left;
this.__oz=w.top;
this.__ow=v.left-e.getDocumentLeft();
this.__ox=v.top-e.getDocumentTop();
this.addState(l);
this.__ot.capture();
if(this.getUseMoveFrame()){this.__oE();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__oF(e);

if(this.getUseMoveFrame()){this.__oD().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__oA){parent.unblockContent();
parent.setBlockerColor(this.__oB);
parent.setBlockerOpacity(this.__oC);
this.__oB=null;
this.__oC=0;
this.__oA=false;
}}this.__ot.releaseCapture();
var z=this.__oF(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__oD().exclude();
}e.stopPropagation();
},__oG:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__oD().exclude();
...
A.addListener(i,this.__oW,this,true);
A.addListener(q,this.__oX,this);
A.addListener(x,this.__pa,this);
A.addListener(s,this.__pb,this);
A.addListener(o,this.__oY,this);
var B=this.getContainerElement().getDomElement();

if(B==null){B=window;
}this.__oJ=qx.event.Registration.getManager(B).getHandler(qx.event.handler.DragDrop);
},properties:{resizableTop:{check:k,init:true},resizableRight:{check:k,init:true},resizableBottom:{check:k,init:true},resizableLeft:{check:k,init:true},resizable:{group:[l,d,m,y],mode:v},resizeSensitivity:{check:a,init:5},useResizeFrame:{check:k,init:true}},members:{__oJ:null,__oK:null,__oL:null,__oM:null,__oN:null,__oO:null,__oP:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__oK;

if(!C){C=this.__oK=new qx.ui.core.Widget();
C.setAppearance(p);
C.exclude();
qx.core.Init.getApplication().getRoot().add(C);
}return C;
},__oQ:function(){var location=this.__oT();
...
},__oR:function(e){var F=this.__oL;
...
if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){L=Math.max(K.top,Math.min(K.bottom,e.getDocumentTop()))-this.__oN;
...
if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){L=Math.max(K.left,Math.min(K.right,e.getDocumentLeft()))-this.__oM;
...
},__oS:{1:f,2:n,4:h,8:z,5:t,6:g,9:c,10:b},__oT:function(){var O=this.getDecoratorElement();
if(O&&O.getDomElement()){return qx.bom.element.Location.get(O.getDomElement());
}else{return this.getContentLocation();
}},__oU:function(e){var location=this.__oT();
var P=this.getResizeSensitivity();
var S=e.getDocumentLeft();
var R=e.getDocumentTop();
var Q=this.__oV(location,S,R,P);
if(Q>0){Q=Q|this.__oV(location,S,R,P*2);
}this.__oL=Q;
},__oV:function(location,T,U,V){var W=0;
...
},__oW:function(e){if(!this.__oL){return;
}this.addState(j);
this.__oM=e.getDocumentLeft();
this.__oN=e.getDocumentTop();
var bb=this.getContainerLocation();
var X=this.__oT();
var ba=this.getBounds();
this.__oO={top:X.top,left:X.left,containerWidth:bb.right-bb.left,containerHeight:bb.bottom-bb.top,width:X.right-X.left,height:X.bottom-X.top,bounds:qx.lang.Object.clone(ba)};
var parent=this.getLayoutParent();
var bc=parent.getContentLocation();
var Y=parent.getBounds();
this.__oP={left:bc.left,top:bc.top,right:bc.left+Y.width,bottom:bc.top+Y.height};
if(this.getUseResizeFrame()){this.__oQ();
}this.capture();
e.stop();
},__oX:function(e){if(!this.hasState(j)){return;
...
this.releaseCapture();
e.stopPropagation();
},__oY:function(e){if(!this.__oL){return;
}this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
this.removeState(w);
if(this.getUseResizeFrame()){this._getResizeFrame().exclude();
}},__pa:function(e){if(this.hasState(j)){var bh=this.__oR(e);
...
}else if(!this.hasState(u)&&!this.__oJ.isSessionActive()){this.__oU(e);
var bi=this.__oL;
var bg=this.getApplicationRoot();

if(bi){var be=this.__oS[bi];
this.setCursor(be);
bg.setGlobalCursor(be);
}else if(this.getCursor()){this.resetCursor();
bg.resetGlobalCursor();
}}},__pb:function(e){if(this.getCursor()&&!this.hasState(j)){this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
}}},destruct:function(){if(this.__oK!=null&&!qx.core.ObjectRegistry.inShutDown){this.__oK.destroy();
this.__oK=null;
}this.__oJ=null;
}});
})();
...
this.__pd=bh.left===undefined?0:bh.left;
this.__pc=bh.top===undefined?0:bh.top;
...
this.__pd=bi.left===undefined?0:bi.left;
this.__pc=bi.top===undefined?0:bi.top;
this.removeState(b);
this.hide();
this.fireEvent(u);
...
},events:{"modelChanged":e},statics:{EVENT_TYPE_MODEL_CHANGED:"modelChanged"},properties:{firstColumnX:{check:c,init:0,apply:d},maxColumnCount:{check:f,init:-1,apply:a}},members:{__pe:null,__pf:null,_applyFirstColumnX:function(j,k){this.__pe=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},_applyMaxColumnCount:function(l,m){this.__pe=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},setTableColumnModel:function(n){if(this.__pf){this.__pf.removeListener(g,this._onColVisibilityChanged,this);
this.__pf.removeListener(h,this._onColVisibilityChanged,this);
}this.__pf=n;
this.__pf.addListener(g,this._onColVisibilityChanged,this);
this.__pf.addListener(h,this._onHeaderCellRendererChanged,this);
this.__pe=null;
},_onColVisibilityChanged:function(o){this.__pe=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},_onHeaderCellRendererChanged:function(p){this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},getColumnCount:function(){if(this.__pe==null){var q=this.getFirstColumnX();
var s=this.getMaxColumnCount();
var r=this.__pf.getVisibleColumnCount();

if(s==-1||(q+s)>r){this.__pe=r-q;
}else{this.__pe=s;
}}return this.__pe;
},getColumnAtX:function(t){var u=this.getFirstColumnX();
return this.__pf.getVisibleColumnAtX(u+t);
},getX:function(v){var w=this.getFirstColumnX();
var y=this.getMaxColumnCount();
var x=this.__pf.getVisibleX(v)-w;
...
if(A==z){return C;
}C+=this.__pf.getColumnWidth(A);
}return -1;
},getTotalWidth:function(){var D=0;
var E=this.getColumnCount();

for(var x=0;x<E;x++){var F=this.getColumnAtX(x);
D+=this.__pf.getColumnWidth(F);
}return D;
}},destruct:function(){if(this.__pf){this.__pf.removeListener(g,this._onColVisibilityChanged,this);
this.__pf.removeListener(h,this._onColVisibilityChanged,this);
}this.__pf=null;
}});
})();
...
this.__ph=[];
this.__pi={};
},members:{__pg:null,__ph:null,__pi:null,__lS:null,init:function(f){},getRowCount:function(){throw new Error("getRowCount is abstract");
...
},getColumnCount:function(){return this.__pg.length;
},getColumnIndexById:function(y){return this.__pi[y];
},getColumnId:function(z){return this.__pg[z];
},getColumnName:function(A){return this.__ph[A];
},setColumnIds:function(B){this.__pg=B;
this.__pi={};

for(var i=0;i<B.length;i++){this.__pi[B[i]]=i;
}this.__ph=new Array(B.length);
if(!this.__lS){this.fireEvent(e);
}},setColumnNamesByIndex:function(C){if(this.__pg.length!=C.length){throw new Error("this.__columnIdArr and columnNameArr have different length: "+this.__pg.length+" != "+C.length);
}this.__ph=C;
this.fireEvent(e);
},setColumnNamesById:function(D){this.__ph=new Array(this.__pg.length);

for(var i=0;i<this.__pg.length;++i){this.__ph[i]=D[this.__pg[i]];
}},setColumns:function(E,F){var G=this.__pg.length==0||F;

if(F==null){if(this.__pg.length==0){F=E;
}else{F=this.__pg;
}}
...
}},members:{__pj:null,__pm:null,__pn:null,__pl:null,__pk:null,__po:null,getRowData:function(B){var C=this.__pj[B];

if(C==null||C.originalData==null){return C;
}else{return C.originalData;
}},getRowDataAsMap:function(D){var F=this.__pj[D];
...
},setEditable:function(K){this.__pm=[];

for(var L=0;L<this.getColumnCount();L++){this.__pm[L]=K;
}this.fireEvent(d);
},setColumnEditable:function(M,N){if(N!=this.isColumnEditable(M)){if(this.__pm==null){this.__pm=[];
}this.__pm[M]=N;
this.fireEvent(d);
}},isColumnEditable:function(O){return this.__pm?(this.__pm[O]==true):false;
},setColumnSortable:function(P,Q){if(Q!=this.isColumnSortable(P)){if(this.__pn==null){this.__pn=[];
}this.__pn[P]=Q;
this.fireEvent(d);
}},isColumnSortable:function(R){return (this.__pn?(this.__pn[R]!==false):true);
...
},getSortMethods:function(bd){return this.__pl[bd];
},clearSorting:function(){if(this.__pk!=-1){this.__pk=-1;
this.__po=true;
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__pk;
},_setSortColumnIndex:function(be){this.__pk=be;
},isSortAscending:function(){return this.__po;
},_setSortAscending:function(bf){this.__po=bf;
},getRowCount:function(){return this.__pj.length;
},getValue:function(bg,bh){if(bh<0||bh>=this.__pj.length){throw new Error("this.__rowArr out of bounds: "+bh+" (0.."+this.__pj.length+")");
}return this.__pj[bh][bg];
},setValue:function(bi,bj,bk){if(this.__pj[bj][bi]!=bk){this.__pj[bj][bi]=bk;
if(this.hasListener(e)){var bl={firstRow:bj,lastRow:bj,firstColumn:bi,lastColumn:bi};
this.fireDataEvent(e,bl);
}
if(bi==this.__pk){this.clearSorting();
}}},setData:function(bm,bn){this.__pj=bm;
...
},addRows:function(bs,bt,bu){if(bt==null){bt=this.__pj.length;
}bs.splice(0,0,bt,0);
Array.prototype.splice.apply(this.__pj,bs);
var bv={firstRow:bt,lastRow:this.__pj.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
Array.prototype.splice.apply(this.__pj,bA);
var bD={firstRow:bB,lastRow:this.__pj.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
},removeRows:function(bI,bJ,bK){this.__pj.splice(bI,bJ);
var bL={firstRow:bI,lastRow:this.__pj.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1,removeStart:bI,removeCount:bJ};
this.fireDataEvent(e,bL);
...
}},destruct:function(){this.__pj=this.__pm=this.__pl=this.__pn=null;
}});
})();
(function(){var l="id",k="name",h="total",g="seeders",f="rating",e='undefined',d="downloaded",c="leechers",b="progress",a="uploaded",S="online",R="sum_uploaded",Q="sum_downloaded",P="all_time_downloaded",O="all_time_uploaded",N="left",M="state",L="Position ",K="Left",J="Uploaded Now",s="Downloaded Before",t="Total downloaded",q="uploaded Before",r="Id",o="Ss",p="dataAdded",m="%",n="Total",u="Ls",v="Total uploaded",B="Is the table empty?",A="Rating",D="Progress",C="Purge from the table entry by real id ",F="dataUpdated",E="Downloaded Now",x="Name",I="rhyacotriton.Table",H="dataLoadCompleted",G="dataRemoved",w="State",y="dataRemoveFailure",z="On-line";
qx.Class.define(I,{extend:qx.ui.table.Table,construct:function(T){this.__kL=T;
var U=this;
this.__pp=new smart.model.Default;
this.__pp.setColumns([r,x,n,K,D,A,z,o,u,w,E,J,q,s,t,v],[l,k,h,N,b,f,S,g,c,M,d,a,O,P,Q,R]);
qx.ui.table.Table.call(this,this.__pp);
this.__pf=this.getTableColumnModel();
this.__pv=this.__pp.getColumnIndexById(l);
this.__qL=this.__pp.getColumnIndexById(k);
this.__pq=this.__pp.getColumnIndexById(S);
this.__pr=this.__pp.getColumnIndexById(N);
this.__ps=this.__pp.getColumnIndexById(h);
this.__pt=this.__pp.getColumnIndexById(b);
this.__qM=this.__pp.getColumnIndexById(c);
this.__qN=this.__pp.getColumnIndexById(g);
this.__qU=this.__pp.getColumnIndexById(d);
this.__qV=this.__pp.getColumnIndexById(a);
this.__qX=this.__pp.getColumnIndexById(Q);
this.__qY=this.__pp.getColumnIndexById(R);
this.__rc=this.__pp.getColumnIndexById(P);
this.__rd=this.__pp.getColumnIndexById(O);
this.__qR=this.__pp.getColumnIndexById(f);
this.__qW=this.__pp.getColumnIndexById(M);
this.__pf.setColumnWidth(this.__qL,300,true);
this.__pf.setColumnWidth(this.__pv,30,true);
this.__pf.setColumnWidth(this.__pt,70,true);
this.__pf.setColumnWidth(this.__ps,70,true);
this.__pf.setColumnWidth(this.__pr,70,true);
this.__pf.setColumnWidth(this.__qM,40,true);
this.__pf.setColumnWidth(this.__qN,40,true);
this.__pf.setColumnWidth(this.__qW,70,true);
this.__pf.setColumnWidth(this.__qR,70,true);
this.__pf.setColumnWidth(this.__rc,70,true);
this.__pf.setColumnWidth(this.__rd,70,true);
this.__pf.setColumnWidth(this.__qU,70,true);
this.__pf.setColumnWidth(this.__qV,70,true);
var V=function(ba){return U.__qS(ba);
};
var X=function(bb){return U.__qT(bb);
};
this.__pf.setDataCellRenderer(this.__pt,new rhyacotriton.cellrenderer.Progress(V));
this.__pf.setDataCellRenderer(this.__qR,new rhyacotriton.cellrenderer.Rating(X));
this.__pf.setDataCellRenderer(this.__pq,new qx.ui.table.cellrenderer.Boolean());
[this.__ps,this.__pr,this.__qU,this.__qV,this.__rc,this.__rd].map(function(bc){U.__pf.setDataCellRenderer(bc,new rhyacotriton.cellrenderer.Size());
});
var Y=function(bd){return U.__ra(bd);
};
var W=function(be){return U.__rb(be);
};
U.__pf.setDataCellRenderer(this.__qX,new rhyacotriton.cellrenderer.Size(Y));
U.__pf.setDataCellRenderer(this.__qY,new rhyacotriton.cellrenderer.Size(W));
[this.__pr,this.__qU,this.__qV,this.__rc,this.__rd,this.__pq].map(function(bf){U.__pf.setColumnVisible(bf,false);
});
this.__pu=this.getSelectionModel();
this.__pu.setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
this.__pp.indexedSelection(this.__pv,this.__pu);
this.__pp.addIndex(this.__pv);
this.__kL.addListener(p,this.__qO,this);
this.__kL.addListener(G,this.__py,this);
this.__kL.addListener(y,this.__pz,this);
this.__kL.addListener(H,this.__px,this);
this.__kL.addListener(F,this.__pA,this);
},members:{__kL:null,__pv:null,__qL:null,__pq:null,__pr:null,__ps:null,__pt:null,__qM:null,__qN:null,__qW:null,__qR:null,__rc:null,__rd:null,__qU:null,__qV:null,__qX:null,__qY:null,__pp:null,__pf:null,__pu:null,__qS:function(bg){var bi=bg[this.__ps];
var bj=bg[this.__pr];
var bh=bi-bj;
return ((bh/bi)*100).toFixed(2)+m;
},__qT:function(bk){var bn=bk[this.__qV];
var bl=bk[this.__qU];
var bm=bk[this.__rd];
var bo=bk[this.__rc];

if(bl==0)return 0;
return ((bn+bm)/(bl+bo)).toFixed(2);
},__ra:function(bp){var bq=bp[this.__qU];
var br=bp[this.__rc];
return (bq+br);
},__rb:function(bs){var bu=bs[this.__qV];
var bt=bs[this.__rd];
return (bu+bt);
},logSelectedRows:function(){var bv=this.__pp;
this.__pu.iterateSelection(function(bw){console.log(L+bw);
console.dir(bv.getRowData(bw));
});
},getSelectedIds:function(){var by=[];
var bx=this;
this.__pu.iterateSelection(function(bz){by.push(bx.__pp.getValue(bx.__pv,bz));
});
return by;
},removeSelectedRows:function(){this.info("RemoveSelectedRows");
this.logSelectedRows();
var bA=this;
this.__pu.iterateSelection(function(bB){console.log(L+bB);
var bC=bA.__pp.getRowDataAsMap(bB);
bA.__kL.removeElement(bC);
bA.__pp.removeRows(bB,1);
});
},startSelectedRows:function(){this.info("startSelectedRows");
this.__kL.start(this.getSelectedIds());
},stopSelectedRows:function(){this.info("stopSelectedRows");
this.__kL.stop(this.getSelectedIds());
},__px:function(event){var bD=event.getData();

try{this.__pp.removeRows(0,undefined,undefined,false);
}catch(bE){console.log(B);
}this.__pp.setDataAsMapArray(bD.rows);
this.updateContent();
},__qO:function(event){var bF=event.getData();
this.__pp.setDataAsMapArray(bF.rows);
this.updateContent();
},__py:function(event){var bG=event.getData().rows;

for(var i in bG){console.log(C+bH);
var bH=bG[i];
var bI=this.__pp.locate(this.__pv,bH);
if(bI!=e)this.__pp.removeRows(bI,1);
}},__pz:function(event){var bJ=event.getOldData();
this.__pp.addRowsAsMapArray([bJ]);
},particallyUpdateRows:function(bK){for(var i=0,bL=bK.length;i<bL;i++){var bO=bK[i];

if(typeof (bO.id)==e)this.error("Cannot get Rows[i].id");
var bN=this.__pp.locate(this.__pv,bO.id);

for(var j in bO){if(j==l)continue;
var bM=this.__pp.getColumnIndexById(j);
this.__pp.setValue(bM,bN,bO[j]);
}}},__pA:function(event){var bP=event.getData();
this.particallyUpdateRows(bP.rows);
this.updateContent();
}}});
})();
...
qx.Mixin.define(b,{members:{testsFailed:false,__pB:function(name,f){var cb=false;
...
return !cb;
},unitTest:function(){try{this.__pC();
}catch(e){this.__debug(e);
}},__pC:function(){var i,cj=false;
...
this.__pB(bU,function(){for(name in cn){var ct=cn[name];
...
cr.addRows(ci);
this.__pB(bc,function(){return (cr.getRowCount()==cp);
});
this.__pB(bT,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__pB(k,function(){for(i=0;i<N;i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
cr.setData(ci);
this.__pB(bv,function(){return (cr.getRowCount()==N);
});
this.__pB(Q,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__pB(bM,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
this.__pB(O,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(bS,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];
...
cr.addRows(ci);
cp+=10;
this.__pB(a,function(){return (cr.getRowCount()==cp);
});
this.__pB(n,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i)!=i-10)return false;
return true;
});
this.__pB(d,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i-10)return false;
return true;
});
this.__pB(L,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(r,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];
...
cr.addRows(ci);
cp+=10;
this.__pB(q,function(){return (cr.getRowCount()==cp);
});
this.__pB(bX,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i)!=i-10)return false;
return true;
});
this.__pB(bJ,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i-10)return false;
return true;
});
this.__pB(z,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(g,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];
...
cp+=N;
this.__pB(bf,function(){return (cr.getRowCount()==cp);
});
this.__pB(ba,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(bu,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(N>>2,N>>1);
cp-=(N>>1);
this.__pB(br,function(){return (cr.getRowCount()==cp);
});
this.__pB(bN,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(by,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(0,5);
cp-=5;
this.__pB(X,function(){return (cr.getRowCount()==cp);
});
this.__pB(bs,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(bV,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(cr.getRowCount()-5);
cp-=5;
this.__pB(T,function(){return (cr.getRowCount()==cp);
});
this.__pB(bl,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(y,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(0,1);
...
cp-=3;
this.__pB(S,function(){return (cr.getRowCount()==cp);
});
this.__pB(F,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
...
cp+=3;
this.__pB(bb,function(){return (cr.getRowCount()==cp);
});
this.__pB(bC,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(bC,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(c,function(){return (cr.locate(cn[bF],cl)==0);
});
this.__pB(bq,function(){return (cr.locate(cn[bF],cm)==cr.getRowCount()-1);
});
cr.setValue(cn[bB],5,null);
this.__pB(bL,function(){return (cr.getValue(cn[bB],5)==null);
});
this.__pB(K,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(H,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
...
this.__pB(p,function(){return (cr.getValue(cn[bD],i)==ce);
});
this.__pB(t,function(){for(var i=0;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bD],i,1)==ce)return false;
return true;
});
this.__pB(s,function(){return cr.locate(cn[bF],I,1)==undefined;
});
this.__pB(bw,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(l,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(B,function(){return (cr.getRowCount()==cf);
});
this.__pB(bW,function(){return (cr.getRowCount(1)==(cq-1));
});
var cf=cr.getRowCount();
var cq=cr.getRowCount(1);
var cd=bd;
...
this.__pB(bI,function(){return (cr.getValue(cn[bD],i)==cd);
});
this.__pB(Y,function(){for(var i=0;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bD],i,1)==cd)return true;
return false;
});
this.__pB(bi,function(){return cr.locate(cn[bF],I,1)!=undefined;
});
this.__pB(M,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(bg,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(G,function(){return (cr.getRowCount()==cf);
});
this.__pB(bz,function(){return (cr.getRowCount(1)==(cq+1));
});
cf=cr.getRowCount();
cq=cr.getRowCount(1);
ck=0.1;
cr.setValue(cn[bF],cq>>1,ck,1);
this.__pB(V,function(){return (cr.getValue(cn[bF],cr.locate(cn[bF],ck))==ck);
});
this.__pB(o,function(){return (cr.getValue(cn[bF],cr.locate(cn[bF],ck,1),1)==ck);
});
this.__pB(bt,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(W,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__pB(bH,function(){return (cr.getRowCount()==cf);
});
this.__pB(u,function(){return (cr.getRowCount(1)==cq);
});
cf=cr.getRowCount();
cq=cr.getRowCount(1);
cr.sortByColumn(cn[bD],true);
this.__pB(ca,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bD],i-1)>cr.getValue(cn[bD],i))return false;
return true;
});
this.__pB(bp,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bD],i-1)>cr.getValue(cn[bD],i))return false;
return true;
});
this.__pB(bP,function(){return (cr.getRowCount()==cf);
});
this.__pB(bo,function(){return (cr.getRowCount(1)==cq);
});
...
this.__pB(bO,function(){for(i=0;i<cr.getRowCount(2);i++){var x=cr.getValue(cn[bF],i,2);

if(x>=20&&x<=80)return false;
}return true;
});
this.__pB(E,function(){for(i=0;i<cr.getRowCount(3);i++){var x=cr.getValue(cn[bF],i,3);

if(x<20||x>80)return false;
}return true;
});
this.__pB(bY,function(){var cu={};
...
this.__pB(D,function(){return cr.getRowCount(2)+cr.getRowCount(3)==cr.getRowCount(0);
});
var cg=[];

for(i=0;i<cr.getRowCount(2);i++)cg.push(cr.getRowReference(i,2));
cr.removeReferencedRows(cg);
this.__pB(bK,function(){return cr.getRowCount(2)==0;
});
this.__pB(bm,function(){for(i=0;i<cr.getRowCount();i++){var x=cr.getValue(cn[bF],i);

if(x<20||x>80)return false;
}return true;
});
cr.clearAllRows();
this.__pB(m,function(){for(var v=0;v<cr.getViewCount();v++)if(cr.getRowCount(v)!=0)return false;
return true;
});
cr.sortByColumn(cn[bF],true);
N=1000;
...
}this.__pB(P,function(){cc.sort(function(cv,cw){return cv[cn[bF]]-cw[cn[bF]];
});

for(var i=0;i<cc.length;i++)if(cr.getValue(cn[bF],i)!=cc[i][cn[bF]])return false;
return true;
});
this.__pB(h,function(){return cr.getRowCount()==co;
});
N=(1<<10);
cr.clearAllRows();
co={};
...
cr.removeRows(I,1);
}this.__pB(bj,function(){for(i in co)if(co[i]!=undefined)if(cr.locate(cn[bF],i)==undefined)return false;

for(i=0;i<cr.getRowCount();i++)if(co[cr.getValue(cn[bF],i)]!=1)return false;
return true;
});
this.__pB(bk,function(){for(i=0;i<cr.getRowCount(2);i++)if(co[cr.getValue(cn[bF],i,2)]!=1)return false;
return true;
});
cr.clearAllRows();
var N=100,cp=100;
...
this.__pB(U,function(){return (cr.getRowCount()==cp);
});
this.__pB(bA,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__pB(bG,function(){for(i=0;i<N;i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
...
},properties:{view:{init:0,check:G,apply:E,event:d}},members:{__pH:null,__pF:null,__pG:null,__kM:null,init:function(H){this.__kM=H;
...
if(I>=this.__pE.length){throw new Error("_applyView: view out of bounds: "+I+" (0.."+(this.__pE.length-1)+")");
}if(M)this.__pL(J);
this.__pj=this.getRowArray(I);
if(K)this.__qg(I);
},_changeView:function(e){this.__pM();
...
for(var bb in this.__pH){this.__pH[bb].push({});
}var bd=this.__pE.length;
this.__pE.push(bc);
if(this.getView()==bd){this.__pL();
}this.__qb(bd);
if(this.getView()==bd){this.__pM();
}return bd;
},updateView:function(be){if(this.getView()==be){this.__pL();
}this.__qb(be);
if(this.getView()==be){this.__pM();
}},resetFilters:function(bf){this.setFilters(bf);
},getViewCount:function(){return this.__pE.length;
},addIndex:function(bg){var A=[];

for(var v=0;v<this.__pE.length;v++){A.push({});
}this.__pH[bg]=A;
this._updateAssociationMaps(undefined,bg);
},locate:function(bh,bi,bj){if(bj===undefined){bj=this.getView();
}return this.__pH[bh][bj][o+bi];
},indexedSelection:function(bk,bl){this.__pJ=bk;
this.__pu=bl;
this.__pI=[];
this.__pK=false;
},suspendIndexedSelection:function(bm){this.__pK=bm;
},__pL:function(bn){if(bn===undefined){bn=this.getView();
}
if(!this.__pu||this.__pJ<0||this.__pJ>=this.getColumnCount()){return;
}this.__pI=[];
this.__pu.iterateSelection(function(bo){this.__pI.push(this.getValue(this.__pJ,bo,bn));
},this);
},__pM:function(bp){if(this.__pI==null||!this.__pu||this.__pJ<0||this.__pJ>=this.getColumnCount()){return;
}var br=this.__pu;
br.setBatchMode(true);
if(!this.__pK){this.__pN();
var bq=this.__pI;
for(var i=0;i<bq.length;i++){var bs=this.locate(this.__pJ,bq[i],bp);
if(bs!==undefined){br.addSelectionInterval(bs,bs);
}}}br.setBatchMode(false);
},__pN:function(){var bt=this.__pu;

if(bt){bt.resetSelection();
}},getRowArray:function(bu,bv){if(bu===undefined||bu===null){bu=this.getView();
}
if(typeof bv==p){return (bv?this.__pG[bu]:this.__pF[bu]);
}else{var bw=this.__pG[bu]||this.__pF[bu];
return bw;
}},setAlternateRowArray:function(bx,by){if(bx===undefined||bx===null){bx=this.getView();
}this.__pG[bx]=(by||null);

if(this.__pG[bx]){this.__pG[bx].__pO=D;
}return this.__pG[bx];
},__pP:function(bz,A,bA){if(bA===undefined){bA=false;
}var bB=false;
if(bz==this.getView()){bB=true;
}this.__pF[bz]=A;

if(bB){this._applyView(bz,bz,true,true,bA);
}},__pQ:function(bC){if(bC===undefined){bC=this.getView();
}
if(bC<this.__pE.length){return this.__pE[bC].associations;
}else{return undefined;
}},__pR:function(bD){if(bD===undefined){bD=this.getView();
}
if(bD<this.__pE.length){return this.__pE[bD].fFilter;
}else{return undefined;
...
if(bI[bE]===undefined){this.debug("No entry in "+bI.__pO+" for row "+bE);
return null;
...
},__pS:function(bU,bV,bW){if(bW===undefined){bW=true;
}var i;
var A=this.getRowArray(bU);

if(bV.length>=(A.length>>2)){var bX={};

for(i=0;i<bV.length;i++){var ca=this.__qf(bU,bV[i]);
...
}if(!this.isSorted()&&!cr.advanced.fSort){this.__pT(cm,cn,cq);
return;
...
if(!A.length||ct(cn[0],A[A.length-1])>=0){this.__pT(cm,cn,cq);
}else if(ct(cn[cn.length-1],A[0])<=0){this.__pU(cm,cn,cq);
}else{if(cn.length<(A.length>>1)){var cx=cn.length;
var cu=0,cz=A.length-1;
var cy=[];
var i;
for(i=0;i<cx;i++){var cB=this.__qh(A,cn[i],ct,cu,cz);
...
}},__pW:function(cH,cI,V,cJ){if(cJ===undefined){cJ=this.getView();
...
}else{if(this.__pH[cH]!==undefined){this.__qe(cH,R[cH],V);
}R[cH]=V;
}cK=(this.isSorted()&&(cH===this.getSortColumnIndex()));
}else{for(var cL=0;cL<V.length&&cL<cM;cL++){if(this.__pH[cL]!==undefined){this.__qe(cL,R[cL],V[cL]);
}R[cL]=V[cL];
}if(this.isSorted()&&V.length>=this.getSortColumnIndex()){cK=true;
}}this.__pX(R,cK);
this.__pM();
},__pX:function(R,cN,cO,cP){if(cO===undefined){cO=false;
}
if(cP===undefined){cP=true;
}for(var v=(cO?1:0);v<this.__pE.length;v++){var cS=this.__qf(v,R);
var cQ,cR;

if(v==0){cQ=false;
cR=false;
}else{cQ=(this.__qf(v,R)===undefined);
cR=this.__qa(v,R);
}if(cN){if(!cQ){this.__pS(v,[R]);
}if(!cR){this.__pV(v,[R],false);
}}else{if(cQ!=cR){if(cR){this.__pS(v,[R]);
}else{this.__pV(v,[R],false);
}}}if(cP){this.__qg();
}}},setValue:function(cT,cU,cV,cW){this.__pW(cT,cU,cV,cW);
},setValueById:function(cX,cY,da,db){return this.setValue(this.getColumnIndexById(cX),cY,da,db);
},setRow:function(dc,dd,de){this.__pW(-1,dc,dd,de);
...
A.sort(dp);
for(var v=0;v<this.__pE.length;v++){this.setAlternateRowArray(v,null);
this.__pV(v,A,true,true);
}this.__pM();
...
for(var v=0;v<this.__pE.length;v++){var dy=this.__pE[v];
...
if(dx){this.__qg();
}},clearAllRows:function(){if(this.getRowCount()>0){this.__pN();

for(var v=0;v<this.__pE.length;v++){this.__pF[v]=[];
this.__pF[v].__pO=y;
this.__pG[v]=null;
}this._updateAssociationMaps();
this.__qg();
}},forceRedraw:function(){this.__qg();
},getRowId:function(dz){return dz.__cq;
},getRowById:function(dA,dB){return this.getRowArray(dA,false)[this.__pE[dA].associations[dB]];
},__pY:function(dC,R,dD){if(dD===undefined){dD=true;
}var dE=this.__pR(dC);
...
},__qa:function(dI,R){return !this.__pY(dI,R,true);
},__qb:function(dJ,dK,dL){if(dJ==0){return;
}
if(dK===undefined){dK=true;
}
if(dL===undefined){dL=true;
}var U=this.getRowArray(0);
this.__pP(dJ,this.__pY(dJ,U,false));
if(dL){this._updateAssociationMaps(dJ);
}if(dK&&this.getView()==dJ){this.__qg(dJ);
}},__qc:function(dM,dN){for(var v=1;v<this.__pE.length;v++){this.__qb(v,dM,dN);
}},__qd:1,assignRowIDs:function(A){for(var i=0;i<A.length;i++){A[i].__cq=this.__qd++;
}},_updateAssociationMaps:function(dO,dP){for(var v=0;v<this.__pE.length;v++){if(dO!==undefined&&dO!=v){continue;
}var A=this.getRowArray(v);
if(dP===undefined){this.__pE[v].associations={};
}for(var dQ in this.__pH){if(dP===undefined||dP==dQ){this.__pH[dQ][v]={};
}}for(var j=0;j<A.length;j++){var R=A[j];

if(dP===undefined){this.__pE[v].associations[R.__cq]=j;
}for(dQ in this.__pH){if(dP===undefined||dP==dQ){this.__pH[dQ][v][o+R[dQ]]=j;
}}}}},_updateAssociationMapsAfterPush:function(dR,dS,dT){var dV=this.__pQ(dR);
var dW=dT;
for(var i=0;i<dS.length;i++,dW++){var R=dS[i];
dV[R.__cq]=dW;
for(var dU in this.__pH){this.__pH[dU][dR][o+R[dU]]=dW;
}}},__qe:function(dX,dY,ea){dY=o+dY;
ea=o+ea;

for(var ed=0;ed<this.__pE.length;ed++){for(var eb in this.__pH){var ec=this.__pH[eb][ed][dY];
this.__pH[eb][ed][dY]=undefined;
this.__pH[eb][ed][ea]=ec;
}}},__qf:function(ee,R){try{if(R.__cq===undefined){return undefined;
}}catch(e){this.__qi(e);
return undefined;
}return this.__pQ(ee)[R.__cq];
...
}for(var v=0;v<this.__pE.length;v++){this.getRowArray(v).reverse();
}this._setSortAscending(el);
}else{this._setSortColumnIndex(ek);
this._setSortAscending(el);
var en=this.getComparator();
en.columnIndex=ek;
for(var v=0;v<this.__pE.length;v++){this.getRowArray(v).sort(en);
}}this._updateAssociationMaps();
this.__pM();
this.fireEvent(C);
...
}},__qg:function(eq){if(this.hasListener(k)){if(eq===undefined){eq=this.getView();
}var er={firstRow:0,lastRow:this.getRowCount(eq)-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(k,er);
}},__qh:function(A,e,es,et,eu){if(et===undefined){et=0;
}
...
},__qi:function(ex){if(typeof (qx.core.Variant)!==m){if(qx.core[l].isSet(n,c)){if(this.__pD){this.debug(ex);
}}}
if(typeof (qx.core.Environment)!==m){if(qx.core[h].get(n)){if(this.__pD){this.debug(ex);
}}}},__qj:function(ey,ez,eA){if(ez===undefined){ez=o;
}
if(eA===undefined){eA=true;
}this.__qi(ez+w+(ey?(ey+B):q));

if(ey==null||ey===undefined){this.__qi(t);
return;
}
for(var eB in ey){if(eA&&!ey.hasOwnProperty(eB)){continue;
}this.__qi(x+eB+z+ey[eB]);
...
this.__qt=v||s;
this.__qu=w||s;
},members:{__qr:null,__qs:null,__qt:null,__qu:null,__qv:function(x,y){if(x[1]!=null){y[f]=x[1];
}
if(x[2]!=null){y[d]=x[2];
}
if(x[3]!=null){y[g]=x[3];
...
var bf;
var Y;
var bb={"text-align":this.__qr,"color":this.__qs,"font-style":this.__qt,"font-weight":this.__qu};
...
bf=ba.test(Y);
}if(bf==true){this.__qv(this.conditions[i],bb);
}}var be=[];
...
(function(){var b="qooxdoo-table-cell-right qooxdoo-table-cell",a="rhyacotriton.cellrenderer.Progress";
qx.Class.define(a,{extend:qx.ui.table.cellrenderer.Conditional,construct:function(c){this.__qw=c;
qx.ui.table.cellrenderer.Conditional.call(this);
},members:{__qw:null,_getContentHtml:function(d){return this.__qw(d.rowData);
...
},init:i}},members:{__qk:16,__ql:16,__qm:null,_insetY:2,_identifyImage:function(A){throw new Error("_identifyImage is abstract");
},_getImageInfos:function(B){var C=this._identifyImage(B);
if(C==null||typeof C==e){C={url:C,tooltip:null};
}if(!C.imageWidth||!C.imageHeight){var D=this.__qn(C.url);
C.imageWidth=D.width;
C.imageHeight=D.height;
}C.width=C.imageWidth;
C.height=C.imageHeight;
return C;
},__qn:function(E){var H=qx.util.ResourceManager.getInstance();
...
},createDataCellHtml:function(J,K){this.__qm=this._getImageInfos(J);
...
if(this.__qm.url){content=qx.bom.element.Decoration.create(this.__qm.url,this.getRepeat(),{width:this.__qm.width+j,height:this.__qm.height+j,display:qx.core.Environment.get(h),verticalAlign:q,position:d});
}return content;
},_getCellAttributes:function(N){var O=this.__qm.tooltip;
...
this.initIconTrue();
this.initIconFalse();
},properties:{iconTrue:{check:g,init:e,apply:f},iconFalse:{check:g,init:b,apply:a}},members:{__qp:null,__qq:false,__qo:null,_applyIconTrue:function(h){this.__qp=this.__qo.resolve(h);
},_applyIconFalse:function(i){this.__qq=this.__qo.resolve(i);
...
switch(k.value){case true:l.url=this.__qp;
break;
case false:l.url=this.__qq;
break;
default:l.url=null;
break;
}return l;
...
(function(){var h=' B',g="undefined",f="qooxdoo-table-cell-right qooxdoo-table-cell",e="rhyacotriton.cellrenderer.Size",d=' TB',c=' KB',b=' GB',a=' MB';
qx.Class.define(e,{extend:qx.ui.table.cellrenderer.Conditional,construct:function(i){if(typeof (i)!=g)this.__qw=i;
qx.ui.table.cellrenderer.Conditional.call(this);
},members:{__qw:undefined,_getContentHtml:function(j){var k;
k=(typeof (this.__qw)==g)?j.rowData[j.col]:this.__qw(j.rowData);
return this.bytesToSize(k,2);
},_getCellClass:function(l){return f;
},bytesToSize:function(m,n){var o=1024;
var p=o*1024;
var q=p*1024;
var r=q*1024;

if((m>=0)&&(m<o)){return m+h;
}else if((m>=o)&&(m<p)){return (m/o).toFixed(n)+c;
}else if((m>=p)&&(m<q)){return (m/p).toFixed(n)+a;
}else if((m>=q)&&(m<r)){return (m/q).toFixed(n)+b;
}else if(m>=r){return (m/r).toFixed(n)+d;
}else{return m+h;
}}}});
})();
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__qx:g,__qy:g,execute:function(U){this.fireDataEvent(L,U);
},__qz:function(event){if(this.getEnabled()&&this.__qA(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__fR:function(event){if(this.getEnabled()&&this.__qA(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__qz,this);
qx.event.Registration.addListener(document.documentElement,d,this.__fR,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__qz,this);
...
this.error(bc);
throw new Error(bc);
}this.__qx={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__qy=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__qC(a[i]);

switch(bd){case f:case u:case K:case C:this.__qx[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__qy){var bc=r;
this.error(bc);
throw bc;
}this.__qy=bd;
}}}return true;
},__qA:function(e){var be=this.__qy;

if(!be){return ;
}if((!this.__qx.Shift&&e.isShiftPressed())||(this.__qx.Shift&&!e.isShiftPressed())||(!this.__qx.Control&&e.isCtrlPressed())||(this.__qx.Control&&!e.isCtrlPressed())||(!this.__qx.Meta&&e.isMetaPressed())||(this.__qx.Meta&&!e.isMetaPressed())||(!this.__qx.Alt&&e.isAltPressed())||(this.__qx.Alt&&!e.isAltPressed())){return false;
}
if(be==e.getKeyIdentifier()){return true;
}return false;
},__qB:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__qC:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__qB[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__qy;
var bj=[];

for(var bi in this.__qx){if(this.__qx[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__qx=this.__qy=null;
}});
})();
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__qD=new qx.ui.decoration.css3.BorderImage();

if(p){this.__qE(p);
}}else{this.__qD=new qx.ui.decoration.GridDiv(p);
}
if(q!=null){this.__qD.setInsets(q);
}},properties:{baseImage:{check:d,nullable:true,apply:j},insetLeft:{check:o,nullable:true,apply:n},insetRight:{check:o,nullable:true,apply:n},insetBottom:{check:o,nullable:true,apply:n},insetTop:{check:o,nullable:true,apply:n},insets:{group:[k,l,i,e],mode:f}},members:{__qD:null,getMarkup:function(){return this.__qD.getMarkup();
},resize:function(r,s,t){this.__qD.resize(r,s,t);
},tint:function(u,v){},getInsets:function(){return this.__qD.getInsets();
},_applyInsets:function(w,x,name){var y=g+qx.lang.String.firstUp(name);
this.__qD[y](w);
},_applyBaseImage:function(z,A){if(this.__qD instanceof qx.ui.decoration.GridDiv){this.__qD.setBaseImage(z);
}else{this.__qE(z);
}},__qE:function(B){var G,H,J,I;
this.__qD.setBorderImage(B);
...
qx.Mixin.define(o,{include:[qx.ui.decoration.MSingleBorder,qx.ui.decoration.MBackgroundImage],construct:function(){this._getDefaultInsetsForBorder=this.__qK;
this._resizeBorder=this.__qJ;
this._styleBorder=this.__qH;
this._generateMarkup=this.__qI;
},properties:{innerWidthTop:{check:f,init:0},innerWidthRight:{check:f,init:0},innerWidthBottom:{check:f,init:0},innerWidthLeft:{check:f,init:0},innerWidth:{group:[w,z,n,k],mode:a},innerColorTop:{nullable:true,check:g},innerColorRight:{nullable:true,check:g},innerColorBottom:{nullable:true,check:g},innerColorLeft:{nullable:true,check:g},innerColor:{group:[u,v,x,r],mode:a}},members:{__qG:null,__qH:function(D){{var F=qx.theme.manager.Color.getInstance();
...
}},__qI:function(K){var O=this._generateBackgroundMarkup(K);
...
K[y]=0;
K[p]=0;
return this.__qG=this._generateBackgroundMarkup(K,O);
},__qJ:function(S,T,U){var V=this.getInsets();
T-=V.left+V.right;
U-=V.top+V.bottom;
...
},__qK:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
