...
var libinfo = {"__out__":{"sourceUri":"script"},"qx":{"resourceUri":"resource","sourceUri":"script"},"rhyacotriton":{"resourceUri":"resource","sourceUri":"script"},"smart":{"resourceUri":"resource","sourceUri":"script"}};
...
qx.$$loader = {
  parts : {"boot":[0]},
  packages : {"0":{"uris":["__out__:rhyacotriton.f98354fc5d13.js"]}},
  urisBefore : [],
  cssBefore : [],
  boot : "boot",
  closureParts : {},
...
(function(){var g="",f="qx.core.BaseInit",d="engine.name",c="os.name",b="engine.version",a="rhyacotriton.Application";
...
},toUriParameter:function(L,M){var P,O=[],N=window.encodeURIComponent;

for(P in L){if(L.hasOwnProperty(P)){if(M){O.push(N(P).replace(/%20/g,c)+d+N(L[P]).replace(/%20/g,c));
}else{O.push(N(P)+d+N(L[P]));
}}}return O.join(b);
}}});
})();
...
(function(){var m="engine.version",l="os.name",k="x",j="y",i="engine.name",h="browser.name",g="osx",f="?",d="win",c="qx.dynamicmousewheel",a="chrome",b="qx.event.type.MouseWheel";
...
this.preventDefault();
},__fG:function(n){var o=Math.abs(n);
if(qx.event.type.MouseWheel.MINSCROLL==null||qx.event.type.MouseWheel.MINSCROLL>o){qx.event.type.MouseWheel.MINSCROLL=o;
this.__fH();
}if(qx.event.type.MouseWheel.MAXSCROLL==null||qx.event.type.MouseWheel.MAXSCROLL<o){qx.event.type.MouseWheel.MAXSCROLL=o;
this.__fH();
}if(qx.event.type.MouseWheel.MAXSCROLL===o&&qx.event.type.MouseWheel.MINSCROLL===o){return 2*(n/o);
}var p=qx.event.type.MouseWheel.MAXSCROLL-qx.event.type.MouseWheel.MINSCROLL;
var q=(n/p)*Math.log(p)*qx.event.type.MouseWheel.FACTOR;
return q<0?Math.min(q,-1):Math.max(q,1);
},__fH:function(){var r=qx.event.type.MouseWheel.MAXSCROLL||0;
var u=qx.event.type.MouseWheel.MINSCROLL||r;

if(r<=u){return;
}var s=r-u;
var t=(r/s)*Math.log(s);

if(t==0){t=1;
}qx.event.type.MouseWheel.FACTOR=6/t;
},getWheelDelta:function(v){var e=this._native;
if(v===undefined){if(w===undefined){var w=-e.wheelDelta;

if(e.wheelDelta===undefined){w=e.detail;
}}return this.__fI(w);
}if(v===k){var x=0;
...
},getAxis:function(){var e=this._native;
if(e.axis&&e.axis==e.HORIZONTAL_AXIS){return k;
}else if(!(e.axis&&e.axis==e.HORIZONTAL_AXIS)){return j;
}else{return f;
}},__fI:function(z){if(qx.core.Environment.get(c)){return this.__fG(z);
}else{var A=qx.core.Environment.select(i,{"default":function(){return z/40;
},"gecko":function(){return z;
},"webkit":function(){if(qx.core.Environment.get(h)==a){if(qx.core.Environment.get(l)==g){return z/60;
}else{return z/120;
}}else{if(qx.core.Environment.get(l)==d){var B=120;
if(parseFloat(qx.core.Environment.get(m))==533.16){B=1200;
}}else{B=40;
if(parseFloat(qx.core.Environment.get(m))==533.16||parseFloat(qx.core.Environment.get(m))==533.17||parseFloat(qx.core.Environment.get(m))==533.18){B=1200;
}}return z/B;
}}});
return A.call(this);
}}}});
...
(function(){var t="execute",s="",r="north",q="Control+S",p="log",o="horizontal",n="Control+R",m="Control+P",l="files",k="peers",d="stateChanged",j="value",h="changeSelection",c="Control+A",b="main",g="Show view: ",f="rhyacotriton.Container",i="Control+Shift+R",a="Control+D";
qx.Class.define(f,{extend:qx.ui.container.Composite,construct:function(){this.__kI=new qx.ui.layout.Dock();
qx.ui.container.Composite.call(this,this.__kI);
this._initializeCommands();
this._initStore();
this.__kJ=new rhyacotriton.Header();
this.add(this.__kJ,{edge:r});
this.__kK=new rhyacotriton.ToolBar(this);
this.add(this.__kK,{edge:r});
this.__cd=new qx.ui.container.Stack;
this.__cd.setDecorator(b);
this.__cd.resetSelection();
this.__cd.setWidth(200);
this.__cd.exclude();
this.__rO=new rhyacotriton.peers.Table(this.__kQ);
this.__kL=new qx.ui.embed.Html(s);
this.__kM=this.__rO;
this.__kN=new qx.ui.embed.Html(s);
this.__cd.add(this.__kL);
this.__cd.add(this.__kM);
this.__cd.add(this.__kN);
this.__kO=new rhyacotriton.Table(this.__kQ);
this.add(this.__kO);
this.__kP=new qx.ui.splitpane.Pane(o);
this.__kP.setDecorator(null);
this.add(this.__kP);
this.__kP.add(this.__kO,2);
this.__kP.add(this.__cd,1);
this._initToolbarButtonActivation();
this.setEnabled(false);
},members:{__kI:null,__kJ:null,__kK:null,__kQ:null,__kO:null,__kR:null,_initStore:function(){var u=this;
this.__kQ=new rhyacotriton.store.Remote;
this.__kQ.addListener(d,function(e){var v=e.getCurrentTarget().isActive();
u.setEnabled(v);
},this.__kQ);
},_initToolbarButtonActivation:function(){var w=this.__kO.getSelectionModel();
w.addListener(h,function(e){var x=!(e.getCurrentTarget().isSelectionEmpty());
this.enableRowButtons(x);
},this.__kK);
},removeSelectedRows:function(){this.__kO.removeSelectedRows();
},startSelectedRows:function(){this.__kO.startSelectedRows();
},stopSelectedRows:function(){this.__kO.stopSelectedRows();
},reconnect:function(){this.__kQ.reconnect();
},reload:function(){this.__kQ.reload();
},showAddTorrent:function(){},getCommand:function(y){return this.__kR[y];
},_initializeCommands:function(){var z={};
z.reconnect=new qx.ui.core.Command(i);
z.reconnect.addListener(t,this.reconnect,this);
z.reload=new qx.ui.core.Command(n);
z.reload.addListener(t,this.reload,this);
z.addTorrent=new qx.ui.core.Command(c);
z.addTorrent.addListener(t,this.showAddTorrent,this);
z.removeSelectedRows=new qx.ui.core.Command(a);
z.removeSelectedRows.addListener(t,this.removeSelectedRows,this);
z.startSelectedRows=new qx.ui.core.Command(q);
z.startSelectedRows.addListener(t,this.startSelectedRows,this);
z.stopSelectedRows=new qx.ui.core.Command(m);
z.stopSelectedRows.addListener(t,this.stopSelectedRows,this);
this.__kR=z;
},syncStackView:function(e){var A=e.getData()[0];
var B=A!=null?A.getUserData(j):s;
console.log(g+B);

switch(B){case l:this.__cd.setSelection([this.__kL]);
this.__cd.show();
break;
case k:this.__kQ.reloadPeers();
this.__cd.setSelection([this.__kM]);
this.__cd.show();
break;
case p:this.__cd.setSelection([this.__kN]);
this.__cd.show();
break;
default:this.__cd.resetSelection();
this.__cd.exclude();
}},setEnabled:function(C){this.__kK.setEnabled(C);
this.__kO.setEnabled(C);
}}});
})();
...
}},properties:{sort:{check:[l,m,n],init:l,apply:j},separatorX:{check:p,nullable:true,apply:s},separatorY:{check:p,nullable:true,apply:s},connectSeparators:{check:e,init:false,apply:s},spacingX:{check:o,init:0,apply:s},spacingY:{check:o,init:0,apply:s}},members:{__ff:null,__kS:null,verifyLayoutProperty:null,_applySort:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kT:{north:1,south:2,west:3,east:4,center:5},__kU:{1:q,2:d,3:r,4:g},__kV:function(){var x=this._getLayoutChildren();
var F,z;
var length=x.length;
var B=[];
var E=[];
...
},renderLayout:function(H,I){if(this._invalidChildrenCache){this.__kV();
...
}},destruct:function(){this.__kS=this.__ff=null;
}});
})();
(function(){var e="0.1",d="default",c="rhyacotriton.Header",b="app-header",a="Rhyacotriton";
...
}},properties:{alignX:{check:[m,k,c],init:m,apply:n},alignY:{check:[j,g,e],init:j,apply:n},spacing:{check:a,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:d,init:false,apply:f}},members:{__kW:null,__kX:null,__kY:null,__ff:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kV:function(){var w=this._getLayoutChildren();
var length=w.length;
var t=false;
var r=this.__kW&&this.__kW.length!=length&&this.__kX&&this.__kW;
var u;
var s=r?this.__kW:new Array(length);
...
this.__ff=w;
delete this._invalidChildrenCache;
},verifyLayoutProperty:null,renderLayout:function(x,y){if(this._invalidChildrenCache){this.__kV();
}var E=this.__ff;
var length=E.length;
...
for(i=0;i<length;i+=1){J=this.__kW[i];
K=J!=null?Math.floor((x-B)*J):E[i].getSizeHint().width;
P.push(K);
F+=K;
}if(this.__kY&&F!=x){var H={};
var L,O;

for(i=0;i<length;i+=1){L=this.__kX[i];
...
}},destruct:function(){this.__kW=this.__kX=this.__ff=null;
}});
})();
...
}else if(this.__la.length>0){do{var M=this.__la[0];
...
var K=false;
if(this.__la.length==1&&I>0){var D=L-this.getSpacing();
var H=B-I+F+D;
K=A>H;
}if(A>B+F+L||K){this.__lc(M);
B+=F;
if(C&&this.__la.length==0){C.setVisibility(v);
}}else{return;
}}}while(A>=B&&this.__la.length>0);
}},__lc:function(N){N.setVisibility(w);
this.__la.shift();
this.fireDataEvent(p,N);
},__ld:function(O){if(!O){return;
}this.__la.unshift(O);
O.setVisibility(v);
this.fireDataEvent(j,O);
},_getNextToHide:function(){for(var i=this.__lb.length-1;i>=0;i--){var P=this.__lb[i];
...
}}},setRemovePriority:function(S,T,U){if(!U&&this.__lb[T]!=undefined){throw new Error("Priority already in use!");
}this.__lb[T]=S;
...
}for(var i=0;i<this.__la.length;i++){this.__la[i].setVisibility(w);
}this.__la=[];
...
}},__le:false,_setAllowMenuOpenHover:function(bc){this.__le=bc;
},_isAllowMenuOpenHover:function(){return this.__le;
...
(function(){var u="keypress",t="interval",s="keydown",r="mousedown",q="keyup",p="__lk",o="blur",n="Enter",m="__lj",l="__jG",d="Up",k="Escape",h="event.touch",c="qx.ui.menu.Manager",b="Left",g="Down",f="Right",j="singleton",a="Space";
...
}this.__lj=new qx.event.Timer;
this.__lj.addListener(t,this._onOpenInterval,this);
this.__lk=new qx.event.Timer;
this.__lk.addListener(t,this._onCloseInterval,this);
},members:{__ll:null,__lm:null,__lj:null,__lk:null,__jG:null,_getChild:function(x,y,z,A){var B=x.getChildren();
var length=B.length;
var C;
...
if(L.isVisible()){if(this.__ll){this.cancelOpen(this.__ll);
}}else if(this.__ll!=L){this.__ll=L;
this.__lj.restartWith(L.getOpenInterval());
}},scheduleClose:function(M){this.cancelOpen(M);
if(!M.isVisible()){if(this.__lm){this.cancelClose(this.__lm);
}}else if(this.__lm!=M){this.__lm=M;
this.__lk.restartWith(M.getCloseInterval());
}},cancelOpen:function(N){if(this.__ll==N){this.__lj.stop();
this.__ll=null;
}},cancelClose:function(O){if(this.__lm==O){this.__lk.stop();
this.__lm=null;
}},_onOpenInterval:function(e){this.__lj.stop();
this.__ll.open();
this.__ll=null;
},_onCloseInterval:function(e){this.__lk.stop();
this.__lm.exclude();
this.__lm=null;
...
}},__ln:{"Enter":1,"Space":1},__lo:{"Escape":1,"Up":1,"Down":1,"Left":1,"Right":1},_onKeyUpDown:function(e){var Q=this.getActiveMenu();

if(!Q){return;
}var R=e.getKeyIdentifier();

if(this.__lo[R]||(this.__ln[R]&&Q.getSelectedButton())){e.stopPropagation();
...
},__ls:function(){if(this.isVisible()){if(this.getBlockBackground()){var R=this.getZIndex();
...
},__lt:function(){var bj=this._getMenuBounds();

if(!bj){this.addListenerOnce(j,this.__lt,this);
return;
...
if(this.__lr){this.__lr.call(this);
delete this.__lr;
...
if(this.__lq){bo.cancelOpen(this.__lq);
this.__lq=null;
...
}if(this.__lq){br.cancelOpen(this.__lq);
...
if(q){this.setSeparator(q);
}},properties:{alignY:{check:[m,j,e],init:m,apply:n},alignX:{check:[k,g,a],init:k,apply:n},spacing:{check:c,init:0,apply:n},separator:{check:h,nullable:true,apply:n},reversed:{check:b,init:false,apply:f}},members:{__lu:null,__kX:null,__kY:null,__ff:null,_applyReversed:function(){this._invalidChildrenCache=true;
this._applyLayoutChange();
},__kV:function(){var w=this._getLayoutChildren();
var length=w.length;
var s=false;
var r=this.__lu&&this.__lu.length!=length&&this.__kX&&this.__lu;
var u;
...
K.push(B);
Q+=B;
}if(this.__kY&&Q!=y){var H={};
var N,R;

for(i=0;i<length;i+=1){N=this.__kX[i];
...
}},destruct:function(){this.__lu=this.__kX=this.__ff=null;
}});
})();
(function(){var c="Integer",b="_applyLayoutChange",a="qx.ui.menu.Layout";
qx.Class.define(a,{extend:qx.ui.layout.VBox,properties:{columnSpacing:{check:c,init:0,apply:b},spanColumn:{check:c,init:1,nullable:true,apply:b},iconColumnWidth:{check:c,init:0,themeable:true,apply:b},arrowColumnWidth:{check:c,init:0,themeable:true,apply:b}},members:{__lv:null,_computeSizeHint:function(){var q=this._getLayoutChildren();
var o,g,j;
var e=this.getSpanColumn();
var h=this.__lv=[0,0,0,0];
...
},getColumnSizes:function(){return this.__lv||null;
...
}},__lw:function(t){while(!(t instanceof qx.ui.menu.Menu)){t=t.getLayoutParent();
}return t;
...
(function(){var v="horizontal",u="scrollpane",t="vertical",s="button-backward",r="button-forward",q="content",p="x",o="y",n="execute",m="qx.ui.container.SlideBar",f="engine.version",l="engine.name",i="removeChildWidget",c="scrollX",b="scrollY",h="_applyOrientation",g="mousewheel",j="gecko",a="Integer",k="slidebar",d="update";
qx.Class.define(m,{extend:qx.ui.core.Widget,include:[qx.ui.core.MRemoteChildrenHandling,qx.ui.core.MRemoteLayoutHandling],construct:function(w){qx.ui.core.Widget.call(this);
...
},properties:{appearance:{refine:true,init:k},orientation:{check:[v,t],init:v,apply:h},scrollStep:{check:a,init:15,themeable:true}},members:{getChildrenContainer:function(){return this.getChildControl(q);
},_createChildControlImpl:function(y,z){var A;

switch(y){case r:A=new qx.ui.form.RepeatButton;
A.addListener(n,this._onExecuteForward,this);
A.setFocusable(false);
this._addAt(A,2);
break;
case s:A=new qx.ui.form.RepeatButton;
A.addListener(n,this._onExecuteBackward,this);
A.setFocusable(false);
this._addAt(A,0);
break;
case q:A=new qx.ui.container.Composite();
if(qx.core.Environment.get(l)==j&&parseInt(qx.core.Environment.get(f))<2){A.addListener(i,this._onRemoveChild,this);
}this.getChildControl(u).add(A);
break;
case u:A=new qx.ui.core.scroll.ScrollPane();
A.addListener(d,this._onResize,this);
A.addListener(c,this._onScroll,this);
A.addListener(b,this._onScroll,this);
break;
...
if(this.getOrientation()===v&&e.getAxis()==p){Q=e.getWheelDelta(p);
var M=P.getScrollX();
var N=P.getScrollMaxX();
var O=parseInt(Q);
if(!(O<0&&M<=0||O>0&&M>=N)){e.stop();
}}else if(this.getOrientation()===t&&e.getAxis()==o){Q=e.getWheelDelta(o);
var M=P.getScrollY();
...
}this.getChildControl(s).setEnabled(V>0);
this.getChildControl(r).setEnabled(V<X);
...
s.addListener(b,this.__lB,this);
this._add(s);
break;
...
},__lB:function(){var w=this.getChildrenContainer().getChildren();
...
(function(){var j="value",i="icon/22/apps/utilities-log-viewer.png",h="reconnect",g="startSelectedRows",f="log",e="Log",d="files",c="rhyacotriton.ToolBar",b="icon/22/actions/media-playback-start.png",a="Reconnect",E="stopSelectedRows",D="Pause",C="changeSelection",B="peers",A="Files",z="removeSelectedRows",y="icon/22/actions/view-refresh.png",w="reload",v="icon/22/actions/list-remove.png",u="Remove",q="icon/22/places/folder-open.png",r="Peers",o="Add",p="Start",m="icon/22/actions/list-add.png",n="icon/22/actions/media-playback-pause.png",k="addTorrent",l="icon/22/apps/preferences-users.png",s="Refresh",t="icon/22/devices/network-wired.png";
qx.Class.define(c,{extend:qx.ui.toolbar.ToolBar,construct:function(F){qx.ui.toolbar.ToolBar.call(this);
this.__lC=new qx.ui.toolbar.Button(this.tr(a),t);
this.__lC.setCommand(F.getCommand(h));
this.add(this.__lC);
this.__lD=new qx.ui.toolbar.Button(this.tr(s),y);
this.__lD.setCommand(F.getCommand(w));
this.add(this.__lD);
this.__lE=new qx.ui.toolbar.Button(this.tr(o),m);
this.__lE.setCommand(F.getCommand(k));
this.add(this.__lE);
this.__lF=new qx.ui.toolbar.Button(this.tr(u),v);
this.__lF.setCommand(F.getCommand(z));
this.add(this.__lF);
this.__lG=new qx.ui.toolbar.Button(this.tr(p),b);
this.__lG.setCommand(F.getCommand(g));
this.add(this.__lG);
this.__lH=new qx.ui.toolbar.Button(this.tr(D),n);
this.__lH.setCommand(F.getCommand(E));
this.add(this.__lH);
this.addSpacer();
this.__kM=new qx.ui.toolbar.RadioButton(this.tr(r),l);
this.__kM.setUserData(j,B);
this.add(this.__kM);
this.__kL=new qx.ui.toolbar.RadioButton(this.tr(A),q);
this.__kL.setUserData(j,d);
this.add(this.__kL);
this.__kN=new qx.ui.toolbar.RadioButton(this.tr(e),i);
this.__kN.setUserData(j,f);
this.add(this.__kN);
this.__lI=new qx.ui.form.RadioGroup;
this.__lI.setAllowEmptySelection(true);
this.__lI.add(this.__kN);
this.__lI.add(this.__kL);
this.__lI.add(this.__kM);
this.__lI.addListener(C,F.syncStackView,F);
this.enableRowButtons(false);
},members:{__lJ:true,__lK:function(G){[this.__lF,this.__lG,this.__lH].map(function(x){x.setEnabled(G);
});
},enableRowButtons:function(H){this.__lJ=H;
this.__lK(H);
},setEnabled:function(I){[this.__lE,this.__lD].map(function(x){x.setEnabled(I);
});

if(I==this.__lJ)this.__lK(I);
}}});
})();
...
qx.Mixin.define(f,{events:{"changeSelection":a},members:{__eI:null,getSelection:function(){var g=this.__lL().getSelected();
...
},__lL:function(){if(this.__eI==null){var m=this;
...
(function(){var g="Boolean",f="qx.ui.core.SingleSelectionManager",e="__lM",d="changeSelected",c="__lO",b="__lN",a="qx.event.type.Data";
qx.Class.define(f,{extend:qx.core.Object,construct:function(h){qx.core.Object.call(this);
this.__lM=h;
},events:{"changeSelected":a},properties:{allowEmptySelection:{check:g,init:true,apply:c}},members:{__lN:null,__lM:null,getSelected:function(){return this.__lN;
},setSelected:function(j){if(!this.__lQ(j)){throw new Error("Could not select "+j+", because it is not a child element!");
}this.__lP(j);
},resetSelected:function(){this.__lP(null);
},isSelected:function(k){if(!this.__lQ(k)){throw new Error("Could not check if "+k+" is selected,"+" because it is not a child element!");
}return this.__lN===k;
},isSelectionEmpty:function(){return this.__lN==null;
},getSelectables:function(l){var m=this.__lM.getItems();
var n=[];

for(var i=0;i<m.length;i++){if(this.__lM.isItemSelectable(m[i])){n.push(m[i]);
}}if(!l){for(var i=n.length-1;i>=0;i--){if(!n[i].getEnabled()){n.splice(i,1);
}}}return n;
},__lO:function(o,p){if(!o){this.__lP(this.__lN);
}},__lP:function(q){var t=this.__lN;
var s=q;
...
},__lQ:function(u){var v=this.__lM.getItems();

for(var i=0;i<v.length;i++){if(v[i]===u){return true;
}}return false;
}},destruct:function(){if(this.__lM.toHashCode){this._disposeObjects(e);
}else{this.__lM=null;
}this._disposeObjects(b);
}});
})();
(function(){var f="change",d="__lR",c="qx.event.type.Data",b="qx.ui.form.MModelSelection",a="changeSelection";
qx.Mixin.define(b,{construct:function(){this.__lR=new qx.data.Array();
this.__lR.addListener(f,this.__lU,this);
this.addListener(a,this.__lT,this);
},events:{changeModelSelection:c},members:{__lR:null,__lS:false,__lT:function(){if(this.__lS){return;
...
}}},__lU:function(){this.__lS=true;
var n=this.getSelectables(true);
var p=[];
var o=this.__lR.toArray();
...
if(!qx.lang.Array.equals(q,p)){this.__lT();
}},getModelSelection:function(){return this.__lR;
},setModelSelection:function(t){if(!t){this.__lR.removeAll();
return;
}t.unshift(this.__lR.getLength());
t.unshift(0);
var u=this.__lR.splice.apply(this.__lR,t);
u.dispose();
}},destruct:function(){this._disposeObjects(d);
}});
})();
...
},_registerEventChaining:function(m,n,name){if((m instanceof qx.core.Object)&&qx.Class.hasMixin(m.constructor,qx.data.marshal.MEventBubbling)){var o=qx.lang.Function.bind(this.__lV,this,name);
...
}},__lV:function(name,e){var y=e.getData();
var u=y.value;
var s=y.old;
...
if(x<v){var r=y.name.substring(0,x);
...
}}else{if(qx.Class.hasInterface(this.constructor,qx.data.IListData)&&name!==b){name=j+name+h;
}var t=name+f+y.name;
...
if(p==undefined){this.__lW=[];
}else if(arguments.length>1){this.__lW=[];

for(var i=0;i<arguments.length;i++){this.__lW.push(arguments[i]);
}}else if(typeof p==c){this.__lW=new Array(p);
}else if(p instanceof Array){this.__lW=qx.lang.Array.clone(p);
}else{this.__lW=[];
this.dispose();
throw new Error("Type of the parameter not supported!");
}for(var i=0;i<this.__lW.length;i++){this._applyEventPropagation(this.__lW[i],null,i);
}this.__lX();
},properties:{autoDisposeItems:{check:e,init:false}},events:{"change":f,"changeLength":f},members:{__lW:null,concat:function(q){if(q){var r=this.__lW.concat(q);
}else{var r=this.__lW.concat();
}return new qx.data.Array(r);
},join:function(s){return this.__lW.join(s);
},pop:function(){var t=this.__lW.pop();
this.__lX();
...
return t;
},push:function(u){for(var i=0;i<arguments.length;i++){this.__lW.push(arguments[i]);
this.__lX();
...
}var v=this.__lW.concat();
this.__lW.reverse();
this.fireDataEvent(o,{start:0,end:this.length-1,type:j,items:null},null);
this.fireDataEvent(n,{value:this.__lW,name:k+(this.__lW.length-1),old:v});
},shift:function(){if(this.length==0){return;
}var w=this.__lW.shift();
this.__lX();
...
return w;
},slice:function(x,y){return new qx.data.Array(this.__lW.slice(x,y));
},splice:function(z,A,B){var J=this.__lW.length;
var F=this.__lW.splice.apply(this.__lW,arguments);
if(this.__lW.length!=J){this.__lX();
}var H=A>0;
var D=arguments.length>2;
var E=null;

if(H||D){if(this.__lW.length>J){var I=m;
}else if(this.__lW.length<J){var I=l;
E=F;
}else{var I=j;
...
}var L=this.__lW.concat();
this.__lW.sort.apply(this.__lW,arguments);
this.fireDataEvent(o,{start:0,end:this.length-1,type:j,items:null},null);
this.fireDataEvent(n,{value:this.__lW,name:k+(this.length-1),old:L});
},unshift:function(M){for(var i=arguments.length-1;i>=0;i--){this.__lW.unshift(arguments[i]);
this.__lX();
this._registerEventChaining(arguments[i],null,0);
this.fireDataEvent(n,{value:[this.__lW[0]],name:g,old:[this.__lW[1]]});
this.fireDataEvent(o,{start:0,end:this.length-1,type:m,items:[arguments[i]]},null);
}return this.length;
},toArray:function(){return this.__lW;
},getItem:function(N){return this.__lW[N];
},setItem:function(O,P){var Q=this.__lW[O];
if(Q===P){return;
}this.__lW[O]=P;
this._registerEventChaining(P,Q,O);
if(this.length!=this.__lW.length){this.__lX();
}this.fireDataEvent(n,{value:[P],name:O,old:[Q]});
this.fireDataEvent(o,{start:O,end:O,type:m,items:[P]},null);
},getLength:function(){return this.length;
},indexOf:function(R){return this.__lW.indexOf(R);
},toString:function(){if(this.__lW!=null){return this.__lW.toString();
}return b;
},contains:function(S){return this.__lW.indexOf(S)!==-1;
...
}Array.prototype.push.apply(this.__lW,bh);
for(var i=0;i<bh.length;i++){this._registerEventChaining(bh[i],null,this.__lW.length+i);
}var bi=this.length;
this.__lX();
...
},forEach:function(bp,bq){for(var i=0;i<this.__lW.length;i++){bp.call(bq,this.__lW[i],i,this);
}},__lX:function(){var br=this.length;
this.length=this.__lW.length;
this.fireDataEvent(a,this.length,br);
}},destruct:function(){for(var i=0;i<this.__lW.length;i++){var bs=this.__lW[i];
...
(function(){var r="Boolean",q="changeInvalidMessage",p="changeValue",o="String",n="_applyAllowEmptySelection",m="_applyInvalidMessage",k="qx.ui.form.RadioGroup",j="_applyValid",h="",g="changeRequired",c="changeValid",f="changeEnabled",d="changeSelection",b="_applyEnabled",a="__lY";
...
}},remove:function(w){var x=this.__lY;
...
}}},getChildren:function(){return this.__lY;
...
}},_applyInvalidMessage:function(z,A){for(var i=0;i<this.__lY.length;i++){this.__lY[i].setInvalidMessage(z);
}},_applyValid:function(B,C){for(var i=0;i<this.__lY.length;i++){this.__lY[i].setValid(B);
}},_applyEnabled:function(D,E){var F=this.__lY;
...
},_isItemSelectable:function(O){return this.__lY.indexOf(O)!=-1;
},__ma:function(e){var Q=e.getData()[0];
...
this._setLayout(new qx.ui.layout.Grow);
this.addListener(c,this.__ma,this);
...
},__ma:function(e){var m=e.getOldData()[0];
var n=e.getData()[0];
...
(function(){var p="Boolean",o="column-button",n="Function",m="qx.event.type.Data",k="statusbar",h="qx.ui.table.pane.CellEvent",g="function",f="PageUp",e="dataChanged",d="__ml",bH="changeLocale",bG="changeSelection",bF="appear",bE="qx.dynlocale",bD='"',bC="Enter",bB="metaDataChanged",bA="_applyStatusBarVisible",bz="columnVisibilityMenuCreateStart",by="blur",w="qx.ui.table.Table",y="columnVisibilityMenuCreateEnd",u="changeVisible",v="_applyResetSelectionOnHeaderClick",s="_applyMetaColumnCounts",t="focus",q="changeDataRowRenderer",r="changeHeaderCellHeight",F="Escape",G="A",ba="changeSelectionModel",V="Left",bi="Down",bd="Integer",bu="_applyHeaderCellHeight",bo="visibilityChanged",O="qx.ui.table.ITableModel",bx="orderChanged",bw="_applySelectionModel",bv="menu-button",M="menu",R="_applyAdditionalStatusBarText",T="_applyFocusCellOnMouseMove",X="table",bb="_applyColumnVisibilityButtonVisible",be="__mb",bk="changeTableModel",bq="qx.event.type.Event",z="tableWidthChanged",A="_applyHeaderCellsVisible",Q="Object",bh="__jj",bg="_applyShowCellFocusIndicator",bf="resize",bm="verticalScrollBarChanged",bl="changeScrollY",bc="_applyTableModel",bj="End",a="__mk",bp="_applyKeepFirstVisibleRowComplete",B="widthChanged",C="one of one row",W="Home",b="_applyRowHeight",c="F2",L="Up",D="%1 rows",E="qx.ui.table.selection.Model",K="one row",Y="__mj",bs="PageDown",br="%1 of %2 rows",S="__mc",bt="keypress",N="changeRowHeight",bn="Number",H="header",J="_applyContextMenuFromDataCellsOnly",P="qx.ui.table.IRowRenderer",U="Right",I="Space";
...
}}},members:{__md:null,__me:null,__mb:null,__mc:null,__mg:null,__mh:null,__mi:null,__mj:null,__mk:null,__ml:null,__mm:null,__jj:null,_createChildControlImpl:function(bS,bT){var bU;
...
},_applySelectionModel:function(bW,bX){this.__mc.setSelectionModel(bW);
...
}},getTableColumnModel:function(){if(!this.__mk){var cn=this.__mk=this.getNewTableColumnModel()(this);
...
}},_applyAdditionalStatusBarText:function(cr,cs){this.__mg=cr;
this._updateStatusBar();
...
for(var cG in qx.ui.table.Table.__mf){cB[cG]={};
...
for(cG in qx.ui.table.Table.__mf){if(!cB[cG]){break;
}
...
if(this.__me>=dq&&this.__me<(dq+dr)){this.setFocusedCell();
}}
for(var i=0;i<ds.length;i++){ds[i].onTableModelDataChanged(dl,dm,dn,dp);
}var dt=this.getTableModel().getRowCount();

if(dt!=this.__mh){this.__mh=dt;
this._updateScrollBarVisibility();
this._updateStatusBar();
}},_onScrollY:function(du){if(!this.__mi){this.__mi=true;
...
if(this.__me!=dD){dA=this.startEditing();
}break;
...
default:dA=false;
break;
}}else{switch(dE){case I:this.__mc.handleSelectKeyDown(this.__me,dw);
break;
case c:case bC:this.startEditing();
dA=true;
break;
case W:this.setFocusedCell(this.__md,0,true);
break;
case bj:var dB=this.getTableModel().getRowCount();
this.setFocusedCell(this.__md,dB-1,true);
break;
...
if(dD!=this.__me&&this.getRowFocusChangeModifiesSelection()){this.__mc.handleMoveKeyDown(this.__me,dw);
}
...
if(this.__mj!=null&&dJ.col!=null&&dJ.visible!=null){this.__mj[dJ.col].setVisible(dJ.visible);
}this._updateScrollerWidths();
this._updateScrollBarVisibility();
...
},setFocusedCell:function(dR,dS,dT){if(!this.isEditing()&&(dR!=this.__md||dS!=this.__me)){if(dR===null){dR=0;
}this.__md=dR;
this.__me=dS;
...
}},moveFocusedCell:function(ea,eb){var ef=this.__md;
var eg=this.__me;
...
if(this.__mg){if(eD){eD+=this.__mg;
}else{eD=this.__mg;
}}
...
if(eP){if(this.__mm==null){this.__mm=eJ[i].getVerticalScrollBarVisible();
this.__jj=qx.event.Timer.once(function(){this.__mm=null;
this.__jj=null;
},this,0);
}}eJ[i].setVerticalScrollBarVisible(eP&&eN);
if(eP&&eN!=this.__mm){this.fireDataEvent(bm,eN);
...
this._updateScrollerWidths();
this._updateScrollBarVisibility();
},addListener:function(fd,fe,self,ff){if(this.self(arguments).__mf[fd]){var fh=[fd];
...
}},removeListener:function(fi,fj,self,fk){if(this.self(arguments).__mf[fi]){for(var i=0,fl=this._getPaneScrollerArr();i<fl.length;i++){fl[i].removeListener.apply(fl[i],arguments);
}}else{qx.ui.core.Widget.prototype.removeListener.call(this,fi,fj,self,fk);
}},removeListenerById:function(fm){var fq=fm.split(bD);
var fp=fq.shift();

if(this.self(arguments).__mf[fp]){var fo=true;
...
this._disposeObjects(S,be,d,d,a,bh);
this._disposeMap(Y);
}});
})();
...
},properties:{highlightFocusRow:{check:h,init:true}},members:{_colors:null,__mo:null,__mn:null,_insetY:1,_renderFont:function(x){if(x){this.__mo=x.getStyles();
this.__mn=qx.bom.element.Style.compile(this.__mo);
this.__mn=this.__mn.replace(/"/g,d);
}else{this.__mn=j;
this.__mo=qx.bom.Font.getDefaultStyles();
}},updateDataRowElement:function(y,z){var B=this.__mo;
var A=z.style;
...
}},destruct:function(){this._colors=this.__mo=this.__mn=null;
}});
})();
...
},members:{__mj:null,__ka:null,factory:function(g,h){switch(g){case a:var j=new qx.ui.menu.Menu();
this.setMenu(j);
return j;
...
},members:{__mp:false,_applyVisible:function(h,i){if(!this.bInListener){this.setValue(h);
}}}});
})();
(function(){var b="qx.ui.table.selection.Model",a="qx.ui.table.selection.Manager";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
},properties:{selectionModel:{check:b}},members:{__mq:null,handleMouseDown:function(c,d){if(d.isLeftPressed()){var e=this.getSelectionModel();
...
}}},handleMouseUp:function(f,g){if(g.isLeftPressed()&&!this.__mq){this._handleSelectEvent(f,g);
...
this.__mu=false;
},events:{"changeSelection":f},statics:{NO_SELECTION:1,SINGLE_SELECTION:2,SINGLE_INTERVAL_SELECTION:3,MULTIPLE_INTERVAL_SELECTION:4,MULTIPLE_INTERVAL_SELECTION_TOGGLE:5},properties:{selectionMode:{init:2,check:[1,2,3,4,5],apply:a}},members:{__mu:null,__ms:null,__mt:null,__mr:null,_applySelectionMode:function(o){this.resetSelection();
},setBatchMode:function(p){if(p){this.hasBatchModeRefCount+=1;
}else{if(this.hasBatchModeRefCount==0){throw new Error("Try to turn off batch mode althoug it was not turned on.");
}this.hasBatchModeRefCount-=1;

if(this.__mu){this.__mu=false;
this._fireChangeSelection();
}}return this.hasBatchMode();
},hasBatchMode:function(){return this.hasBatchModeRefCount>0;
},getAnchorSelectionIndex:function(){return this.__ms;
},_setAnchorSelectionIndex:function(q){this.__ms=q;
},getLeadSelectionIndex:function(){return this.__mt;
},_setLeadSelectionIndex:function(r){this.__mt=r;
},_getSelectedRangeArr:function(){return this.__mr;
},resetSelection:function(){if(!this.isSelectionEmpty()){this._resetSelection();
this._fireChangeSelection();
}},isSelectionEmpty:function(){return this.__mr.length==0;
},getSelectedCount:function(){var t=0;

for(var i=0;i<this.__mr.length;i++){var s=this.__mr[i];
t+=s.maxIndex-s.minIndex+1;
}return t;
},isSelectedIndex:function(u){for(var i=0;i<this.__mr.length;i++){var v=this.__mr[i];

if(u>=v.minIndex&&u<=v.maxIndex){return true;
}}return false;
},getSelectedRanges:function(){var w=[];

for(var i=0;i<this.__mr.length;i++){w.push({minIndex:this.__mr[i].minIndex,maxIndex:this.__mr[i].maxIndex});
}return w;
},iterateSelection:function(x,y){for(var i=0;i<this.__mr.length;i++){for(var j=this.__mr[i].minIndex;j<=this.__mr[i].maxIndex;j++){x.call(y,j);
...
break;
}},removeSelectionInterval:function(F,G){this.__ms=F;
this.__mt=G;
var H=Math.min(F,G);
var J=Math.max(F,G);
for(var i=0;i<this.__mr.length;i++){var L=this.__mr[i];

if(L.minIndex>J){break;
}else if(L.maxIndex>=H){var M=(L.minIndex>=H)&&(L.minIndex<=J);
var K=(L.maxIndex>=H)&&(L.maxIndex<=J);

if(M&&K){this.__mr.splice(i,1);
i--;
}else if(M){L.minIndex=J+1;
}else if(K){L.maxIndex=H-1;
}else{var I={minIndex:J+1,maxIndex:L.maxIndex};
this.__mr.splice(i+1,0,I);
L.maxIndex=H-1;
break;
}}}this._fireChangeSelection();
},_resetSelection:function(){this.__mr=[];
this.__ms=-1;
this.__mt=-1;
},_addSelectionInterval:function(N,O){this.__ms=N;
this.__mt=O;
var P=Math.min(N,O);
var R=Math.max(N,O);
var Q=0;

for(;Q<this.__mr.length;Q++){var S=this.__mr[Q];

if(S.minIndex>P){break;
}}this.__mr.splice(Q,0,{minIndex:P,maxIndex:R});
var T=this.__mr[0];

for(var i=1;i<this.__mr.length;i++){var S=this.__mr[i];

if(T.maxIndex+1>=S.minIndex){T.maxIndex=Math.max(T.maxIndex,S.maxIndex);
this.__mr.splice(i,1);
i--;
}else{T=S;
}}},_dumpRanges:function(){var U=d;

for(var i=0;i<this.__mr.length;i++){var V=this.__mr[i];
U+=k+V.minIndex+m+V.maxIndex+n;
}this.debug(U);
},_fireChangeSelection:function(){if(this.hasBatchMode()){this.__mu=true;
...
if(this.__my in qx.util.format.DateFormat.ISO_MASKS){if(this.__my===d){this.__mz=true;
}this.__my=qx.util.format.DateFormat.ISO_MASKS[this.__my];
}}else{this.__my=qx.locale.Date.getDateFormat(bd,this.__iz)+N+qx.locale.Date.getDateTimeFormat(dq,dl,this.__iz);
...
},ASSUME_YEAR_2000_THRESHOLD:30,LOGGING_DATE_TIME__format:da,ISO_MASKS:{isoDate:"yyyy-MM-dd",isoTime:"HH:mm:ss",isoDateTime:"yyyy-MM-dd'T'HH:mm:ss",isoUtcDateTime:"yyyy-MM-dd'T'HH:mm:ss'Z'"},AM_MARKER:"am",PM_MARKER:"pm"},members:{__mw:null,__iz:null,__mx:null,__my:null,__mA:null,__mB:null,__mC:null,__mz:null,__mD:function(dy,dz){var dA=cG+(dy<0?((-1)*dy):dy);

while(dA.length<dz){dA=bH+dA;
}return dy<0?cF+dA:dA;
},__mE:function(dB){var dC=new Date(dB.getTime());
var dD=dC.getDate();

while(dC.getMonth()!=0){dC.setDate(-1);
dD+=dC.getDate()+1;
}return dD;
},__mF:function(dE){return new Date(dE.getTime()+(3-((dE.getDay()+6)%7))*86400000);
},__mG:function(dF){var dH=this.__mF(dF);
var dI=dH.getFullYear();
var dG=this.__mF(new Date(dI,0,4));
return Math.floor(1.5+(dH.getTime()-dG.getTime())/86400000/7);
},__mH:function(dJ){var dL=this.__mF(dJ);
var dK=this.__mF(new Date(dJ.getFullYear(),dJ.getMonth(),4));
return Math.floor(1.5+(dL.getTime()-dK.getTime())/86400000/7);
},__mI:function(dM){var dN=this.__mF(dM);
return dN.getFullYear();
},__mJ:function(dO){var dP=new Date(dO,2,1);
dP.setDate(-1);
return dP.getDate()+1===29;
},__mK:function(dQ,dR){var dV=0;
var dW=0;
if(!dR){dR=1971;
}var dS=0;
...
},__mL:function(dX,dY,ea){var eb;

switch(dY){case 11:eb=dX-1;

if(dX!=this.__mI(new Date(eb,dY,ea))){eb=dX;
}break;
case 0:eb=dX+1;
...
},_applyLocale:function(ec,ed){this.__iz=ec===null?this.setLocale(this.__mx):ec;
},format:function(ee){if(ee==null){return null;
}
if(isNaN(ee.getTime())){return null;
}
if(this.__mz){ee=new Date(ee.getUTCFullYear(),ee.getUTCMonth(),ee.getUTCDate(),ee.getUTCHours(),ee.getUTCMinutes(),ee.getUTCSeconds(),ee.getUTCMilliseconds());
}var eh=this.__iz;
...
for(var i=0;i<this.__mC.length;i++){var en=this.__mC[i];
...
switch(eg){case x:if(er==2){ez=this.__mD(em%100,2);
...
case cl:if(er==1||er==2){ez=this.__mD(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case U:if(er==1||er==2){ez=this.__mD(parseInt(ei/4)+1,er);
}
if(er==3){ez=cl+(parseInt(ei/4)+1);
}break;
case cz:ez=this.__mD(this.__mE(ee),er);
break;
case dc:ez=this.__mD(el,er);
break;
case A:ez=this.__mD(this.__mG(ee),er);
break;
case E:ez=this.__mH(ee);
break;
...
if(er>=1&&er<=2){ez=this.__mD(eu,er);
...
case H:if(er==1||er==2){ez=this.__mD(ei+1,er);
...
case t:if(er==1||er==2){ez=this.__mD(ei+1,er);
...
break;
case ca:ez=this.__mD(es,er);
break;
case cu:ez=this.__mD((es==0)?24:es,er);
break;
case cy:ez=this.__mD(es%12,er);
break;
case bT:ez=this.__mD(((es%12)==0)?12:(es%12),er);
break;
case m:ez=this.__mD(ey,er);
break;
case bR:ez=this.__mD(eq,er);
break;
case cb:ez=ew+cG;
...
case bb:if(er>=1&&er<=4){ez=dm+((et>0)?cF:cO)+this.__mD(Math.abs(ef),2)+w+this.__mD(ev,2);
}break;
case K:if(er>=1&&er<=3){ez=((et>0)?cF:cO)+this.__mD(Math.abs(ef),2)+this.__mD(ev,2);
}else{ez=dm+((et>0)?cF:cO)+this.__mD(Math.abs(ef),2)+w+this.__mD(ev,2);
}break;
}ex+=ez;
}}return ex;
},parse:function(eA){this.__mN();
var eJ=this.__mA.regex.exec(eA);
...
for(var i=0;i<this.__mA.usedRules.length;i++){var eF=this.__mA.usedRules[i];
var eG=eJ[eD];
...
for(var k=0;k<this.__mA.usedRules.length;k++){if(this.__mA.usedRules[k].pattern==dr){eH=true;
break;
}}
if(!eH){eC=true;
}}
...
for(var k=0;k<this.__mA.usedRules.length;k++){if(this.__mA.usedRules[k].pattern.indexOf(bi)!=-1){eL=true;
break;
}}
if(!eL){eE=true;
}}eD+=(eF.groups==null)?1:eF.groups;
}
if(eC){eB.year=this.__mL(eB.weekYear,eB.month,eB.day);
}
if(eE){var eK=this.__mK(eB.dayOfYear,eB.year);
eB.month=eK.month;
eB.day=eK.day;
}
...
if(this.__mz){eI=new Date(eI.getUTCFullYear(),eI.getUTCMonth(),eI.getUTCDate(),eI.getUTCHours(),eI.getUTCMinutes(),eI.getUTCSeconds(),eI.getUTCMilliseconds());
}
...
},__mM:function(){if(this.__mC!=null){return;
}this.__mC=[];
var eQ;
var eO=0;
var eS=cG;
...
case bF:if(eR==eQ){eO++;
i++;
}else{this.__mC.push({type:bF,character:eQ,size:eO});
eQ=null;
eO=0;
eP=bI;
}break;
...
if(eP!=bI){if(eS.length>0){this.__mC.push({type:bG,text:eS});
eS=cG;
}}else{eS+=eR;
i++;
}break;
}}if(eQ!=null){this.__mC.push({type:bF,character:eQ,size:eO});
}else if(eS.length>0){this.__mC.push({type:bG,text:eS});
}},__mN:function(){if(this.__mA!=null){return ;
}var eW=this.__my;
this.__mP();
this.__mM();
var fd=[];
...
for(var eU=0;eU<this.__mC.length;eU++){var fe=this.__mC[eU];
...
for(var ff=0;ff<this.__mB.length;ff++){var fb=this.__mB[ff];

if(this.__mO(fb,eV,fa)){eX=fb;
break;
...
}this.__mA={regex:eT,"usedRules":fd,pattern:eY};
},__mO:function(fh,fi,fj){if(fi===x&&fh.pattern===dr){fh.regex=fh.regexFunc(fj);
return true;
}else if(fi===be&&fh.pattern===bw){fh.regex=fh.regexFunc(fj);
return true;
}else{return fi==fh.pattern.charAt(0)&&fj==fh.pattern.length;
}},__mP:function(){var fx=qx.util.format.DateFormat;
...
}this.__mC=this.__mA=this.__mB=null;
}});
})();
...
qx.Class.define(l,{statics:{__mQ:qx.locale.Manager.getInstance(),getAmMarker:function(F){return this.__mQ.localize(o,[],F);
},getPmMarker:function(G){return this.__mQ.localize(p,[],G);
},getDayNames:function(length,H,I,J){var I=I?I:h;
var L=[B,C,D,a,e,E,d];
var M=[];

for(var i=0;i<L.length;i++){var K=c+I+k+length+k+L[i];
M.push(J?this.__mR(I,I===j?f:j,K,H):this.__mQ.localize(K,[],H));
}return M;
},getDayName:function(length,N,O,P,Q){var P=P?P:h;
var S=[B,C,D,a,e,E,d];
var R=c+P+k+length+k+S[N];
return Q?this.__mR(P,P===j?f:j,R,O):this.__mQ.localize(R,[],O);
},getMonthNames:function(length,T,U,V){var U=U?U:h;
var X=[];

for(var i=0;i<12;i++){var W=b+U+k+length+k+(i+1);
X.push(V?this.__mR(U,U===j?f:j,W,T):this.__mQ.localize(W,[],T));
}return X;
},getMonthName:function(length,Y,ba,bb,bc){var bb=bb?bb:h;
var bd=b+bb+k+length+k+(Y+1);
return bc?this.__mR(bb,bb===j?f:j,bd,ba):this.__mQ.localize(bd,[],ba);
},getDateFormat:function(be,bf){var bg=v+be;
return this.__mQ.localize(bg,[],bf);
},getDateTimeFormat:function(bh,bi,bj){var bl=m+bh;
var bk=this.__mQ.localize(bl,[],bj);

if(bk==bl){bk=bi;
}return bk;
},getTimeFormat:function(bm,bn){var bp=t+bm;
var bo=this.__mQ.localize(bp,[],bn);
...
}else{bE=this.__mQ.getTerritory()||this.__mQ.getLanguage();
}return bE.toUpperCase();
},__mR:function(bF,bG,bH,bI){var bK=this.__mQ.localize(bH,[],bI);

if(bK==bH){var bJ=bH.replace(g+bF+g,g+bG+g);
return this.__mQ.localize(bJ,[],bI);
}else{return bK;
}}}});
})();
...
qx.Mixin.define(d,{construct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().addListener(j,this.__mS,this);
...
},__mS:qx.core.Environment.select(l,{"true":function(e){var o=this.getInvalidMessage();
...
}},"false":null})},destruct:function(){if(qx.core.Environment.get(l)){qx.locale.Manager.getInstance().removeListener(j,this.__mS,this);
}}});
})();
...
if(bk&&this.__mT){this.__nb().setStyles({"left":bl.left+bp,"top":bl.top+bp});
}
if(bm){if(this.__mT){this.__nb().setStyles({"width":innerWidth+bp,"height":innerHeight+bp});
...
if(this.__mT){if(bs){this._showPlaceholder();
...
if(this.__mT){this.__nb().setStyles(bx);
}if(bv){this.__mY=qx.bom.Label.getTextSize(d,bx);
...
var bC=true;
this.__mU=false;
if(this.__mX&&this.__mX===bD){bC=false;
...
}}},_removePlaceholder:function(){if(this.hasState(k)){this.__nb().setStyle(bc,X);
this.removeState(k);
}},_syncPlaceholder:function(){if(this.hasState(k)){this.__nb().setStyle(bc,L);
}},__nb:function(){if(this.__mV==null){this.__mV=new qx.html.Label();
var bS=qx.theme.manager.Color.getInstance();
this.__mV.setStyles({"visibility":X,"zIndex":6,"position":W,"color":bS.resolve(I)});
...
}},"false":null}),_applyPlaceholder:function(bT,bU){if(this.__mT){this.__nb().setValue(bT);
...
}}},destruct:function(){this.__mV=this.__jK=null;
...
}},setEnabled:qx.core.Environment.select(k,{"webkit":function(u){this.__ne=u;

if(!u){this.setStyles({"userModify":d,"userSelect":j});
}else{this.setStyles({"userModify":null,"userSelect":this.__nd?null:j});
}},"default":function(v){this.setAttribute(b,v===false);
}}),setSelectable:qx.core.Environment.select(k,{"webkit":function(w){this.__nd=w;
...
},setWrap:function(B,C){if(this.__nc===l){this._setProperty(n,B,C);
...
}},statics:{PRIORITY:qx.event.Registration.PRIORITY_NORMAL,SUPPORTED_TYPES:{input:1,change:1},TARGET_CHECK:qx.event.IEventHandler.TARGET_DOMNODE,IGNORE_CAN_HANDLE:false},members:{__nf:false,__ng:null,__mW:null,__mX:null,canHandleEvent:function(B,C){var D=B.tagName.toLowerCase();

if(C===m&&(D===m||D===g)){return true;
}
if(C===j&&(D===m||D===g||D===p)){return true;
}return false;
},registerEvent:function(E,F,G){if(qx.core.Environment.get(k)==x&&(qx.core.Environment.get(d)<9||(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)<9))){if(!E.__nh){var H=E.tagName.toLowerCase();
var I=E.type;
...
qx.bom.Event.addNativeListener(E,a,this._onKeyPressWrapped);
}}}}},__ni:qx.core.Environment.select(k,{"mshtml":function(J){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.addNativeListener(J,m,this._onInputWrapper);
...
}}),unregisterEvent:function(O,P){if(qx.core.Environment.get(k)==x&&qx.core.Environment.get(d)<9&&qx.core.Environment.get(A)<9){if(O.__nh){var Q=O.tagName.toLowerCase();
var R=O.type;
...
}}}},__nj:qx.core.Environment.select(k,{"mshtml":function(T){if(qx.core.Environment.get(d)>=9&&qx.core.Environment.get(A)>=9){qx.bom.Event.removeNativeListener(T,m,this._onInputWrapper);
...
}}),_onKeyPress:qx.core.Environment.select(k,{"mshtml|opera":function(e,Y){if(e.keyCode===13){if(Y.value!==this.__mW){this.__mW=Y.value;
qx.event.Registration.fireEvent(Y,j,qx.event.type.Data,[Y.value]);
}}},"default":null}),_inputFix:qx.core.Environment.select(k,{"mshtml":function(e,ba){if(e.keyCode===46||e.keyCode===8){if(ba.value!==this.__mX){this.__mX=ba.value;
qx.event.Registration.fireEvent(ba,m,qx.event.type.Data,[ba.value]);
}}},"default":null}),_onKeyDown:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__nf=true;
}},"default":null}),_onKeyUp:qx.core.Environment.select(k,{"opera":function(e){if(e.keyCode===13){this.__nf=false;
}},"default":null}),_onBlur:qx.core.Environment.select(k,{"opera":function(e){if(this.__ng&&qx.core.Environment.get(t)<10.6){window.clearTimeout(this.__ng);
...
if(!this.__nf||bb!==m){if((qx.core.Environment.get(k)==z)&&qx.core.Environment.get(t)<10.6){this.__ng=window.setTimeout(function(){qx.event.Registration.fireEvent(bc,m,qx.event.type.Data,[bc.value]);
},0);
...
}},properties:{spacingX:{check:h,init:0,apply:p},spacingY:{check:h,init:0,apply:p}},members:{__nm:null,__nk:null,__nl:null,__nn:null,__no:null,__np:null,__nq:null,__nr:null,__ns:null,verifyLayoutProperty:null,__nt:function(){var B=[];
var A=[];
var C=[];
var w=-1;
var v=-1;
...
this.__ns=null;
delete this._invalidChildrenCache;
},_setRowData:function(G,H,I){var J=this.__nk[G];

if(!J){this.__nk[G]={};
this.__nk[G][H]=I;
}else{J[H]=I;
}},_setColumnData:function(K,L,M){var N=this.__nl[K];
...
this._applyLayoutChange();
return this;
},getColumnAlign:function(S){var T=this.__nl[S]||{};
return {vAlign:T.vAlign||q,hAlign:T.hAlign||r};
},setRowAlign:function(U,V,W){this._setRowData(U,o,V);
this._setRowData(U,m,W);
this._applyLayoutChange();
return this;
},getRowAlign:function(X){var Y=this.__nk[X]||{};
return {vAlign:Y.vAlign||q,hAlign:Y.hAlign||r};
},getCellWidget:function(ba,bb){if(this._invalidChildrenCache){this.__nt();
}var ba=this.__nm[ba]||{};
return ba[bb]||null;
},getRowCount:function(){if(this._invalidChildrenCache){this.__nt();
}return this.__np+1;
},getColumnCount:function(){if(this._invalidChildrenCache){this.__nt();
}return this.__nq+1;
},getCellAlign:function(bc,bd){var bj=q;
var bh=r;
var bi=this.__nk[bc];
var bf=this.__nl[bd];
var be=this.__nm[bc][bd];
...
for(var i=0,l=this.__no.length;i<l;i++){var cl=this.__no[i];
var ch=this.__nu(cl);
...
for(var i=0,l=this.__nn.length;i<l;i++){var ct=this.__nn[i];
var cv=this.__nu(ct);
...
for(var j=0;j<cy.colSpan;j++){cB=cA[cu+j]?cA[cu+j].offset:0;
cs[cu+j].minWidth+=cB;
}}}},_getRowHeights:function(){if(this.__nr!=null){return this.__nr;
}var cP=[];
var cI=this.__np;
var cH=this.__nq;
...
for(var cO=0;cO<=cH;cO++){var cG=this.__nm[cQ][cO];
...
if(this.__no.length>0){this._fixHeightsRowSpan(cP);
}this.__nr=cP;
return cP;
},_getColWidths:function(){if(this.__ns!=null){return this.__ns;
}var cV=[];
var cS=this.__nq;
var cU=this.__np;
...
for(var dc=0;dc<=cU;dc++){var cR=this.__nm[dc][db];
...
if(this.__nn.length>0){this._fixWidthsColSpan(cV);
}this.__ns=cV;
return cV;
...
},renderLayout:function(ds,dt){if(this._invalidChildrenCache){this.__nt();
...
for(var dC=0;dC<=du;dC++){var dJ=this.__nm[dC][dP];
if(!dJ){top+=dV[dC]+dB;
continue;
...
this.__ns=null;
this.__nr=null;
},_computeSizeHint:function(){if(this._invalidChildrenCache){this.__nt();
...
return eb;
}},destruct:function(){this.__nm=this.__nk=this.__nl=this.__nn=this.__no=this.__ns=this.__nr=null;
}});
})();
(function(){var k="qx.event.type.Data",j="visibilityChanged",h="orderChanged",g="visibilityChangedPre",f="__nz",e="__nA",d="widthChanged",c="qx.ui.table.columnmodel.Basic",b="__nB",a="headerCellRendererChanged";
qx.Class.define(c,{extend:qx.core.Object,construct:function(){qx.core.Object.call(this);
this.__nv=[];
this.__nw=[];
},events:{"widthChanged":k,"visibilityChangedPre":k,"visibilityChanged":k,"orderChanged":k,"headerCellRendererChanged":k},statics:{DEFAULT_WIDTH:100,DEFAULT_HEADER_RENDERER:qx.ui.table.headerrenderer.Default,DEFAULT_DATA_RENDERER:qx.ui.table.cellrenderer.Default,DEFAULT_EDITOR_FACTORY:qx.ui.table.celleditor.TextField},members:{__mi:null,__nx:null,__nw:null,__nv:null,__ny:null,__nz:null,__nA:null,__nB:null,init:function(l,m){this.__ny=[];
var q=qx.ui.table.columnmodel.Basic.DEFAULT_WIDTH;
var r=this.__nz||(this.__nz=new qx.ui.table.columnmodel.Basic.DEFAULT_HEADER_RENDERER());
var o=this.__nA||(this.__nA=new qx.ui.table.columnmodel.Basic.DEFAULT_DATA_RENDERER());
var n=this.__nB||(this.__nB=new qx.ui.table.columnmodel.Basic.DEFAULT_EDITOR_FACTORY());
this.__nv=[];
this.__nw=[];
var t;
...
for(var u=0;u<l;u++){this.__ny[u]={width:q,headerRenderer:r,dataRenderer:o,editorFactory:n};
this.__nv[u]=u;
this.__nw[u]=u;
}this.__nx=null;
this.__mi=true;
...
}},getVisibleColumns:function(){return this.__nw!=null?this.__nw:[];
},setColumnWidth:function(v,w,y){var A=this.__ny[v].width;

if(A!=w){this.__ny[v].width=w;
var z={col:v,newWidth:w,oldWidth:A,isMouseAction:y||false};
this.fireDataEvent(d,z);
}},getColumnWidth:function(B){return this.__ny[B].width;
},setHeaderCellRenderer:function(C,D){var E=this.__ny[C].headerRenderer;

if(E!==this.__nz){E.dispose();
}this.__ny[C].headerRenderer=D;
this.fireDataEvent(a,{col:C});
},getHeaderCellRenderer:function(F){return this.__ny[F].headerRenderer;
},setDataCellRenderer:function(G,H){this.__ny[G].dataRenderer=H;
var I=this.__ny[G].dataRenderer;

if(I!==this.__nA){return I;
}return null;
},getDataCellRenderer:function(J){return this.__ny[J].dataRenderer;
},setCellEditorFactory:function(K,L){var M=this.__ny[K].headerRenderer;

if(M!==this.__nB){M.dispose();
}this.__ny[K].editorFactory=L;
},getCellEditorFactory:function(N){return this.__ny[N].editorFactory;
},_getColToXPosMap:function(){if(this.__nx==null){this.__nx={};

for(var Q=0;Q<this.__nv.length;Q++){var P=this.__nv[Q];
this.__nx[P]={overX:Q};
}
for(var O=0;O<this.__nw.length;O++){var P=this.__nw[O];
this.__nx[P].visX=O;
}}return this.__nx;
},getVisibleColumnCount:function(){return this.__nw!=null?this.__nw.length:0;
},getVisibleColumnAtX:function(R){return this.__nw[R];
},getVisibleX:function(S){return this._getColToXPosMap()[S].visX;
},getOverallColumnCount:function(){return this.__nv.length;
},getOverallColumnAtX:function(T){return this.__nv[T];
...
for(var x=bb+1;x<this.__nv.length;x++){var bd=this.__nv[x];
var Y=be[bd].visX;

if(Y!=null){bc=Y;
break;
}}if(bc==null){bc=this.__nw.length;
}this.__nw.splice(bc,0,W);
}else{var ba=this.getVisibleX(W);
this.__nw.splice(ba,1);
}this.__nx=null;
if(!this.__mi){var bf={col:W,visible:X};
this.fireDataEvent(g,bf);
this.fireDataEvent(j,bf);
}}},moveColumn:function(bg,bh){this.__mi=true;
var bk=this.__nv[bg];
var bi=this.isColumnVisible(bk);

if(bi){this.setColumnVisible(bk,false);
}this.__nv.splice(bg,1);
this.__nv.splice(bh,0,bk);
this.__nx=null;
...
},setColumnsOrder:function(bl){if(bl.length==this.__nv.length){this.__mi=true;
var bo=new Array(bl.length);

for(var bm=0;bm<this.__nv.length;bm++){var bn=this.isColumnVisible(bm);
bo[bm]=bn;

if(bn){this.setColumnVisible(bm,false);
}}this.__nv=qx.lang.Array.clone(bl);
this.__nx=null;
for(var bm=0;bm<this.__nv.length;bm++){if(bo[bm]){this.setColumnVisible(bm,true);
}}this.__mi=false;
this.fireDataEvent(h);
}else{throw new Error("setColumnsOrder: Invalid number of column positions given, expected "+this.__nv.length+", got "+bl.length);
}}},destruct:function(){for(var i=0;i<this.__ny.length;i++){this.__ny[i].headerRenderer.dispose();
this.__ny[i].dataRenderer.dispose();
this.__ny[i].editorFactory.dispose();
}this.__nv=this.__nw=this.__ny=this.__nx=null;
...
this.__mh=0;
this.__nE=[];
},events:{"paneReloadsData":b,"paneUpdated":z},properties:{firstVisibleRow:{check:j,init:0,apply:o},visibleRowCount:{check:j,init:0,apply:w},maxCacheLines:{check:j,init:1000,apply:C},allowShrinkX:{refine:true,init:false}},members:{__mh:null,__nD:null,__nC:null,__nF:null,__me:null,__md:null,__nE:null,__nG:0,_applyFirstVisibleRow:function(G,H){this.updateContent(false,G-H);
},_applyVisibleRowCount:function(I,J){this.updateContent(true);
},_getContentHint:function(){return {width:this.getPaneScroller().getTablePaneModel().getTotalWidth(),height:400};
},getPaneScroller:function(){return this.__nC;
},getTable:function(){return this.__nC.getTable();
},setFocusedCell:function(K,L,M){if(K!=this.__md||L!=this.__me){var N=this.__me;
this.__md=K;
this.__me=L;
...
},onTableModelDataChanged:function(Q,R,S,T){this.__nH();
var V=this.getFirstVisibleRow();
var U=this.getVisibleRowCount();

if(R==-1||R>=V&&Q<V+U){this.updateContent();
}},onTableModelMetaDataChanged:function(){this.updateContent(true);
},_applyMaxCacheLines:function(W,X){if(this.__nG>=W&&W!==-1){this.__nH();
}},__nH:function(){this.__nE=[];
this.__nG=0;
},__nI:function(Y,ba,bb){if(!ba&&!bb&&this.__nE[Y]){return this.__nE[Y];
}else{return null;
}},__nJ:function(bc,bd,be,bf){var bg=this.getMaxCacheLines();

if(!be&&!bf&&!this.__nE[bc]&&bg>0){this._applyMaxCacheLines(bg);
this.__nE[bc]=bd;
this.__nG+=1;
}},updateContent:function(bh,bi,bj,bk){if(bh){this.__nH();
...
bz.push({col:bY,xPos:x,editable:bA.isColumnEditable(bY),focusedCol:this.__md==bY,styleLeft:bC,styleWidth:bF});
bC+=bF;
}var bX=[];
var ca=false;
...
}}if(!this.__nF){this.__nF=document.createElement(E);
}var ck=e;
ck+=this._getRowsHtml(ci+ce,Math.abs(cb));
ck+=h;
this.__nF.innerHTML=ck;
var cf=this.__nF.firstChild.childNodes;
...
}}if(this.__me!==null){this._updateRowStyles(this.__me-cb);
this._updateRowStyles(this.__me);
}this.fireEvent(f);
...
this.__mh=cq;
this.fireEvent(f);
}},destruct:function(){this.__nF=this.__nC=this.__nE=null;
}});
})();
(function(){var e="first",d="last",c="hovered",b="__nC",a="qx.ui.table.pane.Header";
qx.Class.define(a,{extend:qx.ui.core.Widget,construct:function(f){qx.ui.core.Widget.call(this);
this._setLayout(new qx.ui.layout.HBox());
this.__ka=new qx.ui.core.Blocker(this);
this.__nC=f;
},members:{__nC:null,__nK:null,__nL:null,__ka:null,getPaneScroller:function(){return this.__nC;
},getTable:function(){return this.__nC.getTable();
},getBlocker:function(){return this.__ka;
...
}},setMouseOverColumn:function(k){if(k!=this.__nL){if(this.__nL!=null){var l=this.getHeaderWidgetAtColumn(this.__nL);
...
if(this.__nK==null){var y=this.getTable();
...
this.__nK=q;
}this.__nK.setLayoutProperties({left:s.left+x});
this.__nK.show();
},hideColumnMoveFeedback:function(){if(this.__nK!=null){this.__nK.destroy();
this.__nK=null;
}},isShowingColumnMoveFeedback:function(){return this.__nK!=null;
...
return {width:this.__nM?100:G,maxWidth:this.__nM?null:G,minWidth:this.__nM?null:G,height:this.__nM?G:100,maxHeight:this.__nM?G:null,minHeight:this.__nM?G:null};
...
if(this.__nM){content.scrollToX(K);
}else{content.scrollToY(K);
}},_applyOrientation:function(L,M){var N=this.__nM=L===k;
...
qx.ui.core.queue.Layout.add(this);
},_updateScrollBar:function(){var P=this.__nM;
...
}if((qx.core.Environment.get(d)==r)){var Q=this.getBounds();
...
var W=this.__nM?X.getScrollX():X.getScrollY();
this.setPosition(W);
...
var bn=this.__nP=qx.bom.element.Location.get(this.getContentElement().getDomElement())[bj];
var bl=this.__nQ=qx.bom.element.Location.get(bi.getContainerElement().getDomElement())[bj];

if(e.getTarget()===bi){this.__nS=true;

if(!this.__nX){this.__nX=new qx.event.Timer(100);
this.__nX.addListener(W,this._fireValue,this);
}this.__nX.start();
this.__nT=bm+bn-bl;
bi.addState(b);
}else{this.__nU=true;
...
if(this.__nM){bp=e.getDocumentLeft()-(this._valueToPosition(this.getValue())+this.__nP);
...
}else{bp=e.getDocumentTop()-(this._valueToPosition(this.getValue())+this.__nP);
...
if(bq<0||bq>this.__nR||bp<0||bp>this.__nR){this.getChildControl(k).removeState(g);
}}}else if(this.__nU){this.__jj.stop();
this.releaseCapture();
delete this.__nU;
delete this.__nV;
delete this.__nW;
...
}},_onMouseMove:function(e){if(this.__nS){var bs=this.__nM?e.getDocumentLeft():e.getDocumentTop();
var br=bs-this.__nT;
this.slideTo(this._positionToValue(br));
}else if(this.__nU){this.__oc(e);
}e.stopPropagation();
},_onInterval:function(e){var bt=this.getValue()+(this.__nV*this.getPageStep());
if(bt<this.getMinimum()){bt=this.getMinimum();
}else if(bt>this.getMaximum()){bt=this.getMaximum();
}var bu=this.__nV==-1;

if((bu&&bt<=this.__nW)||(!bu&&bt>=this.__nW)){bt=this.__nW;
}this.slideTo(bt);
},_onUpdate:function(e){var bw=this.getInnerSize();
var bx=this.getChildControl(k).getBounds();
var bv=this.__nM?E:C;
this._updateKnobSize();
this.__ob=bw[bv]-bx[bv];
this.__nR=bx[bv];
this._updateKnobPosition();
},__nM:false,__ob:0,__oc:function(e){var by=this.__nM;
...
var bI=this.__nV<0?N:z;
bD=bE+(Math[bI]((bD-bE)/bC)*bC);
}if(this.__nW==null||(this.__nV==-1&&bD<=this.__nW)||(this.__nV==1&&bD>=this.__nW)){this.__nW=bD;
}},_positionToValue:function(bK){var bL=this.__ob;
...
},_valueToPosition:function(bO){var bP=this.__ob;
...
if(this.__nM){bT.setStyle(f,bS+a,true);
...
if(bU==null){return;
}if(this.__nM){this.getChildControl(k).setWidth(Math.round(bV*bU.width));
...
this.__nM=bY===j;
if(this.__nM){this.removeState(i);
cb.removeState(i);
this.addState(j);
cb.addState(j);
...
(function(){var o="Boolean",n="resize-line",m="mousedown",l="qx.event.type.Data",k="mouseup",j="qx.ui.table.pane.CellEvent",i="scroll",h="focus-indicator",g="excluded",d="scrollbar-y",bq="table-scroller-focus-indicator",bp="visible",bo="mousemove",bn="header",bm="editing",bl="click",bk="modelChanged",bj="scrollbar-x",bi="cellClick",bh="pane",v="mouseout",w="__oj",t="changeHorizontalScrollBarVisible",u="__od",r="bottom",s="_applyScrollTimeout",p="changeScrollX",q="_applyTablePaneModel",C="Integer",D="dblclick",M="dataEdited",J="__kJ",U="mousewheel",P="interval",bd="qx.ui.table.pane.Scroller",ba="__oh",F="_applyShowCellFocusIndicator",bg="y",bf="resize",be="vertical",E="__of",H="changeScrollY",I="__oi",L="__oe",N="appear",Q="table-scroller",W="beforeSort",bc="__jj",y="cellDblclick",z="__og",G="horizontal",T="losecapture",S="contextmenu",R="col-resize",Y="disappear",X="_applyVerticalScrollBarVisible",O="_applyHorizontalScrollBarVisible",V="__ok",a="os.scrollBarOverlayed",bb="cellContextmenu",A="close",B="changeTablePaneModel",K="x",b="qx.ui.table.pane.Model",c="changeVerticalScrollBarVisible";
...
bs.setRowFlex(1,1);
this._setLayout(bs);
this.__kJ=this._showChildControl(bn);
this.__od=this._showChildControl(bh);
...
this.__of.add(this.__kJ);
this.__of.addListener(T,this._onChangeCaptureHeader,this);
this.__of.addListener(bo,this._onMousemoveHeader,this);
this.__of.addListener(m,this._onMousedownHeader,this);
this.__of.addListener(k,this._onMouseupHeader,this);
this.__of.addListener(bl,this._onClickHeader,this);
this.__oe.add(this.__of,{flex:1});
this.__og=new qx.ui.table.pane.Clipper();
this.__og.add(this.__od);
this.__og.addListener(U,this._onMousewheel,this);
this.__og.addListener(bo,this._onMousemovePane,this);
this.__og.addListener(m,this._onMousedownPane,this);
this.__og.addListener(k,this._onMouseupPane,this);
this.__og.addListener(bl,this._onClickPane,this);
this.__og.addListener(S,this._onContextMenu,this);
this.__og.addListener(D,this._onDblclickPane,this);
this.__og.addListener(bf,this._onResizePane,this);
if(qx.core.Environment.get(a)){this.__oh=new qx.ui.container.Composite();
this.__oh.setLayout(new qx.ui.layout.Canvas());
this.__oh.add(this.__og,{edge:0});
this._add(this.__oh,{row:1,column:0});
}else{this._add(this.__og,{row:1,column:0});
}this.__oi=this._showChildControl(bj);
this.__oj=this._showChildControl(d);
this.__ok=this.getChildControl(h);
...
},statics:{MIN_COLUMN_WIDTH:10,RESIZE_REGION_RADIUS:5,CLICK_TOLERANCE:5,HORIZONTAL_SCROLLBAR:1,VERTICAL_SCROLLBAR:2},events:{"changeScrollY":l,"changeScrollX":l,"cellClick":j,"cellDblclick":j,"cellContextmenu":j,"beforeSort":l},properties:{horizontalScrollBarVisible:{check:o,init:false,apply:O,event:t},verticalScrollBarVisible:{check:o,init:false,apply:X,event:c},tablePaneModel:{check:b,apply:q,event:B},liveResize:{check:o,init:false},focusCellOnMouseMove:{check:o,init:false},selectBeforeFocus:{check:o,init:false},showCellFocusIndicator:{check:o,init:true,apply:F},contextMenuFromDataCellsOnly:{check:o,init:true},resetSelectionOnHeaderClick:{check:o,init:true},scrollTimeout:{check:C,init:100,apply:s},appearance:{refine:true,init:Q}},members:{__mh:null,__kO:null,__ol:null,__om:null,__on:null,__oo:null,__op:null,__oq:null,__or:null,__os:null,__ot:null,__ou:null,__ov:null,__ow:null,__ox:false,__oy:null,__oz:null,__oA:null,__md:null,__me:null,__oB:null,__oC:null,__oD:null,__oi:null,__oj:null,__kJ:null,__of:null,__od:null,__og:null,__oh:null,__ok:null,__oe:null,__jj:null,getPaneInsetRight:function(){var bv=this.getTopRightWidget();
...
if(this.__oh!=null){bA.setMinHeight(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);
this.__oh.add(bA,{bottom:0,right:0,left:0});
}else{this._add(bA,{row:2,column:0});
}break;
case d:bA=this._createScrollBar(be);
bA.addListener(i,this._onScrollY,this);

if(this.__oh!=null){bA.setMinWidth(qx.bom.element.Overflow.DEFAULT_SCROLLBAR_WIDTH);
this.__oh.add(bA,{right:0,bottom:0,top:0});
}else{this._add(bA,{row:1,column:1});
}break;
}return bA||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,by);
},_applyHorizontalScrollBarVisible:function(bB,bC){this.__oi.setVisibility(bB?bp:g);
},_applyVerticalScrollBarVisible:function(bD,bE){this.__oj.setVisibility(bD?bp:g);
...
},_applyShowCellFocusIndicator:function(bH,bI){if(bH){this.__ok.setDecorator(bq);
this._updateFocusIndicator();
}else{if(this.__ok){this.__ok.setDecorator(null);
}}},getScrollY:function(){return this.__oj.getPosition();
},setScrollY:function(scrollY,bJ){this.__oj.scrollTo(scrollY);

if(bJ){this._updateContent();
}},getScrollX:function(){return this.__oi.getPosition();
},setScrollX:function(scrollX){this.__oi.scrollTo(scrollX);
},getTable:function(){return this.__kO;
},onColVisibilityChanged:function(){this.updateHorScrollBarMaximum();
this._updateFocusIndicator();
},setColumnWidth:function(bK,bL){this.__kJ.setColumnWidth(bK,bL);
this.__od.setColumnWidth(bK,bL);
...
},onTableModelDataChanged:function(bN,bO,bP,bQ){this.__od.onTableModelDataChanged(bN,bO,bP,bQ);
var bR=this.getTable().getTableModel().getRowCount();

if(bR!=this.__mh){this.updateVerScrollBarMaximum();
...
this.__kO._updateScrollBarVisibility();
},updateHorScrollBarMaximum:function(){var bV=this.__og.getInnerSize();
...
bU.setKnobFactor(1);
bU.setPosition(0);
}},updateVerScrollBarMaximum:function(){var cf=this.__og.getInnerSize();
...
this.__of.scrollToX(cg);
this.__og.scrollToX(cg);
...
}this.__oj.scrollBySteps(ck);
ck=e.getWheelDelta(K);
if(ck>0&&ck<1){ck=1;
}else if(ck<0&&ck>-1){ck=-1;
}this.__oi.scrollBySteps(ck);
if(this.__oz&&this.getFocusCellOnMouseMove()){this._focusCellAtPagePos(this.__oz,this.__oA);
}var cj=this.__oj.getPosition();
var ci=this.__oj.getMaximum();
if(ck<0&&cj<=0||ck>0&&cj>=ci){return;
}e.stop();
},__oE:function(cl){var cq=this.getTable();
var cr=this.__kJ.getHeaderWidgetAtColumn(this.__ot);
var cm=cr.getSizeHint().minWidth;
var co=Math.max(cm,this.__ov+cl-this.__ou);

if(this.getLiveResize()){var cn=cq.getTableColumnModel();
cn.setColumnWidth(this.__ot,co,true);
}else{this.__kJ.setColumnWidth(this.__ot,co,true);
var cp=this.getTablePaneModel();
this._showResizeLine(cp.getColumnLeft(this.__ot)+co);
}this.__ou+=co-this.__ov;
this.__ov=co;
},__oF:function(cs){var ct=qx.ui.table.pane.Scroller.CLICK_TOLERANCE;

if(this.__kJ.isShowingColumnMoveFeedback()||cs>this.__os+ct||cs<this.__os-ct){this.__op+=cs-this.__os;
this.__kJ.showColumnMoveFeedback(this.__oo,this.__op);
var cu=this.__kO.getTablePaneScrollerAtPageX(cs);

if(this.__or&&this.__or!=cu){this.__or.hideColumnMoveFeedback();
}
if(cu!=null){this.__oq=cu.showColumnMoveFeedback(cs);
}else{this.__oq=null;
}this.__or=cu;
this.__os=cs;
...
if(this.__ot!=null){this.__oE(cz);
cC=true;
e.stopPropagation();
}else if(this.__oo!=null){this.__oF(cz);
e.stopPropagation();
...
this.setCursor(cx);
this.__kJ.setMouseOverColumn(cv);
...
this.__ot=cL;
this.__ou=cM;
this.__ov=cN.getColumnWidth(this.__ot);
this.__of.capture();
},_startMoveHeader:function(cO,cP){this.__oo=cO;
this.__os=cP;
this.__op=this.getTablePaneModel().getColumnLeft(cO);
this.__of.capture();
...
if(cV!==null){this.__ow={row:cV,col:cU};
this.__ox=false;
...
if(!cR){cT.getSelectionManager().handleMouseDown(cV,e);
}}},_onMouseupFocusIndicator:function(e){if(this.__ow&&!this.__ox&&!this.isEditing()&&this.__ok.getRow()==this.__ow.row&&this.__ok.getColumn()==this.__ow.col){this.fireEvent(bi,qx.ui.table.pane.CellEvent,[this,e,this.__ow.row,this.__ow.col],true);
this.__ox=true;
}else if(!this.isEditing()){this._onMousedownPane(e);
}},_onChangeCaptureHeader:function(e){if(this.__ot!=null){this._stopResizeHeader();
}
if(this.__oo!=null){this._stopMoveHeader();
}},_stopResizeHeader:function(){var cW=this.getTable().getTableColumnModel();
if(!this.getLiveResize()){this._hideResizeLine();
cW.setColumnWidth(this.__ot,this.__ov,true);
}this.__ot=null;
this.__of.releaseCapture();
this.getApplicationRoot().setGlobalCursor(null);
this.setCursor(null);
if(this.isEditing()){var cX=this.__oB.getBounds().height;
this.__oB.setUserBounds(0,0,this.__ov,cX);
...
if(this.__or){this.__or.hideColumnMoveFeedback();
}
if(this.__oq!=null){var dg=de.getFirstColumnX()+de.getX(this.__oo);
var dc=this.__oq;
...
if(this.__ot!=null){this._stopResizeHeader();
this.__oy=true;
e.stop();
}else if(this.__oo!=null){this._stopMoveHeader();
e.stop();
}},_onClickHeader:function(e){if(this.__oy){this.__oy=false;
return;
...
if(this.__ok.isHidden()||(this.__ow&&!this.__ox&&!this.isEditing()&&du==this.__ow.row&&dv==this.__ow.col)){this.fireEvent(bi,qx.ui.table.pane.CellEvent,[this,e,du,dv],true);
this.__ox=true;
...
if(!this.getShowCellFocusIndicator()||dz===null||(this.__ow&&dz==this.__ow.row&&dA==this.__ow.col)){this.fireEvent(bb,qx.ui.table.pane.CellEvent,[this,e,dz,dA],true);
...
}if(this.__ot==null){this.setCursor(null);
this.getApplicationRoot().setGlobalCursor(null);
}this.__kJ.setMouseOverColumn(null);
if(this.getFocusCellOnMouseMove()){this.__kO.setFocusedCell();
}},_showResizeLine:function(x){var dI=this._showChildControl(n);
var dH=dI.getWidth();
var dJ=this.__og.getBounds();
...
var dN=this.__od.getContainerLocation().left;
...
dO=dL+1;
dM=dW-dN;
}var dQ=this.__og.getContainerLocation().left;
var dV=this.__og.getBounds().width;
var scrollX=dQ-dN;
...
this.__kO.setFocusedCell(ea,eb);
}},setFocusedCell:function(ec,ed){if(!this.isEditing()){this.__od.setFocusedCell(ec,ed,this.__om);
this.__md=ec;
this.__me=ed;
this._updateFocusIndicator();
}},getFocusedColumn:function(){return this.__md;
...
if(eg!=-1){var em=this.__og.getInnerSize();
...
if(!this.isEditing()&&(ey!=null)&&eu.isColumnEditable(ey)){var ez=this.__me;
var es=this.getTablePaneModel().getX(ey);
var et=eu.getValue(ey,ez);
this.scrollCellVisible(es,ez);
this.__oC=ew.getTableColumnModel().getCellEditorFactory(ey);
var ev={col:ey,row:ez,xPos:es,value:et,table:ew};
this.__oB=this.__oC.createCellEditor(ev);
...
this.__oB.addListener(A,this._onCellEditorModalWindowClose,this);
var f=ew.getModalCellEditorPreOpenFunction();

if(f!=null){f(this.__oB,ev);
}this.__oB.open();
}else{var ex=this.__ok.getInnerSize();
this.__oB.setUserBounds(0,0,ex.width,ex.height);
this.__ok.addListener(m,function(e){this.__ow={row:this.__me,col:this.__md};
e.stopPropagation();
},this);
this.__ok.add(this.__oB);
this.__ok.addState(bm);
this.__ok.setKeepActive(false);
this.__ok.setDecorator(bq);
this.__oB.focus();
this.__oB.activate();
}return true;
}return false;
},stopEditing:function(){if(!this.getShowCellFocusIndicator()){this.__ok.setDecorator(null);
}this.flushEditor();
this.cancelEditing();
},flushEditor:function(){if(this.isEditing()){var eB=this.__oC.getCellEditorValue(this.__oB);
var eA=this.getTable().getTableModel().getValue(this.__md,this.__me);
this.getTable().getTableModel().setValue(this.__md,this.__me,eB);
this.__kO.focus();
this.__kO.fireDataEvent(M,{row:this.__me,col:this.__md,oldValue:eA,value:eB});
...
}else{this.__ok.removeState(bm);
this.__ok.setKeepActive(true);
this.__oB.destroy();
this.__oB=null;
this.__oC=null;
...
var eI=this.__od.getContentLocation().left;
...
var eQ=this.__kJ.getContainerLocation().left;
...
},_getRowForPagePos:function(eR,eS){var eT=this.__od.getContentLocation();

if(eR<eT.left||eR>eT.right){return null;
}
if(eS>=eT.top&&eS<=eT.bottom){var eU=this.getTable().getRowHeight();
var scrollY=this.__oj.getPosition();
...
}var eW=this.__kJ.getContainerLocation();

if(eS>=eW.top&&eS<=eW.bottom&&eR<=eW.right){return -1;
}return null;
},setTopRightWidget:function(fb){var fc=this.__oD;

if(fc!=null){this.__oe.remove(fc);
}
if(fb!=null){this.__oe.add(fb);
}this.__oD=fb;
},getTopRightWidget:function(){return this.__oD;
},getHeader:function(){return this.__kJ;
},getTablePane:function(){return this.__od;
},getVerticalScrollBarWidth:function(){var fd=this.__oj;
return fd.isVisible()?(fd.getSizeHint().width||0):0;
},getNeededScrollBars:function(fe,ff){var fo=this.__oj;
...
},getPaneClipper:function(){return this.__og;
...
},_oninterval:qx.event.GlobalError.observeMethod(function(){if(this.__om&&!this.__od._layoutPending){this.__om=false;
this._updateContent();
}}),_updateContent:function(){var fC=this.__og.getInnerSize();

if(!fC){return;
}var fF=fC.height;
var scrollX=this.__oi.getPosition();
var scrollY=this.__oj.getPosition();
var fz=this.getTable().getRowHeight();
var fA=Math.floor(scrollY/fz);
var fE=this.__od.getFirstVisibleRow();
this.__od.setFirstVisibleRow(fA);
...
if(!fD){fB++;
fy=scrollY%fz;
}this.__od.setVisibleRowCount(fB);

if(fA!=fE){this._updateFocusIndicator();
}this.__og.scrollToX(scrollX);
if(!fD){this.__og.scrollToY(fy);
}},_updateFocusIndicator:function(){var fG=this.getTable();

if(!fG.getEnabled()){return;
}this.__ok.moveToCell(this.__md,this.__me);
}},destruct:function(){this._stopInterval();
var fH=this.getTablePaneModel();

if(fH){fH.dispose();
}this.__ow=this.__oD=this.__kO=null;
this._disposeObjects(I,w,E,z,V,J,u,L,bc,ba);
}});
})();
...
}},moveToCell:function(j,k){if(!this.__oG.getShowCellFocusIndicator()&&!this.__oG.getTable().getTableModel().isColumnEditable(j)){this.exclude();
return;
}else{this.show();
}
if(j==null){this.hide();
...
(function(){var l="move",k="Boolean",j="mouseup",i="mousedown",h="losecapture",g="qx.ui.core.MMovable",f="__oI",d="mousemove",c="maximized",b="__oJ",a="move-frame";
qx.Mixin.define(g,{properties:{movable:{check:k,init:true},useMoveFrame:{check:k,init:false}},members:{__oI:null,__oJ:null,__oK:null,__oL:null,__oM:null,__oN:null,__oO:null,__oP:false,__oQ:null,__oR:0,_activateMoveHandle:function(m){if(this.__oI){throw new Error("The move handle could not be redefined!");
}this.__oI=m;
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(!parent.isContentBlocked()){this.__oQ=parent.getBlockerColor();
this.__oR=parent.getBlockerOpacity();
parent.setBlockerColor(null);
parent.setBlockerOpacity(1);
parent.blockContent(this.getZIndex()-1);
this.__oP=true;
}}this.__oK={left:w.left,top:w.top,right:w.left+x.width,bottom:w.top+x.height};
var v=this.getContainerLocation();
this.__oN=w.left;
this.__oO=w.top;
this.__oL=v.left-e.getDocumentLeft();
this.__oM=v.top-e.getDocumentTop();
this.addState(l);
this.__oI.capture();
if(this.getUseMoveFrame()){this.__oT();
}e.stop();
},_onMoveMouseMove:function(e){if(!this.hasState(l)){return;
}var y=this.__oU(e);

if(this.getUseMoveFrame()){this.__oS().setDomPosition(y.viewportLeft,y.viewportTop);
...
if(qx.Class.implementsInterface(parent,qx.ui.window.IDesktop)){if(this.__oP){parent.unblockContent();
parent.setBlockerColor(this.__oQ);
parent.setBlockerOpacity(this.__oR);
this.__oQ=null;
this.__oR=0;
this.__oP=false;
}}this.__oI.releaseCapture();
var z=this.__oU(e);
this.setLayoutProperties({left:z.parentLeft,top:z.parentTop});
if(this.getUseMoveFrame()){this.__oS().exclude();
}e.stopPropagation();
},__oV:function(e){if(!this.hasState(l)){return;
}this.removeState(l);
if(this.getUseMoveFrame()){this.__oS().exclude();
...
qx.Mixin.define(k,{properties:{contentPaddingTop:{check:p,init:0,apply:o,themeable:true},contentPaddingRight:{check:p,init:0,apply:o,themeable:true},contentPaddingBottom:{check:p,init:0,apply:o,themeable:true},contentPaddingLeft:{check:p,init:0,apply:o,themeable:true},contentPadding:{group:[e,a,d,c],mode:b,themeable:true}},members:{__oW:{contentPaddingTop:i,contentPaddingRight:h,contentPaddingBottom:m,contentPaddingLeft:f},__oX:{contentPaddingTop:l,contentPaddingRight:n,contentPaddingBottom:g,contentPaddingLeft:j},_applyContentPadding:function(q,r,name){var s=this._getContentPaddingTarget();

if(q==null){var t=this.__oX[name];
s[t]();
}else{var u=this.__oW[name];
s[u](q);
}}}});
})();
...
A.addListener(i,this.__pm,this,true);
A.addListener(q,this.__pn,this);
A.addListener(x,this.__pp,this);
A.addListener(s,this.__pq,this);
A.addListener(o,this.__po,this);
...
},properties:{resizableTop:{check:k,init:true},resizableRight:{check:k,init:true},resizableBottom:{check:k,init:true},resizableLeft:{check:k,init:true},resizable:{group:[l,d,m,y],mode:v},resizeSensitivity:{check:a,init:5},useResizeFrame:{check:k,init:true}},members:{__oY:null,__pa:null,__pb:null,__pc:null,__pd:null,__pe:null,__pf:null,RESIZE_TOP:1,RESIZE_BOTTOM:2,RESIZE_LEFT:4,RESIZE_RIGHT:8,_getResizeFrame:function(){var C=this.__pa;

if(!C){C=this.__pa=new qx.ui.core.Widget();
C.setAppearance(p);
C.exclude();
qx.core.Init.getApplication().getRoot().add(C);
}return C;
},__pg:function(){var location=this.__pj();
...
},__ph:function(e){var F=this.__pb;
...
if((F&this.RESIZE_TOP)||(F&this.RESIZE_BOTTOM)){L=Math.max(K.top,Math.min(K.bottom,e.getDocumentTop()))-this.__pd;
...
if((F&this.RESIZE_LEFT)||(F&this.RESIZE_RIGHT)){L=Math.max(K.left,Math.min(K.right,e.getDocumentLeft()))-this.__pc;
...
},__pi:{1:f,2:n,4:h,8:z,5:t,6:g,9:c,10:b},__pj:function(){var O=this.getDecoratorElement();
if(O&&O.getDomElement()){return qx.bom.element.Location.get(O.getDomElement());
}else{return this.getContentLocation();
}},__pk:function(e){var location=this.__pj();
var P=this.getResizeSensitivity();
var S=e.getDocumentLeft();
var R=e.getDocumentTop();
var Q=this.__pl(location,S,R,P);
if(Q>0){Q=Q|this.__pl(location,S,R,P*2);
}this.__pb=Q;
},__pl:function(location,T,U,V){var W=0;
...
},__pm:function(e){if(!this.__pb){return;
}this.addState(j);
this.__pc=e.getDocumentLeft();
this.__pd=e.getDocumentTop();
var bb=this.getContainerLocation();
var X=this.__pj();
var ba=this.getBounds();
this.__pe={top:X.top,left:X.left,containerWidth:bb.right-bb.left,containerHeight:bb.bottom-bb.top,width:X.right-X.left,height:X.bottom-X.top,bounds:qx.lang.Object.clone(ba)};
var parent=this.getLayoutParent();
var bc=parent.getContentLocation();
var Y=parent.getBounds();
this.__pf={left:bc.left,top:bc.top,right:bc.left+Y.width,bottom:bc.top+Y.height};
if(this.getUseResizeFrame()){this.__pg();
}this.capture();
e.stop();
},__pn:function(e){if(!this.hasState(j)){return;
...
this.releaseCapture();
e.stopPropagation();
},__po:function(e){if(!this.__pb){return;
}this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
this.removeState(w);
if(this.getUseResizeFrame()){this._getResizeFrame().exclude();
}},__pp:function(e){if(this.hasState(j)){var bh=this.__ph(e);
...
}else if(!this.hasState(u)&&!this.__oY.isSessionActive()){this.__pk(e);
var bi=this.__pb;
var bg=this.getApplicationRoot();

if(bi){var be=this.__pi[bi];
this.setCursor(be);
bg.setGlobalCursor(be);
}else if(this.getCursor()){this.resetCursor();
bg.resetGlobalCursor();
}}},__pq:function(e){if(this.getCursor()&&!this.hasState(j)){this.resetCursor();
this.getApplicationRoot().resetGlobalCursor();
}}},destruct:function(){if(this.__pa!=null&&!qx.core.ObjectRegistry.inShutDown){this.__pa.destroy();
this.__pa=null;
}this.__oY=null;
}});
})();
...
this.__ps=bh.left===undefined?0:bh.left;
this.__pr=bh.top===undefined?0:bh.top;
...
this.__ps=bi.left===undefined?0:bi.left;
this.__pr=bi.top===undefined?0:bi.top;
this.removeState(b);
this.hide();
this.fireEvent(u);
...
},events:{"modelChanged":e},statics:{EVENT_TYPE_MODEL_CHANGED:"modelChanged"},properties:{firstColumnX:{check:c,init:0,apply:d},maxColumnCount:{check:f,init:-1,apply:a}},members:{__pt:null,__pu:null,_applyFirstColumnX:function(j,k){this.__pt=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},_applyMaxColumnCount:function(l,m){this.__pt=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},setTableColumnModel:function(n){if(this.__pu){this.__pu.removeListener(g,this._onColVisibilityChanged,this);
this.__pu.removeListener(h,this._onColVisibilityChanged,this);
}this.__pu=n;
this.__pu.addListener(g,this._onColVisibilityChanged,this);
this.__pu.addListener(h,this._onHeaderCellRendererChanged,this);
this.__pt=null;
},_onColVisibilityChanged:function(o){this.__pt=null;
this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},_onHeaderCellRendererChanged:function(p){this.fireEvent(qx.ui.table.pane.Model.EVENT_TYPE_MODEL_CHANGED);
},getColumnCount:function(){if(this.__pt==null){var q=this.getFirstColumnX();
var s=this.getMaxColumnCount();
var r=this.__pu.getVisibleColumnCount();

if(s==-1||(q+s)>r){this.__pt=r-q;
}else{this.__pt=s;
}}return this.__pt;
},getColumnAtX:function(t){var u=this.getFirstColumnX();
return this.__pu.getVisibleColumnAtX(u+t);
},getX:function(v){var w=this.getFirstColumnX();
var y=this.getMaxColumnCount();
var x=this.__pu.getVisibleX(v)-w;
...
if(A==z){return C;
}C+=this.__pu.getColumnWidth(A);
}return -1;
},getTotalWidth:function(){var D=0;
var E=this.getColumnCount();

for(var x=0;x<E;x++){var F=this.getColumnAtX(x);
D+=this.__pu.getColumnWidth(F);
}return D;
}},destruct:function(){if(this.__pu){this.__pu.removeListener(g,this._onColVisibilityChanged,this);
this.__pu.removeListener(h,this._onColVisibilityChanged,this);
}this.__pu=null;
}});
})();
...
this.__pw=[];
this.__px={};
},members:{__pv:null,__pw:null,__px:null,__mi:null,init:function(f){},getRowCount:function(){throw new Error("getRowCount is abstract");
...
},getColumnCount:function(){return this.__pv.length;
},getColumnIndexById:function(y){return this.__px[y];
},getColumnId:function(z){return this.__pv[z];
},getColumnName:function(A){return this.__pw[A];
},setColumnIds:function(B){this.__pv=B;
this.__px={};

for(var i=0;i<B.length;i++){this.__px[B[i]]=i;
}this.__pw=new Array(B.length);
if(!this.__mi){this.fireEvent(e);
}},setColumnNamesByIndex:function(C){if(this.__pv.length!=C.length){throw new Error("this.__columnIdArr and columnNameArr have different length: "+this.__pv.length+" != "+C.length);
}this.__pw=C;
this.fireEvent(e);
},setColumnNamesById:function(D){this.__pw=new Array(this.__pv.length);

for(var i=0;i<this.__pv.length;++i){this.__pw[i]=D[this.__pv[i]];
}},setColumns:function(E,F){var G=this.__pv.length==0||F;

if(F==null){if(this.__pv.length==0){F=E;
}else{F=this.__pv;
}}
...
}},destruct:function(){this.__pv=this.__pw=this.__px=null;
}});
})();
...
}},members:{__py:null,__pB:null,__pC:null,__pA:null,__pz:null,__pD:null,getRowData:function(B){var C=this.__py[B];

if(C==null||C.originalData==null){return C;
}else{return C.originalData;
}},getRowDataAsMap:function(D){var F=this.__py[D];
...
},setEditable:function(K){this.__pB=[];

for(var L=0;L<this.getColumnCount();L++){this.__pB[L]=K;
}this.fireEvent(d);
},setColumnEditable:function(M,N){if(N!=this.isColumnEditable(M)){if(this.__pB==null){this.__pB=[];
}this.__pB[M]=N;
this.fireEvent(d);
}},isColumnEditable:function(O){return this.__pB?(this.__pB[O]==true):false;
},setColumnSortable:function(P,Q){if(Q!=this.isColumnSortable(P)){if(this.__pC==null){this.__pC=[];
}this.__pC[P]=Q;
this.fireEvent(d);
}},isColumnSortable:function(R){return (this.__pC?(this.__pC[R]!==false):true);
...
},getSortMethods:function(bd){return this.__pA[bd];
},clearSorting:function(){if(this.__pz!=-1){this.__pz=-1;
this.__pD=true;
this.fireEvent(d);
}},getSortColumnIndex:function(){return this.__pz;
},_setSortColumnIndex:function(be){this.__pz=be;
},isSortAscending:function(){return this.__pD;
},_setSortAscending:function(bf){this.__pD=bf;
},getRowCount:function(){return this.__py.length;
},getValue:function(bg,bh){if(bh<0||bh>=this.__py.length){throw new Error("this.__rowArr out of bounds: "+bh+" (0.."+this.__py.length+")");
}return this.__py[bh][bg];
},setValue:function(bi,bj,bk){if(this.__py[bj][bi]!=bk){this.__py[bj][bi]=bk;
if(this.hasListener(e)){var bl={firstRow:bj,lastRow:bj,firstColumn:bi,lastColumn:bi};
this.fireDataEvent(e,bl);
}
if(bi==this.__pz){this.clearSorting();
}}},setData:function(bm,bn){this.__py=bm;
...
}},getData:function(){return this.__py;
},setDataAsMapArray:function(bp,bq,br){this.setData(this._mapArray2RowArr(bp,bq),br);
},addRows:function(bs,bt,bu){if(bt==null){bt=this.__py.length;
}bs.splice(0,0,bt,0);
Array.prototype.splice.apply(this.__py,bs);
var bv={firstRow:bt,lastRow:this.__py.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
Array.prototype.splice.apply(this.__py,bA);
var bD={firstRow:bB,lastRow:this.__py.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
...
},removeRows:function(bI,bJ,bK){this.__py.splice(bI,bJ);
var bL={firstRow:bI,lastRow:this.__py.length-1,firstColumn:0,lastColumn:this.getColumnCount()-1,removeStart:bI,removeCount:bJ};
this.fireDataEvent(e,bL);
...
(function(){var d="rhyacotriton.BasicTable",c="Position ",b="object",a='undefined';
qx.Class.define(d,{extend:qx.ui.table.Table,construct:function(e){this.__pE=new smart.model.Default;
this.__pF=e;
this.__pG=[];
this.__pH=[];
this.__pI=[];
...
for(var name in this.__pF){var f=this.__pF[name];
this.__pG[name]=i;
this.__pH[i]=name;
this.__pI[i]=f;
i++;
...
var h=this.__qc=this.getSelectionModel();
h.setSelectionMode(qx.ui.table.selection.Model.MULTIPLE_INTERVAL_SELECTION);
var k=this.getColumnNameToPositionIndex();
var g=this.__pE;
g.indexedSelection(k.id,h);
g.addIndex(k.id);
this.__rP={"dataAdded":this.__qi,"dataRemoved":this.__qk,"dataRemoveFailure":this.__onDataRemoveFailure,"dataLoadCompleted":this.__qh,"dataUpdated":this.__qo};
},members:{__pE:null,__qc:null,__rP:null,__pG:null,__pH:null,__pI:null,setColumnNames:function(l){},getColumnNameToPositionIndex:function(){return this.__pG;
},logSelectedRows:function(){var m=this.__pE;
this.__qc.iterateSelection(function(n){console.log(c+n);
console.dir(m.getRowData(n));
});
},getSelectedIds:function(){var q=[];
var o=this.__pE;
var p=this.getColumnNameToPositionIndex();
this.__qc.iterateSelection(function(r){q.push(o.getValue(p.id,r));
});
return q;
},particallyUpdateRows:function(s){var x=this.__pE;
var y=this.getColumnNameToPositionIndex();

for(var i=0,t=s.length;i<t;i++){var z=s[i];

if(typeof (z.id)==a)this.error("Cannot get Rows[i].id");
var w=x.locate(y.id,z.id);

if(isNaN(w)){this.error("Cannot locate row.");
continue;
}var u=x.getRowData(w,undefined,false);

if(typeof (u)!=b){this.error("Cannot retrieve old values.");
continue;
}var v=u.slice(0);
v=this.fillFields(z,v,true);
var w=x.locate(y.id,z.id);
x.setRow(w,v);
}},removeSelectedRows:function(){this.info("RemoveSelectedRows");
this.logSelectedRows();
var A=this;
var B=this.getSelectedIds();
this.__removeIds(B);
},__qj:function(C){var D=[];

for(var i in C){var E=C[i];
D[i]=this.fillFields(E,[],true);
}this.debug("Bulk update.");
this.__pE.addRows(D,true,false);
this.updateContent();
},fillFields:function(F,G,H){var I=this.getColumnNameToPositionIndex();

for(var j in F){var J=I[j];
G[J]=F[j];
}return G;
},getEventHandler:function(name){return this.__rP[name];
},__qh:function(event){var K=event.getData();

try{this.__pE.removeRows(0,undefined,undefined,false);
}catch(L){this.debug("Is the table empty?");
}this.__qj(K.rows);
},__qi:function(event){var M=event.getData();
this.__qj(M.rows);
},__qk:function(event){var N=event.getData().rows;
this.__removeIds(N);
},__qo:function(event){var O=event.getData();
this.particallyUpdateRows(O.rows);
}}});
})();
...
try{if(!f.call(this))cb=true;
}catch(e){this.__debug(e);
cb=true;
}
if(cb)this.__debug(bR+name);
else{}if(cb)this.testsFailed=true;
return !cb;
},unitTest:function(){try{this.__qq();
}catch(e){this.__debug(e);
}},__qq:function(){var i,cj=false;
var cr=new smart.model.Default();
var cn={"number":0,"string":1,"object":2,"function":3,"random":4};
cr.setColumns(function(){var C=[];

for(name in cn){var cs=cn[name];
C[cs]=name;
}return C;
}());
this.__qp(bU,function(){for(name in cn){var ct=cn[name];

if(cr.getColumnIndexById(name)!=ct)return false;
}return true;
});
cr.addIndex(cn[bF]);
cr.sortByColumn(cn[bF],true);
cr.addView(function(R){return R[cn[bD]].length&1;
});
var N=100,cp=100;
var ci=[];

for(i=0;i<N;i++)ci.push([i,bE+i,new qx.ui.basic.Label(bE+i),function(x){return x==i;
},Math.random()]);
cr.addRows(ci);
this.__qp(bc,function(){return (cr.getRowCount()==cp);
});
this.__qp(bT,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__qp(k,function(){for(i=0;i<N;i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
cr.setData(ci);
this.__qp(bv,function(){return (cr.getRowCount()==N);
});
this.__qp(Q,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__qp(bM,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
this.__qp(O,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(bS,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];

for(i=-10;i<0;i++)ci.push([i,bE+i,new qx.ui.basic.Label(bE+i),function(x){return x==i;
},Math.random()]);
cr.addRows(ci);
cp+=10;
this.__qp(a,function(){return (cr.getRowCount()==cp);
});
this.__qp(n,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i)!=i-10)return false;
return true;
});
this.__qp(d,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i-10)return false;
return true;
});
this.__qp(L,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(r,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];

for(i=N;i<N+10;i++)ci.push([i,bE+i,new qx.ui.basic.Label(bE+i),function(x){return x==i;
},Math.random()]);
cr.addRows(ci);
cp+=10;
this.__qp(q,function(){return (cr.getRowCount()==cp);
});
this.__qp(bX,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i)!=i-10)return false;
return true;
});
this.__qp(bJ,function(){for(i=0;i<cr.getRowCount();i++)if(cr.getValueById(bF,i)!=i-10)return false;
return true;
});
this.__qp(z,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(g,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
ci=[];

for(i=0;i<N;i++){var I=i+0.5;
ci.push([I,bE+I,new qx.ui.basic.Label(bE+I),function(x){return x==I;
},Math.random()]);
}cr.addRows(ci);
cp+=N;
this.__qp(bf,function(){return (cr.getRowCount()==cp);
});
this.__qp(ba,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(bu,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(N>>2,N>>1);
cp-=(N>>1);
this.__qp(br,function(){return (cr.getRowCount()==cp);
});
this.__qp(bN,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(by,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(0,5);
cp-=5;
this.__qp(X,function(){return (cr.getRowCount()==cp);
});
this.__qp(bs,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(bV,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(cr.getRowCount()-5);
cp-=5;
this.__qp(T,function(){return (cr.getRowCount()==cp);
});
this.__qp(bl,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(y,function(){for(i=1;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bF],i-1,1)>cr.getValue(cn[bF],i,1))return false;
return true;
});
cr.removeRows(0,1);
cr.removeRows(cr.getRowCount()-1);
cr.removeRows(cr.getRowCount()>>1,1);
cp-=3;
this.__qp(S,function(){return (cr.getRowCount()==cp);
});
this.__qp(F,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
var ck=33.33,cl=-10000,cm=10000;
cr.addRows([[ck,bE+ck,new qx.ui.basic.Label(bE+ck),function(x){return x==ck;
},Math.random()]]);
cr.addRows([[cl,bE+cl,new qx.ui.basic.Label(bE+cl),function(x){return x==cl;
},Math.random()]]);
cr.addRows([[cm,bE+cm,new qx.ui.basic.Label(bE+cm),function(x){return x==cm;
},Math.random()]]);
cp+=3;
this.__qp(bb,function(){return (cr.getRowCount()==cp);
});
this.__qp(bC,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(bC,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(c,function(){return (cr.locate(cn[bF],cl)==0);
});
this.__qp(bq,function(){return (cr.locate(cn[bF],cm)==cr.getRowCount()-1);
});
cr.setValue(cn[bB],5,null);
this.__qp(bL,function(){return (cr.getValue(cn[bB],5)==null);
});
this.__qp(K,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(H,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
var cf=cr.getRowCount();
var cq=cr.getRowCount(1);
var ce=bn;

for(i=0;i<cr.getRowCount();i++)if((cr.getValue(cn[bD],i).length&1)==0)break;
var I=cr.getValue(cn[bF],i);
cr.setValue(cn[bD],i,ce);
this.__qp(p,function(){return (cr.getValue(cn[bD],i)==ce);
});
this.__qp(t,function(){for(var i=0;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bD],i,1)==ce)return false;
return true;
});
this.__qp(s,function(){return cr.locate(cn[bF],I,1)==undefined;
});
this.__qp(bw,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(l,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(B,function(){return (cr.getRowCount()==cf);
});
this.__qp(bW,function(){return (cr.getRowCount(1)==(cq-1));
});
var cf=cr.getRowCount();
var cq=cr.getRowCount(1);
var cd=bd;

for(i=0;i<cr.getRowCount();i++)if((cr.getValue(cn[bD],i).length&1)==1)break;
var I=cr.getValue(cn[bF],i);
cr.setValue(cn[bD],i,cd);
this.__qp(bI,function(){return (cr.getValue(cn[bD],i)==cd);
});
this.__qp(Y,function(){for(var i=0;i<cr.getRowCount(1);i++)if(cr.getValue(cn[bD],i,1)==cd)return true;
return false;
});
this.__qp(bi,function(){return cr.locate(cn[bF],I,1)!=undefined;
});
this.__qp(M,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(bg,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(G,function(){return (cr.getRowCount()==cf);
});
this.__qp(bz,function(){return (cr.getRowCount(1)==(cq+1));
});
cf=cr.getRowCount();
cq=cr.getRowCount(1);
ck=0.1;
cr.setValue(cn[bF],cq>>1,ck,1);
this.__qp(V,function(){return (cr.getValue(cn[bF],cr.locate(cn[bF],ck))==ck);
});
this.__qp(o,function(){return (cr.getValue(cn[bF],cr.locate(cn[bF],ck,1),1)==ck);
});
this.__qp(bt,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(W,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bF],i-1)>cr.getValue(cn[bF],i))return false;
return true;
});
this.__qp(bH,function(){return (cr.getRowCount()==cf);
});
this.__qp(u,function(){return (cr.getRowCount(1)==cq);
});
cf=cr.getRowCount();
cq=cr.getRowCount(1);
cr.sortByColumn(cn[bD],true);
this.__qp(ca,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bD],i-1)>cr.getValue(cn[bD],i))return false;
return true;
});
this.__qp(bp,function(){for(i=1;i<cr.getRowCount();i++)if(cr.getValue(cn[bD],i-1)>cr.getValue(cn[bD],i))return false;
return true;
});
this.__qp(bP,function(){return (cr.getRowCount()==cf);
});
this.__qp(bo,function(){return (cr.getRowCount(1)==cq);
});
cr.addView([function(R){return R[cn[bF]]>=20;
},function(R){return R[cn[bF]]<=80;
}],null,be);
cr.addView([function(R){return R[cn[bF]]<20;
},function(R){return R[cn[bF]]>80;
}],null,bx);
this.__qp(bO,function(){for(i=0;i<cr.getRowCount(2);i++){var x=cr.getValue(cn[bF],i,2);

if(x>=20&&x<=80)return false;
}return true;
});
this.__qp(E,function(){for(i=0;i<cr.getRowCount(3);i++){var x=cr.getValue(cn[bF],i,3);

if(x<20||x>80)return false;
}return true;
});
this.__qp(bY,function(){var cu={};

for(i=0;i<cr.getRowCount(2);i++)cu[cr.getValue(cn[bF],i,2)]=1;

for(i=0;i<cr.getRowCount(3);i++)if(cu[cr.getValue(cn[bF],i,3)]==1)return false;
return true;
});
this.__qp(D,function(){return cr.getRowCount(2)+cr.getRowCount(3)==cr.getRowCount(0);
});
var cg=[];

for(i=0;i<cr.getRowCount(2);i++)cg.push(cr.getRowReference(i,2));
cr.removeReferencedRows(cg);
this.__qp(bK,function(){return cr.getRowCount(2)==0;
});
this.__qp(bm,function(){for(i=0;i<cr.getRowCount();i++){var x=cr.getValue(cn[bF],i);

if(x<20||x>80)return false;
}return true;
});
cr.clearAllRows();
this.__qp(m,function(){for(var v=0;v<cr.getViewCount();v++)if(cr.getRowCount(v)!=0)return false;
return true;
});
cr.sortByColumn(cn[bF],true);
N=1000;
...
for(i=0;i<N;i++){var A=[];
var ch=(Math.random()&7)+1;

for(var j=0;j<ch;j++){var x=(Math.random()*~0)^0;
var R=[x,bE+x,null,null,x];
A.push(R);
cc.push(R);
co++;
}cr.addRows(A);
}
if(false){this.__debugobj(cc);

for(i=0;i<cr.getRowCount();i++)this.__debug(bh+i+bQ+cr.getValue(cn[bF],i));
}this.__qp(P,function(){cc.sort(function(cv,cw){return cv[cn[bF]]-cw[cn[bF]];
});

for(var i=0;i<cc.length;i++)if(cr.getValue(cn[bF],i)!=cc[i][cn[bF]])return false;
return true;
});
this.__qp(h,function(){return cr.getRowCount()==co;
});
N=(1<<10);
cr.clearAllRows();
co={};
...
for(i=0;i<(N>>2);i++){var I=(Math.random()*(cr.getRowCount()-1))^0;
co[cr.getValue(cn[bF],I)]=undefined;
cr.removeRows(I,1);
}this.__qp(bj,function(){for(i in co)if(co[i]!=undefined)if(cr.locate(cn[bF],i)==undefined)return false;

for(i=0;i<cr.getRowCount();i++)if(co[cr.getValue(cn[bF],i)]!=1)return false;
return true;
});
this.__qp(bk,function(){for(i=0;i<cr.getRowCount(2);i++)if(co[cr.getValue(cn[bF],i,2)]!=1)return false;
return true;
});
cr.clearAllRows();
var N=100,cp=100;
...
for(i=0;i<N;i++)if(i!=49&&i!=50&&i!=51)ci.push([i,bE+i,new qx.ui.basic.Label(bE+i),function(x){return x==i;
},Math.random()]);
cr.addRows(ci);
cr.addRows([[49,bE+49,new qx.ui.basic.Label(bE+49),function(x){return x==49;
},Math.random()],[50,bE+50,new qx.ui.basic.Label(bE+50),function(x){return x==50;
},Math.random()],[51,bE+51,new qx.ui.basic.Label(bE+51),function(x){return x==51;
},Math.random()]]);
this.__qp(U,function(){return (cr.getRowCount()==cp);
});
this.__qp(bA,function(){for(i=0;i<N;i++)if(cr.getValue(cn[bF],i)!=i)return false;
return true;
});
this.__qp(bG,function(){for(i=0;i<N;i++)if(cr.getValueById(bF,i)!=i)return false;
return true;
});
if(this.testsFailed)this.__debug(w);
else this.__debug(J);
return !this.testsFailed;
}}});
})();
(function(){var o="",n="qx.debug",m="undefined",l="Variant",k='dataChanged',h="Environment",g="SmartTableModel.setData(): ",f="parameter must be an array of arrays.",d="changeView",c="on",G="Integer",F="function",E="_applyView",D="Alternate backing store",C='metaDataChanged',B="):",z=": ",y="Primary backing store",x="...",w=" (",t="...(no properties)",u="smart.model.Default",q="): (null)",s='or',p="boolean";
qx.Class.define(u,{extend:qx.ui.table.model.Simple,include:smart.MSmartUnitTests,construct:function(){qx.ui.table.model.Simple.call(this);
this.__qr=false;
this.__qs=[];
this.__qt=[];
this.__qu=[];
...
this.__qx=-1;
this.__qc=null;
this.addListener(d,this._changeView,this);
this.addView();
this._applyView(0,0,false,true);
},properties:{view:{init:0,check:G,apply:E,event:d}},members:{__qv:null,__qt:null,__qu:null,__kO:null,init:function(H){this.__kO=H;
H.setResetSelectionOnHeaderClick(false);
},_applyView:function(I,J,K,L,M){if(K===undefined)K=true;
...
if(I>=this.__qs.length){throw new Error("_applyView: view out of bounds: "+I+" (0.."+(this.__qs.length-1)+")");
}if(M)this.__qz(J);
this.__py=this.getRowArray(I);
if(K)this.__qT(I);
},_changeView:function(e){this.__qA();
},addView:function(N,O,P){var Q;
O=O||this;
if(N&&N instanceof Array){Q=function(T){var W=N.length;
if(P===s){for(var i=0;i<W;i++){if(N[i].call(O,T)){return true;
}}return false;
}else{for(var i=0;i<W;i++){if(!N[i].call(O,T)){return false;
}}return true;
}};
}else if(typeof (N)==F){Q=N;
}else{Q=null;
}var S=this.newView(Q,O);
return S;
},newView:function(X,Y,ba){if(ba&&ba.fSort){ba.fSort=qx.lang.Function.bind(ba.fSort,Y);
}var bc={context:Y,fFilter:(X?qx.lang.Function.bind(X,Y):null),advanced:ba||{},associations:{}};
this.__qt.push([]);
this.__qu.push(null);

for(var bb in this.__qv){this.__qv[bb].push({});
}var bd=this.__qs.length;
this.__qs.push(bc);
if(this.getView()==bd){this.__qz();
}this.__qO(bd);
if(this.getView()==bd){this.__qA();
}return bd;
},updateView:function(be){if(this.getView()==be){this.__qz();
}this.__qO(be);
if(this.getView()==be){this.__qA();
}},resetFilters:function(bf){this.setFilters(bf);
},getViewCount:function(){return this.__qs.length;
},addIndex:function(bg){var A=[];

for(var v=0;v<this.__qs.length;v++){A.push({});
}this.__qv[bg]=A;
this._updateAssociationMaps(undefined,bg);
},locate:function(bh,bi,bj){if(bj===undefined){bj=this.getView();
}return this.__qv[bh][bj][o+bi];
},indexedSelection:function(bk,bl){this.__qx=bk;
this.__qc=bl;
this.__qw=[];
this.__qy=false;
},suspendIndexedSelection:function(bm){this.__qy=bm;
},__qz:function(bn){if(bn===undefined){bn=this.getView();
}
if(!this.__qc||this.__qx<0||this.__qx>=this.getColumnCount()){return;
}this.__qw=[];
this.__qc.iterateSelection(function(bo){this.__qw.push(this.getValue(this.__qx,bo,bn));
},this);
},__qA:function(bp){if(this.__qw==null||!this.__qc||this.__qx<0||this.__qx>=this.getColumnCount()){return;
}var br=this.__qc;
br.setBatchMode(true);
if(!this.__qy){this.__qB();
var bq=this.__qw;
for(var i=0;i<bq.length;i++){var bs=this.locate(this.__qx,bq[i],bp);
if(bs!==undefined){br.addSelectionInterval(bs,bs);
}}}br.setBatchMode(false);
},__qB:function(){var bt=this.__qc;

if(bt){bt.resetSelection();
}},getRowArray:function(bu,bv){if(bu===undefined||bu===null){bu=this.getView();
}
if(typeof bv==p){return (bv?this.__qu[bu]:this.__qt[bu]);
}else{var bw=this.__qu[bu]||this.__qt[bu];
return bw;
}},setAlternateRowArray:function(bx,by){if(bx===undefined||bx===null){bx=this.getView();
}this.__qu[bx]=(by||null);

if(this.__qu[bx]){this.__qu[bx].__qC=D;
}return this.__qu[bx];
},__qD:function(bz,A,bA){if(bA===undefined){bA=false;
}var bB=false;
if(bz==this.getView()){bB=true;
}this.__qt[bz]=A;

if(bB){this._applyView(bz,bz,true,true,bA);
}},__qE:function(bC){if(bC===undefined){bC=this.getView();
}
if(bC<this.__qs.length){return this.__qs[bC].associations;
}else{return undefined;
}},__qF:function(bD){if(bD===undefined){bD=this.getView();
}
if(bD<this.__qs.length){return this.__qs[bD].fFilter;
}else{return undefined;
}},getRowData:function(bE,bF,bG){if(bF===undefined){bF=this.getView();
}
if(bG===undefined){bG=true;
}var bH=this.getRowCount(bF);

if(bE<0||bE>=bH){throw new Error("rowIndex out of bounds: "+bE+" (0.."+(bH-1)+")");
}var bI=this.getRowArray(bF);

if(bI[bE]===undefined){this.debug("No entry in "+bI.__qC+" for row "+bE);
return null;
}return (bG?bI[bE].slice(0):bI[bE]);
},getRowReference:function(bJ,bK){return this.getRowData(bJ,bK,false);
},getRowCount:function(bL){if(bL===undefined){bL=this.getView();
}var bM=this.getRowArray(bL);
return bM.length;
},getValue:function(bN,bO,bP){if(bP===undefined){bP=this.getView();
}var bQ=this.getRowCount(bP);

if(bO<0||bO>=bQ){throw new Error("this.__rowArr out of bounds: "+bO+" (0.."+bQ+")");
}return this.getRowArray(bP)[bO][bN];
},getValueById:function(bR,bS,bT){if(bT===undefined){bT=this.getView();
}return this.getValue(this.getColumnIndexById(bR),bS,bT);
},__qG:function(bU,bV,bW){if(bW===undefined){bW=true;
}var i;
var A=this.getRowArray(bU);

if(bV.length>=(A.length>>2)){var bX={};

for(i=0;i<bV.length;i++){var ca=this.__qS(bU,bV[i]);

if(ca!==undefined){bX[ca]=1;
}}var cd=[];

for(i=0;i<A.length;i++){if(!bX[i]){cd.push(A[i]);
}}this.__qD(bU,cd);
}else{var bX=[];

for(i=0;i<bV.length;i++){var ca=this.__qS(bU,bV[i]);

if(ca!==undefined){bX.push(ca);
}}bX.sort(function(a,b){return b-a;
});
var cb=bX.length;
var bY;
A=this.getRowArray(bU);

for(i=0;i<cb;i+=bY){var cc=bX[i];
var ce=cc;
bY=1;

for(var j=1;i+j<cb;j++){if(bX[i+j]==ce-j){bY++;
}else{break;
}}A.splice(cc-bY+1,bY);
}}if(bW){this._updateAssociationMaps(bU);
}},__qH:function(cf,cg,ch){if(ch===undefined){ch=true;
}var A=this.getRowArray(cf);
var ci=A.length;
A.push.apply(A,cg);

if(ch){this._updateAssociationMapsAfterPush(cf,cg,ci);
}},__qI:function(cj,ck,cl){if(cl===undefined){cl=true;
}var A=this.getRowArray(cj);
A.unshift.apply(A,ck);
if(cl){this._updateAssociationMaps(cj);
}},__qJ:function(cm,cn,co,cp,cq){if(co===undefined){co=true;
}
if(cp===undefined){cp=false;
}
if(cq===undefined){cq=true;
}if(co&&cm){cn=this.__qM(cm,cn,false);
}if(cn.length==0){return;
}var cr=this.__qs[cm];
if(cr.advanced.fPreInsertRows){cr.advanced.fPreInsertRows.call(cr.context,cm,this.getRowArray(cm),cn,this);
}if(!this.isSorted()&&!cr.advanced.fSort){this.__qH(cm,cn,cq);
return;
}var ct=cr.advanced.fSort||this.getComparator();

if(!cp||cr.advanced.fSort){cn.sort(ct);
}var A=this.getRowArray(cm);

if(!A.length||ct(cn[0],A[A.length-1])>=0){this.__qH(cm,cn,cq);
}else if(ct(cn[cn.length-1],A[0])<=0){this.__qI(cm,cn,cq);
}else{if(cn.length<(A.length>>1)){var cx=cn.length;
var cu=0,cz=A.length-1;
var cy=[];
var i;
for(i=0;i<cx;i++){var cB=this.__qU(A,cn[i],ct,cu,cz);

if(cB>=0){}else{cB=-cB-1;
}cy.push(cB);
cu=cB;
}var cs;

for(i=cx-1;i>=0;i-=cs){var cG=cy[i];
var cv=[cy[i],0];
var cD=[cn[i]];
cs=1;

for(var j=1;i-j>=0;j++){if(cy[i-j]==cG){cs++;
cD.push(cn[i-j]);
}else{break;
}}cD.reverse();
cv.push.apply(cv,cD);
...
var cA=0,cE=A.length;
var cC=cn.length;

for(i=0;i<cC;i++){var R=cn[i];
while(cA<cE){var cw=ct(R,A[cA]);

if(cw<0){cF.push(R);
break;
}cF.push(A[cA++]);
}if(cA==cE){cF.push(R);
}}if(cA<cE){cF.push.apply(cF,A.slice(cA));
}this.__qD(cm,cF);
}
if(cq){this._updateAssociationMaps(cm);
}}if(cr.advanced.fPostInsertRows){cr.advanced.fPostInsertRows.call(cr.context,cm,this.getRowArray(cm),cn,this);
}},__qK:function(cH,cI,V,cJ){if(cJ===undefined){cJ=this.getView();
}var cM=this.getColumnCount();
var R=this.getRowReference(cI,cJ);

if(R===undefined){throw new Error("__set: "+"could not find the row corresponding to index "+cI+" in view "+cJ);
return;
}this.__qz();
var cK=false;
if(cH>=0&&cH<cM){if(R[cH]===V){return ;
}else{if(this.__qv[cH]!==undefined){this.__qR(cH,R[cH],V);
}R[cH]=V;
}cK=(this.isSorted()&&(cH===this.getSortColumnIndex()));
}else{for(var cL=0;cL<V.length&&cL<cM;cL++){if(this.__qv[cL]!==undefined){this.__qR(cL,R[cL],V[cL]);
}R[cL]=V[cL];
}if(this.isSorted()&&V.length>=this.getSortColumnIndex()){cK=true;
}}this.__qL(R,cK);
this.__qA();
},__qL:function(R,cN,cO,cP){if(cO===undefined){cO=false;
}
if(cP===undefined){cP=true;
}for(var v=(cO?1:0);v<this.__qs.length;v++){var cS=this.__qS(v,R);
var cQ,cR;

if(v==0){cQ=false;
cR=false;
}else{cQ=(this.__qS(v,R)===undefined);
cR=this.__qN(v,R);
}if(cN){if(!cQ){this.__qG(v,[R]);
}if(!cR){this.__qJ(v,[R],false);
}}else{if(cQ!=cR){if(cR){this.__qG(v,[R]);
}else{this.__qJ(v,[R],false);
}}}if(cP){this.__qT();
}}},setValue:function(cT,cU,cV,cW){this.__qK(cT,cU,cV,cW);
},setValueById:function(cX,cY,da,db){return this.setValue(this.getColumnIndexById(cX),cY,da,db);
},setRow:function(dc,dd,de){this.__qK(-1,dc,dd,de);
},setRows:function(df,dg,dh){if(dh===undefined){dh=this.getView();
}for(var i=0;i<df.length;i++){this.setRow(dg+i,df[i],dh);
}},setData:function(di,dj){if(dj===undefined){dj=true;
}
if(di==null||di.length==0){this.clearAllRows();
this.__qB();
return;
}var A=[];
...
for(var i=0;i<di.length;i++){A.push(di[i].slice(0));
}}else{A=di.slice(0);
}this.addRows(A,false,false);
},addRows:function(dk,dl,dm){if(typeof (qx.core.Variant)!==m){if(qx.core[l].isSet(n,c)){this.assertArray(dk[0],g+f);
}}
if(typeof (qx.core.Environment)!==m){if(qx.core[h].get(n)){this.assertArray(dk[0],g+f);
}}
if(dl===undefined){dl=true;
}
if(dm===undefined){dm=true;
}
if(!dk||dk.length==0){return;
}this.__qz();
var dq=(new Date()).getTime();
var A=dl?[]:dk;

if(dl){for(var i=0;i<dk.length;i++){A.push(dk[i].slice(0));
}}this.assignRowIDs(A);
var dp=this.getComparator();
A.sort(dp);
for(var v=0;v<this.__qs.length;v++){this.setAlternateRowArray(v,null);
this.__qJ(v,A,true,true);
}this.__qA();

if(dm){this.__qT();
}var dn=(new Date()).getTime();
},removeRows:function(dr,ds,dt,du){if(dt===undefined){dt=this.getView();
}
if(du===undefined){du=true;
}var A=this.getRowArray(dt);

if(dr===undefined){dr=0;
}
if(ds===undefined){ds=A.length-dr;
}
if(A.length==0){throw new Error("removeRows: attempt to remove rows from empty view");
}
if(dr<0||dr>A.length){throw new Error("removeRows: startIndex out of bounds: "+dr+" (0.."+(A.length-1)+")");
}
if(ds<0||dr+ds-1>=A.length){throw new Error("removeRows: howMany out of bounds: "+ds+" (0.."+(A.length-dr)+")");
}var dv=[];

for(var i=0;i<ds;i++){dv.push(A[dr+i]);
}this.removeReferencedRows(dv,du);
},removeReferencedRows:function(dw,dx){if(dx===undefined){dx=true;
}this.__qz();

for(var v=0;v<this.__qs.length;v++){var dy=this.__qs[v];
if(dy.advanced.fPreRemoveRows){dy.advanced.fPreRemoveRows.call(dy.context,v,this.getRowArray(v,false),dw,this);
}this.setAlternateRowArray(v,null);
this.__qG(v,dw);
if(dy.advanced.fPostRemoveRows){dy.advanced.fPostRemoveRows.call(dy.context,v,this.getRowArray(v),this);
}}this.__qA();

if(dx){this.__qT();
}},clearAllRows:function(){if(this.getRowCount()>0){this.__qB();

for(var v=0;v<this.__qs.length;v++){this.__qt[v]=[];
this.__qt[v].__qC=y;
this.__qu[v]=null;
}this._updateAssociationMaps();
this.__qT();
}},forceRedraw:function(){this.__qT();
},getRowId:function(dz){return dz.__cq;
},getRowById:function(dA,dB){return this.getRowArray(dA,false)[this.__qs[dA].associations[dB]];
},__qM:function(dC,R,dD){if(dD===undefined){dD=true;
}var dE=this.__qF(dC);
if(!dE){return dD?true:qx.lang.Array.clone(R);
}if(dD){return dE(R);
}if(dD){return dE(R);
}var dF=R.length;
var dG=[];

for(var r=0;r<dF;r++){var dH=R[r];

if(dE(dH)){dG.push(dH);
}}return dG;
},__qN:function(dI,R){return !this.__qM(dI,R,true);
},__qO:function(dJ,dK,dL){if(dJ==0){return;
}
if(dK===undefined){dK=true;
}
if(dL===undefined){dL=true;
}var U=this.getRowArray(0);
this.__qD(dJ,this.__qM(dJ,U,false));
if(dL){this._updateAssociationMaps(dJ);
}if(dK&&this.getView()==dJ){this.__qT(dJ);
}},__qP:function(dM,dN){for(var v=1;v<this.__qs.length;v++){this.__qO(v,dM,dN);
}},__qQ:1,assignRowIDs:function(A){for(var i=0;i<A.length;i++){A[i].__cq=this.__qQ++;
}},_updateAssociationMaps:function(dO,dP){for(var v=0;v<this.__qs.length;v++){if(dO!==undefined&&dO!=v){continue;
}var A=this.getRowArray(v);
if(dP===undefined){this.__qs[v].associations={};
}for(var dQ in this.__qv){if(dP===undefined||dP==dQ){this.__qv[dQ][v]={};
}}for(var j=0;j<A.length;j++){var R=A[j];

if(dP===undefined){this.__qs[v].associations[R.__cq]=j;
}for(dQ in this.__qv){if(dP===undefined||dP==dQ){this.__qv[dQ][v][o+R[dQ]]=j;
}}}}},_updateAssociationMapsAfterPush:function(dR,dS,dT){var dV=this.__qE(dR);
var dW=dT;
for(var i=0;i<dS.length;i++,dW++){var R=dS[i];
dV[R.__cq]=dW;
for(var dU in this.__qv){this.__qv[dU][dR][o+R[dU]]=dW;
}}},__qR:function(dX,dY,ea){dY=o+dY;
ea=o+ea;

for(var ed=0;ed<this.__qs.length;ed++){for(var eb in this.__qv){var ec=this.__qv[eb][ed][dY];
this.__qv[eb][ed][dY]=undefined;
this.__qv[eb][ed][ea]=ec;
}}},__qS:function(ee,R){try{if(R.__cq===undefined){return undefined;
}}catch(e){this.__qV(e);
return undefined;
}return this.__qE(ee)[R.__cq];
},isSorted:function(){return this.getSortColumnIndex()!=-1;
},getComparator:function(ef,eg){if(ef===undefined){ef=this.getSortColumnIndex();
}
if(eg===undefined){eg=this.isSortAscending();
}var ej;
var ei=this.getSortMethods(ef);
var eh=qx.ui.table.model.Simple;

if(ei){ej=(eg?ei.ascending:ei.descending);
}else if(this.getCaseSensitiveSorting()){ej=(eg?eh._defaultSortComparatorAscending:eh._defaultSortComparatorDescending);
}else{ej=(eg?eh._defaultSortComparatorInsensitiveAscending:eh._defaultSortComparatorInsensitiveDescending);
}ej.columnIndex=ef;
return ej;
},sortByColumn:function(ek,el,em){if(em===undefined){em=false;
}this.__qz();
if(!em&&this.getSortColumnIndex()==ek){if(this.isSortAscending()==el){return ;
}for(var v=0;v<this.__qs.length;v++){this.getRowArray(v).reverse();
}this._setSortAscending(el);
}else{this._setSortColumnIndex(ek);
this._setSortAscending(el);
var en=this.getComparator();
en.columnIndex=ek;
for(var v=0;v<this.__qs.length;v++){this.getRowArray(v).sort(en);
}}this._updateAssociationMaps();
this.__qA();
this.fireEvent(C);
},_resort:function(){if(this.isSorted()){this.sortByColumn(this.getSortColumnIndex(),this.isSortAscending(),true);
}},setSortMethods:function(eo,ep){arguments.callee.base.apply(this,arguments);
if(eo==this.getSortColumnIndex()){this._resort();
}},__qT:function(eq){if(this.hasListener(k)){if(eq===undefined){eq=this.getView();
}var er={firstRow:0,lastRow:this.getRowCount(eq)-1,firstColumn:0,lastColumn:this.getColumnCount()-1};
this.fireDataEvent(k,er);
}},__qU:function(A,e,es,et,eu){if(et===undefined){et=0;
}
if(eu===undefined){eu=A.length-1;
}
while(et<=eu){var ev=(et+eu)>>>1;
var ew=es(A[ev],e);

if(ew<0){et=ev+1;
}else if(ew>0){eu=ev-1;
}else{return ev;
}}return -(et+1);
},__qV:function(ex){if(typeof (qx.core.Variant)!==m){if(qx.core[l].isSet(n,c)){if(this.__qr){this.debug(ex);
}}}
if(typeof (qx.core.Environment)!==m){if(qx.core[h].get(n)){if(this.__qr){this.debug(ex);
}}}},__qW:function(ey,ez,eA){if(ez===undefined){ez=o;
}
if(eA===undefined){eA=true;
}this.__qV(ez+w+(ey?(ey+B):q));

if(ey==null||ey===undefined){this.__qV(t);
return;
}
for(var eB in ey){if(eA&&!ey.hasOwnProperty(eB)){continue;
}this.__qV(x+eB+z+ey[eB]);
...
(function(){var k="Port",j="Intersted",i="Pid",h="State",g="IP",f="Choking",e="rhyacotriton.peers.Table",d="Is choking?",c="dataLoadCompleted",b="Id",a="peerDataLoadCompleted";
qx.Class.define(e,{extend:rhyacotriton.BasicTable,construct:function(l){var m={"id":this.tr(i),"torrent_id":this.tr(b),"ip":this.tr(g),"port":this.tr(k),"state":this.tr(h),"choke_state":this.tr(f),"interest_state":this.tr(j),"local_choke":this.tr(d)};
rhyacotriton.BasicTable.call(this,m);
var o=this.getTableColumnModel();
var p=this.getColumnNameToPositionIndex();
o.setColumnWidth(p.id,70,true);
o.setColumnWidth(p.ip,90,true);
o.setColumnWidth(p.port,50,true);
o.setColumnWidth(p.torrent_id,30,true);
o.setColumnWidth(p.state,60,true);
o.setColumnWidth(p.interest_state,60,true);
o.setColumnWidth(p.choke_state,60,true);
o.setColumnWidth(p.local_choke,40,true);
o.setDataCellRenderer(p.local_choke,new qx.ui.table.cellrenderer.Boolean());
[p.interest_state,p.choke_state,p.local_choke].map(function(q){o.setColumnVisible(q,false);
});
var n=this.getTableModel();
n.setSortMethods(p.id,l.buildPidComparator(p.id));
n.setSortMethods(p.ip,l.buildIPComparator(p.ip));
l.addListener(a,this.getEventHandler(c),this);
},members:{}});
})();
...
},init:i}},members:{__re:16,__rf:16,__rg:null,_insetY:2,_identifyImage:function(A){throw new Error("_identifyImage is abstract");
},_getImageInfos:function(B){var C=this._identifyImage(B);
if(C==null||typeof C==e){C={url:C,tooltip:null};
}if(!C.imageWidth||!C.imageHeight){var D=this.__rh(C.url);
C.imageWidth=D.width;
C.imageHeight=D.height;
}C.width=C.imageWidth;
C.height=C.imageHeight;
return C;
},__rh:function(E){var H=qx.util.ResourceManager.getInstance();
...
},createDataCellHtml:function(J,K){this.__rg=this._getImageInfos(J);
...
if(this.__rg.url){content=qx.bom.element.Decoration.create(this.__rg.url,this.getRepeat(),{width:this.__rg.width+j,height:this.__rg.height+j,display:qx.core.Environment.get(h),verticalAlign:q,position:d});
}return content;
},_getCellAttributes:function(N){var O=this.__rg.tooltip;
...
this.__ri=qx.util.AliasManager.getInstance();
this.initIconTrue();
this.initIconFalse();
},properties:{iconTrue:{check:g,init:e,apply:f},iconFalse:{check:g,init:b,apply:a}},members:{__rj:null,__rk:false,__ri:null,_applyIconTrue:function(h){this.__rj=this.__ri.resolve(h);
},_applyIconFalse:function(i){this.__rk=this.__ri.resolve(i);
...
switch(k.value){case true:l.url=this.__rj;
break;
case false:l.url=this.__rk;
break;
default:l.url=null;
break;
}return l;
...
(function(){var j="dataUpdated",i="dataAdded",h="dataLoadCompleted",g="dataRemoved",f="dataRemoveFailure",e="Out Before",d="Speed Out",c="Left",b="Id",a="Ss",y="Total",x="Speed In",w="Total In",v="In Now",u="Ls",t="Rating",r="Progress",q="In Before",p="Total Out",o="Name",m="rhyacotriton.Table",n="Out Now",k="State",l="On-line";
qx.Class.define(m,{extend:rhyacotriton.BasicTable,construct:function(z){var A={"id":this.tr(b),"name":this.tr(o),"total":this.tr(y),"left":this.tr(c),"progress":this.tr(r),"rating":this.tr(t),"online":this.tr(l),"seeders":this.tr(a),"leechers":this.tr(u),"state":this.tr(k),"downloaded":this.tr(v),"uploaded":this.tr(n),"all_time_downloaded":this.tr(q),"all_time_uploaded":this.tr(e),"sum_downloaded":this.tr(w),"sum_uploaded":this.tr(p),"speed_in":this.tr(x),"speed_out":this.tr(d)};
rhyacotriton.BasicTable.call(this,A);
var B=this.getTableColumnModel();
var C=this.getColumnNameToPositionIndex();
B.setColumnWidth(C.name,250,true);
B.setColumnWidth(C.id,30,true);
B.setColumnWidth(C.progress,65,true);
B.setColumnWidth(C.total,70,true);
B.setColumnWidth(C.left,70,true);
B.setColumnWidth(C.leechers,40,true);
B.setColumnWidth(C.seeders,40,true);
B.setColumnWidth(C.state,70,true);
B.setColumnWidth(C.rating,60,true);
B.setColumnWidth(C.all_time_downloaded,65,true);
B.setColumnWidth(C.all_time_uploaded,70,true);
B.setColumnWidth(C.downloaded,70,true);
B.setColumnWidth(C.uploaded,70,true);
B.setColumnWidth(C.sum_downloaded,70,true);
B.setColumnWidth(C.sum_uploaded,70,true);
B.setColumnWidth(C.speed_in,70,true);
B.setColumnWidth(C.speed_out,70,true);
B.setDataCellRenderer(C.progress,new rhyacotriton.cellrenderer.Progress());
B.setDataCellRenderer(C.rating,new rhyacotriton.cellrenderer.Rating());
B.setDataCellRenderer(C.online,new qx.ui.table.cellrenderer.Boolean());
[C.total,C.left,C.downloaded,C.uploaded,C.all_time_downloaded,C.all_time_uploaded,C.sum_downloaded,C.sum_uploaded].map(function(D){B.setDataCellRenderer(D,new rhyacotriton.cellrenderer.Size());
});
B.setDataCellRenderer(C.speed_in,new rhyacotriton.cellrenderer.Speed());
B.setDataCellRenderer(C.speed_out,new rhyacotriton.cellrenderer.Speed());
[C.left,C.online,C.downloaded,C.uploaded,C.all_time_downloaded,C.all_time_uploaded].map(function(E){B.setColumnVisible(E,false);
});
var s=this.__kQ=z;
s.addListener(i,this.getEventHandler(i),this);
s.addListener(g,this.getEventHandler(g),this);
s.addListener(f,this.getEventHandler(f),this);
s.addListener(h,this.getEventHandler(h),this);
s.addListener(j,this.getEventHandler(j),this);
},members:{__kQ:undefined,__qd:function(F){var I=this.getColumnNameToPositionIndex();
var G=F[I.total];
var J=F[I.left];
var H=G-J;
return ((H/G)*100).toFixed(2);
},__qe:function(K){var R=this.getColumnNameToPositionIndex();
var Q=K[R.uploaded];
var L=K[R.downloaded];
var O=K[R.all_time_downloaded];
var N=K[R.all_time_uploaded];
var P=L+N;
var M=Q+O;

if(P==0)return 0;
return (M/P).toFixed(2);
},__qf:function(S){var U=this.getColumnNameToPositionIndex();
var T=S[U.downloaded];
var V=S[U.all_time_downloaded];
return T+V;
},__qg:function(W){var Y=this.getColumnNameToPositionIndex();
var X=W[Y.uploaded];
var ba=W[Y.all_time_uploaded];
return X+ba;
},startSelectedRows:function(){this.info("startSelectedRows");
this.__kQ.start(this.getSelectedIds());
},stopSelectedRows:function(){this.info("stopSelectedRows");
this.__kQ.stop(this.getSelectedIds());
},fillFields:function(bb,bc,bd){bc=rhyacotriton.BasicTable.prototype.fillFields.call(this,bb,bc,bd);
var be=this.getColumnNameToPositionIndex();

if(bd){bc[be.speed_in]=0;
bc[be.speed_out]=0;
}bc[be.sum_uploaded]=this.__qg(bc);
bc[be.sum_downloaded]=this.__qf(bc);
bc[be.rating]=this.__qe(bc);
bc[be.progress]=this.__qd(bc);
return bc;
}}});
})();
...
(function(){var c="%",b="qooxdoo-table-cell-right qooxdoo-table-cell",a="rhyacotriton.cellrenderer.Progress";
qx.Class.define(a,{extend:qx.ui.table.cellrenderer.Conditional,members:{__rd:null,_getContentHtml:function(d){return d.rowData[d.col]+c;
},_getCellClass:function(e){return b;
}}});
})();
(function(){var b="qooxdoo-table-cell-right qooxdoo-table-cell",a="rhyacotriton.cellrenderer.Rating";
qx.Class.define(a,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(c){return c.rowData[c.col];
},_getCellClass:function(d){return b;
}}});
})();
(function(){var g=' B',f="qooxdoo-table-cell-right qooxdoo-table-cell",e="rhyacotriton.cellrenderer.Size",d=' TB',c=' KB',b=' GB',a=' MB';
qx.Class.define(e,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(h){return this.bytesToSize(h.rowData[h.col],2);
},_getCellClass:function(i){return f;
},bytesToSize:function(j,k){var l=1024;
var m=l*1024;
var n=m*1024;
var o=n*1024;

if((j>=0)&&(j<l)){return j+g;
}else if((j>=l)&&(j<m)){return (j/l).toFixed(k)+c;
}else if((j>=m)&&(j<n)){return (j/m).toFixed(k)+a;
}else if((j>=n)&&(j<o)){return (j/n).toFixed(k)+b;
}else if(j>=o){return (j/o).toFixed(k)+d;
}else{return j+g;
}}}});
})();
(function(){var e=' B/s',d="rhyacotriton.cellrenderer.Speed",c="qooxdoo-table-cell-right qooxdoo-table-cell",b='0',a=' KiB/s';
qx.Class.define(d,{extend:qx.ui.table.cellrenderer.Conditional,members:{_getContentHtml:function(f){return this.bytesToSize(f.rowData[f.col],0);
},_getCellClass:function(g){return c;
},bytesToSize:function(h,i){if(isNaN(h)||(h==0))return b;
var j=1024;

if((h>=0)&&(h<j)){return h+e;
}else{return (h/j).toFixed(i)+a;
}}}});
})();
...
},properties:{appearance:{refine:true,init:u},offset:{check:b,init:6,apply:v},orientation:{init:i,check:[i,g],apply:w}},members:{__rl:null,__rm:false,__rn:null,__ro:null,__nM:null,__rp:null,__rq:null,__ff:null,__ka:null,_createChildControlImpl:function(F,G){var H;
...
this._add(H,{type:F});
H.addListener(C,this.__rr,this);
break;
}return H||qx.ui.core.Widget.prototype._createChildControlImpl.call(this,F);
},__rr:function(e){this.__rt(e.getData());
},__rs:function(I){this.__ka=new qx.ui.splitpane.Blocker(I);
this.getContentElement().add(this.__ka);
var J=this.getChildControl(j);
var K=J.getWidth();

if(!K){J.addListenerOnce(p,function(){this.__rt();
},this);
...
this.__rt();
},_applyOffset:function(S,T){this.__rt();
},__rt:function(U){var V=this.getChildControl(j);
var ba=this.getOffset();
var bb=V.getBounds();
var Y=V.getContainerElement().getDomElement();
if(!Y){return;
}if(this.__nM){var X=null;
...
this.__rl=this.__nM?e.getDocumentLeft()-bi.left+bh.left:e.getDocumentTop()-bi.top+bh.top;
...
if(this.__rm){this.__ru();
var bl=this.getChildControl(k);
var bm=this.__rp;

if(this.__nM){bl.setDomLeft(bm);
...
},_onMouseUp:function(e){if(!this.__rm){return;
}this._finalizeSizes();
...
this.releaseCapture();
e.stop();
},_finalizeSizes:function(){var br=this.__rp;
var bo=this.__rq;
...
}else{if(this.__nM){bs.setWidth(br);
bp.setWidth(bo);
}else{bs.setHeight(br);
bp.setHeight(bo);
}}},__ru:function(){if(this.__nM){var bx=o,bE=z,by=l,bC=this.__rn;
}else{var bx=q,bE=a,by=B,bC=this.__ro;
...
this.__rq=bw;
},_isActiveDragSession:function(){return this.__rm;
},_setLastMousePosition:function(x,y){this.__rn=x;
this.__ro=y;
...
(function(){var d="qx.event.type.Data",c=".",b="qx.event.type.Event",a="rhyacotriton.store.Abstract";
qx.Class.define(a,{extend:qx.core.Object,construct:function(){},members:{__rv:true,reconnect:function(){},reload:function(){},reloadPeers:function(){},removeElement:function(e){},stop:function(f){},start:function(g){},isActive:function(){return this.__rv;
},setActive:function(h){this.__rv=h;
},buildPidComparator:function(j){return function(k,l){var o=k[j];
var p=l[j];
o=o.substr(2,o.length-1).split(c);
p=p.substr(2,p.length-1).split(c);
var q=[0,2,1];

for(var i=0;i<3;i++){var n=q[i];
var r=parseInt(o[n]);
var m=parseInt(p[n]);
...
},buildIPComparator:function(s){return function(t,u){var w=t[s].split(c);
var v=u[s].split(c);

for(var i=0;i<4;i++){var y=parseInt(w[i]);
var x=parseInt(v[i]);
...
},events:{"execute":R},properties:{enabled:{init:true,check:l,event:o,apply:I},shortcut:{check:n,apply:Q,nullable:true},autoRepeat:{check:l,init:false}},members:{__rA:g,__rB:g,execute:function(U){this.fireDataEvent(L,U);
},__rC:function(event){if(this.getEnabled()&&this.__rD(event)){if(!this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},__fR:function(event){if(this.getEnabled()&&this.__rD(event)){if(this.isAutoRepeat()){this.execute(event.getTarget());
}event.stop();
}},_applyEnabled:function(V,W){if(V){qx.event.Registration.addListener(document.documentElement,h,this.__rC,this);
qx.event.Registration.addListener(document.documentElement,d,this.__fR,this);
}else{qx.event.Registration.removeListener(document.documentElement,h,this.__rC,this);
qx.event.Registration.removeListener(document.documentElement,d,this.__fR,this);
}},_applyShortcut:function(X,Y){if(X){if(X.search(/[\s]+/)!=-1){var bc=y;
this.error(bc);
throw new Error(bc);
}this.__rA={"Control":false,"Shift":false,"Meta":false,"Alt":false};
this.__rB=null;
var ba;
var a=[];
...
for(var i=0;i<bb;i++){var bd=this.__rF(a[i]);

switch(bd){case f:case u:case K:case C:this.__rA[bd]=true;
break;
case m:var bc=F+a[i];
this.error(bc);
throw bc;
default:if(this.__rB){var bc=r;
this.error(bc);
throw bc;
}this.__rB=bd;
}}}return true;
},__rD:function(e){var be=this.__rB;

if(!be){return ;
}if((!this.__rA.Shift&&e.isShiftPressed())||(this.__rA.Shift&&!e.isShiftPressed())||(!this.__rA.Control&&e.isCtrlPressed())||(this.__rA.Control&&!e.isCtrlPressed())||(!this.__rA.Meta&&e.isMetaPressed())||(this.__rA.Meta&&!e.isMetaPressed())||(!this.__rA.Alt&&e.isAltPressed())||(this.__rA.Alt&&!e.isAltPressed())){return false;
}
if(be==e.getKeyIdentifier()){return true;
}return false;
},__rE:{esc:S,ctrl:f,print:P,del:q,pageup:b,pagedown:E,numlock:O,numpad_0:J,numpad_1:D,numpad_2:t,numpad_3:s,numpad_4:A,numpad_5:N,numpad_6:B,numpad_7:H,numpad_8:M,numpad_9:z,numpad_divide:p,numpad_multiply:v,numpad_minus:c,numpad_plus:k},__rF:function(bf){var bg=qx.event.handler.Keyboard;
var bh=m;
...
var bh=this.__rE[bf]||qx.lang.String.firstUp(bf);

if(bg.isValidKeyIdentifier(bh)){return bh;
}else{return m;
}},toString:function(){var bk=this.__rB;
var bj=[];

for(var bi in this.__rA){if(this.__rA[bi]){bj.push(qx.locale.Key.getKeyName(j,bi));
}}
if(bk){bj.push(qx.locale.Key.getKeyName(j,bk));
}return bj.join(k);
}},destruct:function(){this.setEnabled(false);
this.__rA=this.__rB=null;
}});
})();
...
if(gX&&gS){gX+=en;
}return {zIndex:gQ.checked?10:5,decorator:gX,padding:gT,marginTop:gW,marginBottom:gR,marginLeft:gU,marginRight:gV,textColor:gQ.disabled?cC:gQ.checked?bf:cH};
...
return {textColor:cB,backgroundColor:undefined,placeMethod:ep,offset:[0,0,0,14],marginTop:-2,position:C,showTimeout:100,hideTimeout:10000,decorator:hI?K:dn,shadow:bT,font:eh,padding:hI?3:undefined};
...
}},"app-header":{style:function(jK){return {font:eh,textColor:cB,padding:[8,12],decorator:cJ};
}},"app-header-label":cA,"virtual-list":dK,"virtual-list/row-layer":cI,"row-layer":ep,"group-item":{include:cA,alias:cA,style:function(jL){return {padding:4,decorator:qx.core.Environment.get(eq)?eB:eM,textColor:bg,font:eh};
...
}},"virtual-tree-folder":cS,"virtual-tree-file":cr,"column-layer":ep,"cell":{style:function(jN){return {textColor:jN.selected?cB:eT,padding:[3,6],font:T};
...
(function(){var a="rhyacotriton.theme.Appearance";
...
if(qx.ui.decoration.css3.BorderImage.IS_SUPPORTED){this.__rG=new qx.ui.decoration.css3.BorderImage();

if(p){this.__rH(p);
}}else{this.__rG=new qx.ui.decoration.GridDiv(p);
}
if(q!=null){this.__rG.setInsets(q);
}},properties:{baseImage:{check:d,nullable:true,apply:j},insetLeft:{check:o,nullable:true,apply:n},insetRight:{check:o,nullable:true,apply:n},insetBottom:{check:o,nullable:true,apply:n},insetTop:{check:o,nullable:true,apply:n},insets:{group:[k,l,i,e],mode:f}},members:{__rG:null,getMarkup:function(){return this.__rG.getMarkup();
},resize:function(r,s,t){this.__rG.resize(r,s,t);
},tint:function(u,v){},getInsets:function(){return this.__rG.getInsets();
},_applyInsets:function(w,x,name){var y=g+qx.lang.String.firstUp(name);
this.__rG[y](w);
},_applyBaseImage:function(z,A){if(this.__rG instanceof qx.ui.decoration.GridDiv){this.__rG.setBaseImage(z);
}else{this.__rH(z);
}},__rH:function(B){var G,H,J,I;
this.__rG.setBorderImage(B);
...
this.__rG.setSlice([N,D,E,O]);
}},destruct:function(){this.__rG.dispose();
this.__rG=null;
}});
})();
...
this.__rI=[x,qx.bom.element.Style.compile({"borderImage":o+B+p+C.join(f)+f+D,position:k,lineHeight:0,fontSize:0,overflow:l,boxSizing:r,borderWidth:C.join(b)+g}),y].join(q);
return this.__rI;
...
}},members:{__rI:null,_getDefaultInsets:function(){return {top:0,right:0,bottom:0,left:0};
},_isInitialized:function(){return !!this.__rI;
},getMarkup:function(e){if(this.__rI){return this.__rI;
...
}},properties:{innerColor:{check:d,nullable:true,apply:i},innerOpacity:{check:o,init:1,apply:i},outerColor:{check:d,nullable:true,apply:i}},members:{__rI:null,_getDefaultInsets:function(){return {top:2,right:2,bottom:2,left:2};
},_isInitialized:function(){return !!this.__rI;
},_applyStyle:function(){},getMarkup:function(){if(this.__rI){return this.__rI;
...
qx.Mixin.define(o,{include:[qx.ui.decoration.MSingleBorder,qx.ui.decoration.MBackgroundImage],construct:function(){this._getDefaultInsetsForBorder=this.__rN;
this._resizeBorder=this.__rM;
this._styleBorder=this.__rK;
this._generateMarkup=this.__rL;
},properties:{innerWidthTop:{check:f,init:0},innerWidthRight:{check:f,init:0},innerWidthBottom:{check:f,init:0},innerWidthLeft:{check:f,init:0},innerWidth:{group:[w,z,n,k],mode:a},innerColorTop:{nullable:true,check:g},innerColorRight:{nullable:true,check:g},innerColorBottom:{nullable:true,check:g},innerColorLeft:{nullable:true,check:g},innerColor:{group:[u,v,x,r],mode:a}},members:{__rJ:null,__rK:function(D){{var F=qx.theme.manager.Color.getInstance();
...
}},__rL:function(K){var O=this._generateBackgroundMarkup(K);
...
K[y]=0;
K[p]=0;
return this.__rJ=this._generateBackgroundMarkup(K,O);
},__rM:function(S,T,U){var V=this.getInsets();
T-=V.left+V.right;
U-=V.top+V.bottom;
...
},__rN:function(){return {top:this.getWidthTop()+this.getInnerWidthTop(),right:this.getWidthRight()+this.getInnerWidthRight(),bottom:this.getWidthBottom()+this.getInnerWidthBottom(),left:this.getWidthLeft()+this.getInnerWidthLeft()};
}}});
})();
...
(function(){var a="rhyacotriton.theme.Decoration";
...
(function(){var a="rhyacotriton.theme.Color";
...
(function(){var a="rhyacotriton.theme.Font";
qx.Theme.define(a,{extend:qx.theme.modern.Font,fonts:{}});
})();
(function(){var a="rhyacotriton.theme.Theme";
qx.Theme.define(a,{meta:{color:rhyacotriton.theme.Color,decoration:rhyacotriton.theme.Decoration,font:rhyacotriton.theme.Font,icon:qx.theme.icon.Tango,appearance:rhyacotriton.theme.Appearance}});
})();
