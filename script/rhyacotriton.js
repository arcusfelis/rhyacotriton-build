...
(function(){var l='undefined',k="Ls",h="dataUpdated",g="Out Before",f="total",e="seeders",d="rating",c="Speed Out",b="Left",a="sum_uploaded",U="Id",T="speed_out",S="downloaded",R="Ss",Q="dataAdded",P="all_time_uploaded",O="Total",N="leechers",M="Speed In",L="progress",s="uploaded",t="online",q="dataLoadCompleted",r="Total In",o="all_time_downloaded",p="In Now",m="name",n="sum_downloaded",u="left",v="Rating",C="state",A="Progress",F="speed_in",E="In Before",H="Total Out",G="Name",x="rhyacotriton.Table",K="Out Now",J="Position ",I="dataRemoved",w="State",y="dataRemoveFailure",z="On-line",B="id",D="object";
qx.Class.define(x,{extend:qx.ui.table.Table,construct:function(V){this.__kL=V;
var W=this;
this.__pp=new smart.model.Default;
this.__rh={"id":U,"name":G,"total":O,"left":b,"progress":A,"rating":v,"online":z,"seeders":R,"leechers":k,"state":w,"downloaded":p,"uploaded":K,"all_time_downloaded":E,"all_time_uploaded":g,"sum_downloaded":r,"sum_uploaded":H,"speed_in":M,"speed_out":c},this.__rm=[];
this.__ri=[];
this.__rj=[];
var i=0;

for(var name in this.__rh){var X=this.__rh[name];
this.__rm[name]=i;
this.__ri[i]=name;
this.__rj[i]=X;
i++;
...
this.__pv=this.__pp.getColumnIndexById(B);
this.__qL=this.__pp.getColumnIndexById(m);
this.__pq=this.__pp.getColumnIndexById(t);
this.__pr=this.__pp.getColumnIndexById(u);
this.__ps=this.__pp.getColumnIndexById(f);
this.__pt=this.__pp.getColumnIndexById(L);
this.__qM=this.__pp.getColumnIndexById(N);
this.__qN=this.__pp.getColumnIndexById(e);
this.__qU=this.__pp.getColumnIndexById(S);
this.__qV=this.__pp.getColumnIndexById(s);
this.__qX=this.__pp.getColumnIndexById(n);
this.__qY=this.__pp.getColumnIndexById(a);
this.__rc=this.__pp.getColumnIndexById(o);
this.__rd=this.__pp.getColumnIndexById(P);
this.__qR=this.__pp.getColumnIndexById(d);
this.__qW=this.__pp.getColumnIndexById(C);
this.__rf=this.__pp.getColumnIndexById(F);
this.__rg=this.__pp.getColumnIndexById(T);
...
[this.__ps,this.__pr,this.__qU,this.__qV,this.__rc,this.__rd,this.__qX,this.__qY].map(function(Y){W.__pf.setDataCellRenderer(Y,new rhyacotriton.cellrenderer.Size());
});
W.__pf.setDataCellRenderer(this.__rf,new rhyacotriton.cellrenderer.Speed());
W.__pf.setDataCellRenderer(this.__rg,new rhyacotriton.cellrenderer.Speed());
[this.__pr,this.__qU,this.__qV,this.__rc,this.__rd,this.__pq].map(function(ba){W.__pf.setColumnVisible(ba,false);
});
...
this.__kL.addListener(Q,this.__qO,this);
this.__kL.addListener(I,this.__py,this);
this.__kL.addListener(y,this.__pz,this);
this.__kL.addListener(q,this.__px,this);
this.__kL.addListener(h,this.__pA,this);
},members:{__kL:null,__pv:null,__qL:null,__pq:null,__pr:null,__ps:null,__pt:null,__qM:null,__qN:null,__qW:null,__qR:null,__rc:null,__rd:null,__qU:null,__qV:null,__qX:null,__qY:null,__rf:null,__rg:null,__pp:null,__pf:null,__pu:null,__qS:function(bb){var bd=bb[this.__ps];
var be=bb[this.__pr];
var bc=bd-be;
return ((bc/bd)*100).toFixed(2);
},__qT:function(bf){var bl=bf[this.__qV];
var bg=bf[this.__qU];
var bj=bf[this.__rd];
var bi=bf[this.__rc];
var bk=bg+bi;
...
if(bk==0)return 0;
return (bh/bk).toFixed(2);
},__ra:function(bm){var bn=bm[this.__qU];
var bo=bm[this.__rc];
return bn+bo;
},__rb:function(bp){var bq=bp[this.__qV];
var br=bp[this.__rd];
return bq+br;
},logSelectedRows:function(){var bs=this.__pp;
this.__pu.iterateSelection(function(bt){console.log(J+bt);
console.dir(bs.getRowData(bt));
});
},getSelectedIds:function(){var bv=[];
var bu=this;
this.__pu.iterateSelection(function(bw){bv.push(bu.__pp.getValue(bu.__pv,bw));
});
return bv;
...
},__px:function(event){var bx=event.getData();

try{this.__pp.removeRows(0,undefined,undefined,false);
}catch(by){this.debug("Is the table empty?");
}this.__rn(bx.rows);
},__qO:function(event){var bz=event.getData();
this.__rn(bz.rows);
},__rn:function(bA){var bB=[];
...
var bD=this;
var bE=this.getSelectedIds();
this.__re(bE);
},__py:function(event){var bF=event.getData().rows;
this.__re(bF);
},__re:function(bG){for(var i in bG){this.debug("Purge from the table entry by real id "+bH);
var bH=bG[i];
var bI=this.__pp.locate(this.__pv,bH);
if(bI!=l)this.__pp.removeRows(bI,1);
}},__pz:function(event){var bJ=event.getOldData();
this.__pp.addRowsAsMapArray([bJ]);
},particallyUpdateRows:function(bK){for(var i=0,bL=bK.length;i<bL;i++){var bP=bK[i];

if(typeof (bP.id)==l)this.error("Cannot get Rows[i].id");
var bO=this.__pp.locate(this.__pv,bP.id);

if(isNaN(bO)){this.error("Cannot locate row.");
continue;
}var bM=this.__pp.getRowData(bO,undefined,false);

if(typeof (bM)!=D){this.error("Cannot retrieve old values.");
continue;
}var bN=bM.slice(0);
bN=this.__rl(bP,bN);
bO=this.__pp.locate(this.__pv,bP.id);
this.__pp.setRow(bO,bN);
}},__rl:function(bQ,bR){for(var j in bQ){var bS=this.__rm[j];
bR[bS]=bQ[j];
}bR[this.__qY]=this.__rb(bR);
bR[this.__qX]=this.__ra(bR);
bR[this.__qR]=this.__qT(bR);
bR[this.__pt]=this.__qS(bR);
return bR;
},__pA:function(event){var bT=event.getData();
this.particallyUpdateRows(bT.rows);
}}});
})();
