...
this.add(this.__kJ,{edge:q});
this.__kK=new rhyacotriton.ToolBar(this);
this.add(this.__kK,{edge:q});
...
this.__rP=new rhyacotriton.log.Table(this.__kQ,this.__kO);
this.__kL=new qx.ui.embed.Html(r);
this.__kM=this.__rO;
this.__kN=this.__rP;
...
this.add(this.__kO);
this.__kP=new qx.ui.splitpane.Pane(n);
...
this.setEnabled(false);
},members:{__kI:null,__kJ:null,__kK:null,__kQ:null,__kO:null,__kR:null,_initStore:function(){var t=this;
this.__kQ=new rhyacotriton.store.Remote;
this.__kQ.addListener(j,function(e){var u=e.getCurrentTarget().isActive();
t.setEnabled(u);
},this.__kQ);
},_initToolbarButtonActivation:function(){var v=this.__kO.getSelectionModel();
v.addListener(i,function(e){var w=!(e.getCurrentTarget().isSelectionEmpty());
this.enableRowButtons(w);
},this.__kK);
...
},showAddTorrent:function(){},getCommand:function(x){return this.__kR[x];
},_initializeCommands:function(){var y={};
y.reconnect=new qx.ui.core.Command(d);
y.reconnect.addListener(s,this.reconnect,this);
y.reload=new qx.ui.core.Command(m);
y.reload.addListener(s,this.reload,this);
y.addTorrent=new qx.ui.core.Command(g);
y.addTorrent.addListener(s,this.showAddTorrent,this);
y.removeSelectedRows=new qx.ui.core.Command(h);
y.removeSelectedRows.addListener(s,this.removeSelectedRows,this);
y.startSelectedRows=new qx.ui.core.Command(p);
y.startSelectedRows.addListener(s,this.startSelectedRows,this);
y.stopSelectedRows=new qx.ui.core.Command(l);
y.stopSelectedRows.addListener(s,this.stopSelectedRows,this);
this.__kR=y;
},syncStackView:function(e){var z=e.getData()[0];
var A=z!=null?z.getUserData(c):r;
this.info("Show view: "+A);

switch(A){case a:this.__cd.setSelection([this.__kL]);
this.__cd.show();
break;
...
this.__cd.show();
break;
case o:this.__cd.setSelection([this.__kN]);
this.__cd.show();
break;
default:this.__cd.resetSelection();
this.__cd.exclude();
}},setEnabled:function(B){this.__kK.setEnabled(B);
this.__kO.setEnabled(B);
}}});
})();
...
(function(){var j="dataUpdated",i="dataAdded",h="dataLoadCompleted",g="dataRemoved",f="dataRemoveFailure",e="Out Before",d="Speed Out",c="Left",b="Id",a="Ss",z="Pid",y="Total",x="Speed In",w="Total In",v="In Now",u="Ls",t="Rating",r="Progress",q="In Before",p="Total Out",n="Name",o="rhyacotriton.Table",l="Out Now",m="State",k="On-line";
qx.Class.define(o,{extend:rhyacotriton.BasicTable,construct:function(A){var B={"id":this.tr(b),"pid":this.tr(z),"name":this.tr(n),"total":this.tr(y),"left":this.tr(c),"progress":this.tr(r),"rating":this.tr(t),"online":this.tr(k),"seeders":this.tr(a),"leechers":this.tr(u),"state":this.tr(m),"downloaded":this.tr(v),"uploaded":this.tr(l),"all_time_downloaded":this.tr(q),"all_time_uploaded":this.tr(e),"sum_downloaded":this.tr(w),"sum_uploaded":this.tr(p),"speed_in":this.tr(x),"speed_out":this.tr(d)};
rhyacotriton.BasicTable.call(this,B);
var C=this.getTableModel();
var E=this.getTableColumnModel();
var D=this.getColumnNameToPositionIndex();
E.setColumnWidth(D.name,250,true);
E.setColumnWidth(D.id,30,true);
E.setColumnWidth(D.pid,70,true);
E.setColumnWidth(D.progress,65,true);
E.setColumnWidth(D.total,70,true);
E.setColumnWidth(D.left,70,true);
E.setColumnWidth(D.leechers,40,true);
E.setColumnWidth(D.seeders,40,true);
E.setColumnWidth(D.state,70,true);
E.setColumnWidth(D.rating,60,true);
E.setColumnWidth(D.all_time_downloaded,65,true);
E.setColumnWidth(D.all_time_uploaded,70,true);
E.setColumnWidth(D.downloaded,70,true);
E.setColumnWidth(D.uploaded,70,true);
E.setColumnWidth(D.sum_downloaded,70,true);
E.setColumnWidth(D.sum_uploaded,70,true);
E.setColumnWidth(D.speed_in,70,true);
E.setColumnWidth(D.speed_out,70,true);
E.setDataCellRenderer(D.progress,new rhyacotriton.cellrenderer.Progress());
E.setDataCellRenderer(D.rating,new rhyacotriton.cellrenderer.Rating());
E.setDataCellRenderer(D.online,new qx.ui.table.cellrenderer.Boolean());
[D.total,D.left,D.downloaded,D.uploaded,D.all_time_downloaded,D.all_time_uploaded,D.sum_downloaded,D.sum_uploaded].map(function(F){E.setDataCellRenderer(F,new rhyacotriton.cellrenderer.Size());
});
E.setDataCellRenderer(D.speed_in,new rhyacotriton.cellrenderer.Speed());
E.setDataCellRenderer(D.speed_out,new rhyacotriton.cellrenderer.Speed());
[D.left,D.online,D.downloaded,D.uploaded,D.all_time_downloaded,D.all_time_uploaded].map(function(G){E.setColumnVisible(G,false);
});
C.setSortMethods(D.pid,A.buildPidComparator(D.pid));
var s=this.__kQ=A;
...
},members:{__kQ:undefined,__qd:function(H){var K=this.getColumnNameToPositionIndex();
var I=H[K.total];
var L=H[K.left];
var J=I-L;
return ((J/I)*100).toFixed(2);
},__qe:function(M){var T=this.getColumnNameToPositionIndex();
var S=M[T.uploaded];
var N=M[T.downloaded];
var Q=M[T.all_time_uploaded];
var P=M[T.all_time_downloaded];
var R=N+P;
var O=S+Q;

if(R==0)return 0;
return (O/R).toFixed(2);
},__qf:function(U){var W=this.getColumnNameToPositionIndex();
var V=U[W.downloaded];
var X=U[W.all_time_downloaded];
return V+X;
},__qg:function(Y){var bb=this.getColumnNameToPositionIndex();
var ba=Y[bb.uploaded];
var bc=Y[bb.all_time_uploaded];
return ba+bc;
...
},fillFields:function(bd,be,bf){be=rhyacotriton.BasicTable.prototype.fillFields.call(this,bd,be,bf);
var bg=this.getColumnNameToPositionIndex();

if(bf){be[bg.speed_in]=0;
be[bg.speed_out]=0;
}be[bg.sum_uploaded]=this.__qg(be);
be[bg.sum_downloaded]=this.__qf(be);
be[bg.rating]=this.__qe(be);
be[bg.progress]=this.__qd(be);
return be;
}}});
})();
...
qx.Class.define(e,{extend:rhyacotriton.BasicTable,construct:function(l,m){var n={"id":this.tr(i),"torrent_id":this.tr(b),"ip":this.tr(g),"port":this.tr(k),"state":this.tr(h),"choke_state":this.tr(f),"interest_state":this.tr(j),"local_choke":this.tr(d)};
rhyacotriton.BasicTable.call(this,n);
var p=this.getTableColumnModel();
var q=this.getColumnNameToPositionIndex();
p.setColumnWidth(q.id,70,true);
p.setColumnWidth(q.ip,100,true);
p.setColumnWidth(q.port,50,true);
p.setColumnWidth(q.torrent_id,30,true);
p.setColumnWidth(q.state,60,true);
p.setColumnWidth(q.interest_state,60,true);
p.setColumnWidth(q.choke_state,60,true);
p.setColumnWidth(q.local_choke,40,true);
p.setDataCellRenderer(q.local_choke,new qx.ui.table.cellrenderer.Boolean());
[q.interest_state,q.choke_state,q.local_choke].map(function(r){p.setColumnVisible(r,false);
});
var o=this.getTableModel();
o.setSortMethods(q.id,l.buildPidComparator(q.id));
o.setSortMethods(q.ip,l.buildIPComparator(q.ip));
l.addListener(a,this.getEventHandler(c),this);
this.initFilters(m);
},members:{}});
})();
(function(){var h="Name",g="rhyacotriton.log.Table",f="Information",d="Time",c="logEvent",b="Id",a="HH:mm:ss";
qx.Class.define(g,{extend:rhyacotriton.BasicTable,construct:function(i,j){var m={"torrent_id":this.tr(b),"time":this.tr(d),"name":this.tr(h),"message":this.tr(f)};
rhyacotriton.BasicTable.call(this,m);
var n=this.getTableColumnModel();
var p=this.getColumnNameToPositionIndex();
n.setColumnWidth(p.torrent_id,30,true);
n.setColumnWidth(p.time,55,true);
n.setColumnWidth(p.name,90,true);
n.setColumnWidth(p.message,130,true);
var o=this.getTableModel();
var k=new qx.ui.table.cellrenderer.Date();
var l=new qx.util.format.DateFormat(a);
k.setDateFormat(l);
n.setDataCellRenderer(p.time,k);
i.addListener(c,function(e){var q=e.getData();
q.time=new Date();
this.addRows([q]);
},this);
this.initFilters(j);
